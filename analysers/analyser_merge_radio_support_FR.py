...
class _Analyser_Merge_Radio_Support_FR(Analyser_Merge_Point):
    def __init__(
        self, config, logger, clas, NAT_IDs, title, tags_select, tags_generate
    ):
        ...
        self.init(
            ...
            CSV(
                SourceDataGouv(
                    ...
                    resource="b1d8a2a0-a68f-45a4-9b06-94b13ecb22ec",
                    zip="SUP_SUPPORT.txt",
            ...
            Load_XY(
                (
                    "CASE \"COR_CD_EW_LON\" WHEN 'W' THEN -1*(to_number(\"COR_NB_DG_LON\", '99') + to_number(\"COR_NB_MN_LON\", '99') / 60 \
                        + to_number(\"COR_NB_SC_LON\", '99') / 3600) WHEN 'E' THEN to_number(\"COR_NB_DG_LON\", '99') + to_number(\"COR_NB_MN_LON\", '99') / 60 \
                        + to_number(\"COR_NB_SC_LON\", '99') / 3600 END",
                ),
                (
                    "CASE \"COR_CD_NS_LAT\" WHEN 'S' THEN -1*(to_number(\"COR_NB_DG_LAT\", '99') + to_number(\"COR_NB_MN_LAT\", '99') / 60 \
                        + to_number(\"COR_NB_SC_LAT\", '99') / 3600) WHEN 'N' THEN to_number(\"COR_NB_DG_LAT\", '99') \
                        + to_number(\"COR_NB_MN_LAT\", '99') / 60 + to_number(\"COR_NB_SC_LAT\", '99') / 3600 END",
                ),
            ...
            Conflate(
                ...
                mapping=Mapping(
                    ...
                    mapping1={
                        "ref:FR:ANFR": "SUP_ID",
                        "operator": lambda fields: (
                            self.owner[int(fields["TPO_ID"])]
                            if fields["TPO_ID"] and int(fields["TPO_ID"]) in self.owner
                            else None
                        ),
                        "height": lambda fields: (
                            fields["SUP_NM_HAUT"].replace(",", ".")
                    ...
                    text=lambda tags, fields: {
                        "en": "{0}, address: {1}, {2}{3}".format(
                            (
                                lambda x: (
                                    self.Tour_Mat_Pylone[fields["NAT_ID"]]
                            ...
                            (
                                lambda x: (
                                    self.communeNameIndexedByInsee[x]
                            ...
                            (
                                lambda x: (
                                    (", operator: " + self.other_owner[int(x)])
...
class Analyser_Merge_Tour_Mat_Pylone(_Analyser_Merge_Radio_Support_FR):
    def __init__(self, config, logger=None):
        _Analyser_Merge_Radio_Support_FR.__init__(
            self,
            config,
            logger,
            0,
            ["11", "12", "21", "22", "23", "24", "25", "26", "42", "48"],
            "Tour, mât et pylône",
            [
