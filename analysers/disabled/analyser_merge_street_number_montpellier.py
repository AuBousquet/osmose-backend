...
from .Analyser_Merge import CSV, Conflate, Load, Mapping, Source
...
class Analyser_Merge_Street_Number_Montpellier(_Analyser_Merge_Street_Number):
    def __init__(self, config, logger=None):
        _Analyser_Merge_Street_Number.__init__(
            ...
            "Ville de Montpellier - Point adresse",
            ...
            Load("X", "Y", where=lambda res: res["NUM_VOI"] != "0"),
            Conflate(
                mapping=Mapping(
                    static2={"source": self.source},
                    mapping1={"addr:housenumber": "NUM_SUF"},
                    text=lambda tags, fields: {
                        "en": "{0} {1}".format(fields["NUM_SUF"], fields["LIB_OFF"])
                    },
                )
            ),
        )
