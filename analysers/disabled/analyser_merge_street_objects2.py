...
from .Analyser_Merge import CSV, Conflate, Load, Mapping, Select
...
class Analyser_Merge_Street_Objects2(Analyser_Merge_Dynamic):
    ...
    def __init__(self, config, logger=None):
        ...
        for r in mapingfile:
            self.classFactory(
                SubAnalyser_Merge_Street_Objects2,
                r["class"],
                r["class"],
                r["level"],
                r["otype"],
        ...
        self.init(
            ...
            Conflate(
                ...
                mapping=Mapping(
                    ...
                    mapping1={"survey:date": lambda res: res["last_seen_at"][0:10]},
                    text=lambda tags, fields: (
                        T_(
                            "Observed between {0} and {1}",
                            fields["first_seen_at"][0:10],
                            fields["last_seen_at"][0:10],
                        )
                        if fields["first_seen_at"][0:10] != fields["last_seen_at"][0:10]
                        else T_("Observed on {0}", fields["first_seen_at"][0:10])
                    ),
                ),
            ),
        )
