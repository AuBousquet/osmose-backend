...
class Analyser_Merge_Wikipedia_FR_City(w._Analyser_Merge_Wikipedia_City):
    def __init__(self, config, logger=None):
        ...
        self.sourceWhere = (
            lambda res: not res["titel"].startswith("Canton ")
            and not res["titel"].startswith("Circonscription ")
            and not res["titel"].startswith("Arrondissement ")
        )
...
class Analyser_Merge_Wikipedia_FR_RailwayStation(
    w._Analyser_Merge_Wikipedia_RailwayStation
):
    def __init__(self, config, logger=None):
        w._Analyser_Merge_Wikipedia_RailwayStation.__init__(
            self, config, "FR", "fr", logger
        )
