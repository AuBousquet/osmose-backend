...
from modules import OsmGis, OsmSax
...
class Analyser_Gis_Building_Overlaps(Analyser):
    ...
    def analyser(self):
        ...
        # sql querries
        sql1 = (
            "CREATE TEMP TABLE %s_building AS SELECT osm_id, way FROM %s_polygon WHERE st_isvalid(way)='t' AND st_issimple(way)='t' AND building='yes';"
        ...
        sql3 = (
            "SELECT b1.osm_id AS id1, b2.osm_id AS id2, astext(st_transform(st_pointonsurface(ST_Intersection(b1.way, b2.way)), 4020)) FROM %s_building AS b1, %s_building AS b2 WHERE b1.osm_id>b2.osm_id AND st_intersects(b1.way, b2.way)='t' AND st_area(ST_Intersection(b1.way, b2.way))<>0;"
            % (self.config.db_schema, self.config.dbp)
        )
        sql4 = "DROP TABLE %s_building;" % (self.config.db_schema)

        # gis querries
        self.logger.log("create building table")
        giscurs.execute(sql1)
        self.logger.log("create building index")
        giscurs.execute(sql2)
        self.logger.log("analyse overlap")
        giscurs.execute(sql3)

        # format results to outxml
        self.logger.log("generate xml")
        while True:
