...
class Analyser_Merge_Parking_FR_capp(Analyser_Merge):
    def __init__(self, config, logger=None):
        ...
        self.init(
            ...
            "Parkings sur la CAPP",
            CSV(
                Source(
                    attribution="Communauté d'Agglomération Pau-Pyrénées",
                    millesime="01/2013",
                    fileUrl="http://opendata.agglo-pau.fr/sc/call.php?f=1&idf=18",
            ...
            Conflate(
                ...
                mapping=Mapping(
                    ...
                    mapping1={
                        "name": "NOM",
                        "fee": lambda res: (
                            "yes" if res["Pay_grat"] == "Payant" else "no"
                        ),
                        "capacity": lambda res: (
                            res["Places"] if res["Places"] != "0" else None
                        ),
                        "parking": lambda res: (
                            "surface"
                            if res["Ouvrage"] == "Plein air"
                            else (
                                "underground"
                                if res["Ouvrage"] == "Souterrain"
                                else None
                            )
                        ),
                    },
...
class Analyser_Merge_Parking_FR_capp_disabled(Analyser_Merge):
    def __init__(self, config, logger=None):
        ...
        self.init(
            ...
            "Stationnements règlementaires sur la commune de Pau - Stationnement Handi",
            CSV(
                Source(
                    attribution="Communauté d'Agglomération Pau-Pyrénées",
                    millesime="01/2013",
                    fileUrl="http://opendata.agglo-pau.fr/sc/call.php?f=1&idf=21",
                    zip="Sta_Regl_Wgs84.csv",
                    encoding="ISO-8859-15",
                )
            ),
            Load(
            ...
            Conflate(
                select=Select(
                    types=["nodes", "ways"],
                    tags={"amenity": "parking", "capacity:disabled": None},
                ),
                ...
                mapping=Mapping(
                    ...
                    mapping1={
                        "capacity:disabled": lambda res: (
                            res["nombre"] if res["nombre"] != "0" else "yes"
                        )
                    },
                ),
            ),
