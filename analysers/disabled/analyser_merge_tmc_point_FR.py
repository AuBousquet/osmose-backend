...
from .Analyser_Merge import CSV, Analyser_Merge, Conflate, Load, Select, Source


class _Analyser_Merge_TMC_Point_FR(Analyser_Merge):
    def __init__(
        self, config, logger, level, title, osmTags, osmTypes, c, tcd, stcd, threshold
    ):
        ...
        self.def_class_missing_official(
            ...
            id=tcd * 100 + stcd,
            level=level,
            tags=["merge", "highway", "fix:survey", "fix:picture"],
            title=title,
            detail=T_(
                """TMC data refers to a not found element of the road network. """
            ),
            trap=T_("""TMC data may be wrong or poorly located."""),
        )
        ...
        self.init(
            ...
            CSV(
                Source(
                    fileUrl="http://diffusion-numerique.info-routiere.gouv.fr/IMG/zip/cd_alert_c_v12.1.zip",
            ...
            Load(
                ...
                where=lambda res: res["CLASS"] == c
                and res["TCD"] == str(tcd)
                and res["STCD"] == str(stcd),
            ),
            Conflate(
                select=Select(types=osmTypes, tags=osmTags),
...
class Analyser_Merge_TMC_Point_Bridge_Fr(_Analyser_Merge_TMC_Point_FR):
    def __init__(self, config, logger=None):
        _Analyser_Merge_TMC_Point_FR.__init__(
            self,
            config,
            logger,
            2,
            T_("Bridge missing (TMC)"),
            {
                "highway": [
                    "motorway",
                    "motorway_link",
...
class Analyser_Merge_TMC_Point_Traffic_Signals_Fr(_Analyser_Merge_TMC_Point_FR):
    def __init__(self, config, logger=None):
        _Analyser_Merge_TMC_Point_FR.__init__(
            ...
            logger,
            2,
            T_("Traffic signals missing (TMC)"),
