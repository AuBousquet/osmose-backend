...
class Analyser_Merge_Bicycle_Parking_FR_Brest(Analyser_Merge_Point):
    def __init__(self, config, logger=None):
        ...
        self.def_class_update_official(
            item=8152, id=4, level=3, tags=["merge"], title=T_("Bicycle parking update")
        )
        self.init(
            ...
            Conflate(
                ...
                mapping=Mapping(
                    ...
                    mapping1={
                        "capacity": "NB_PLACES",
                        "bicycle_parking": lambda res: self.bicycle_parking.get(
                            res.get("TYPE_STAT")
                        ),
                    },
                    mapping2={
                        "access": lambda res: (
                            "yes" if res.get("ACCES_PUBL") == "OUI" else None
                        ),
                        "locked": lambda res: (
                            "yes" if res.get("CTRLACCES") == "KorriGo" else None
                        ),
                        "start_date": "DATE_POSE",
                        "covered": lambda res: self.covered.get(res.get("TYPE_STAT")),
                        "operator": lambda res: (
                            "Brest Métropole"
                            if res.get("DOM_PRIVE") == "Public"
                            else None
                        ),
                    },
                ),
