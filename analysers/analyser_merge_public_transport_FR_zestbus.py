...
class Analyser_Merge_Public_Transport_FR_zestbus(Analyser_Merge_Point):
    def __init__(self, config, logger=None):
        ...
        self.init(
            ...
            GTFS(
                SourceDataGouv(
                    attribution="Keolis Menton Riviera",
                    millesime="08/2022",
                    dataset="5bb49085634f4115ddc9293b",
                    resource="72609821-2459-47fb-a63b-3dbbc0d96c92",
                )
            ),
            Load_XY(
                "stop_lon", "stop_lat", select={"parent_station": {"like": "ARCOM%"}}
            ),
            Conflate(
                select=Select(
                    types=["nodes", "ways"],
                    tags=[
                        ...
                        {"highway": "bus_stop", "public_transport": False},
                        {"highway": "platform"},
                    ],
                ),
                osmRef="ref:FR:ZEST",
                ...
                mapping=Mapping(
                    ...
                    mapping1={
                        ...
                        "wheelchair": lambda res: (
                            "yes"
                            if res["wheelchair_boarding"] == "1"
                            else "no" if res["wheelchair_boarding"] == "0" else None
                        ),
                    },
                    text=lambda tags, fields: T_(
                        f"{place} stop of {fields['stop_name']}"
                    ),
                ),
            ),
        )
