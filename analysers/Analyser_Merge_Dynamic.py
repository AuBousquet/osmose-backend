...
#########################################################################
#                                                                       #
# Copyrights Copyrights Frédéric Rodrigo 2014                           #
#                                                                       #
...
from .Analyser_Merge import Analyser_Merge_Point, Load_XY, Mapping, Source
...
class Analyser_Merge_Dynamic(Analyser):
    ...
    def classFactory(self, classs, subclass_name, *args):
        ...
        generatedClass = type(
            "{0}_{1}".format(classs.__name__, subclass_name),
            (classs,),
            {"__init__": __init__},
        )
    ...
    def timestamp(self):
        if self.analysers:
            with self.analysers[0](
                self.config, self.error_file, self.logger
            ) as analyser_obj:
...
class SubAnalyser_Merge_Dynamic(Analyser_Merge_Point):
    ...
    def init(self, url, name, source=Source(), load=Load_XY(), mapping=Mapping()):
        if not load.table_name:  # Rename all table of sub analysers the same
            ...
            country_hash = (
                self.config.db_schema.split("_")[-1][0:10]
                + hexastablehash(self.config.db_schema)[-4:]
            )
