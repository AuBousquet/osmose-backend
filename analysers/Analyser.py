...
class Analyser(object):
    ...
    def def_class_(cls, config, back_in_stack=2, **kwargs):
        ...
        Parameters:
        - cls: class instance;
        - config: a config object, as defined in osmose_config.py;
        - back_in_stack: ?;
        - kwargs: a list of arguments that will be used to initialize the analyser.

        Returns:
        - list of validated arguments to initialize the class
        """
        # Check if keys correspond to the pre-defined list above
        diff_keys = set(kwargs.keys()) - set(
            [
                "item",
                "id",
                "level",
        ...
        if "source" not in kwargs:
            ...
            kwargs["source"] = "{0}/analysers/{1}#L{2}".format(
    ...
    def merge_doc(cls, *docs):
        ...
        return dict(
            map(
                lambda l: [l, "\n\n".join(map(lambda d: d.get(l, d.get("en")), docs))],
                langs,
            )
        )
...
class TestAnalyser(unittest.TestCase):
    ...
    @staticmethod
    def compare_list(a, b, ctx=""):
        for k in range(min(len(a), len(b))):
            if a[k] != b[k]:
                if hasattr(a[k], "items") and hasattr(b[k], "items"):
                    return TestAnalyser.compare_dict(
                        a[k], b[k], "{0}.{1}".format(ctx, k)
                    )
                elif isinstance(a[k], list) and isinstance(b[k], list):
                    return TestAnalyser.compare_list(
                        a[k], b[k], "{0}.{1}".format(ctx, k)
                    )
                else:
                    return "key '{0}' is different: '{1}' != '{2}' [{3}]".format(
                        k, a[k], b[k], ctx
                    )
        if len(a) != len(b):
    ...
    def convert_change_to_normal(a):
        ...
        if "analyser" not in a["analysers"]:
            ...
            if not isinstance(a["analysers"]["analyser"]["class"], list):
                a["analysers"]["analyser"]["class"] = [
                    a["analysers"]["analyser"]["class"]
                ]

            if isinstance(a["analysers"]["analyserChange"]["class"], list):
                a["analysers"]["analyser"]["class"].extend(
                    a["analysers"]["analyserChange"]["class"]
                )
            else:
                a["analysers"]["analyser"]["class"].append(
                    a["analysers"]["analyserChange"]["class"]
                )

            if "error" in a["analysers"]["analyser"]:
                if not isinstance(a["analysers"]["analyser"]["error"], list):
                    a["analysers"]["analyser"]["error"] = [
                        a["analysers"]["analyser"]["error"]
                    ]

                if "error" in a["analysers"]["analyserChange"]:
                    if isinstance(a["analysers"]["analyserChange"]["error"], list):
                        a["analysers"]["analyser"]["error"].extend(
                            a["analysers"]["analyserChange"]["error"]
                        )
                    else:
                        a["analysers"]["analyser"]["error"].append(
                            a["analysers"]["analyserChange"]["error"]
                        )

            elif "error" in a["analysers"]["analyserChange"]:
                a["analysers"]["analyser"]["error"] = a["analysers"]["analyserChange"][
                    "error"
                ]
    ...
    def remove_non_checked_entries(a):
        ...
        if "error" in a["analysers"][name_analyser]:
            if not isinstance(a["analysers"][name_analyser]["error"], list):
                a["analysers"][name_analyser]["error"] = [
                    a["analysers"][name_analyser]["error"]
                ]
    ...
    def compare_results(
        self, orig_xml=None, checked_xml=None, convert_checked_to_normal=False
    ):
        if orig_xml is None:
            raise  # TODO
                            ...
                            for fi in f.findall(t):
                                for fix in fi.findall("tag"):
                                    if fix.attrib["action"] == "delete":
                                        ...
                                        fixactions["~"][fix.attrib["k"]] = fix.attrib[
                                            "v"
                                        ]
    ...
    def check_num_err(self, num=None, min=None, max=None):
        ...
        if num is not None:
            self.assertEqual(
                xml_num, num, "Found {0} errors instead of {1}".format(xml_num, num)
            )
        if min is not None:
            self.assertGreaterEqual(
                xml_num, min, "Found {0} errors instead of >= {1}".format(xml_num, min)
            )
        if max is not None:
            self.assertLessEqual(
                xml_num, max, "Found {0} errors instead of <= {1}".format(xml_num, max)
            )
        ...
        if root_analyser is not None:
            err_semihash = list(  # assumes source and item are identical
                map(
                    ...
                    + e.attrib.get("class", "")
                    + " subcl="
                    + e.attrib.get("subclass", "")
                    + " "
                    + " ".join(
                        map(
                            lambda elem: "r" + elem.attrib.get("id", ""),
                            e.findall("relation"),
                        )
                    )
                    + " "
                    + " ".join(
                        map(
                            lambda elem: "w" + elem.attrib.get("id", ""),
                            e.findall("way"),
                        )
                    )
                    + " "
                    + " ".join(
                        map(
                            lambda elem: "n" + elem.attrib.get("id", ""),
                            e.findall("node"),
                        )
                    ),
            ...
            assert len(non_unique) == 0, (
                "Multiple errors having the same (sub)class and elements\n- "
                + "\n- ".join(non_unique)
            )
...
class Test(unittest.TestCase):
    def test_compare_dict(self):
        a = TestAnalyser
        self.assertEqual(a.compare_dict({1: 1, 2: 2}, {1: 1, 2: 2}), "")
        self.assertEqual(a.compare_dict({1: 1, 2: 2}, {2: 2, 1: 1}), "")
        self.assertEqual(
            a.compare_dict({1: 1, 2: 2}, {1: 0, 2: 2}),
            "key '1' is different: '1' != '0' []",
        )
        self.assertEqual(
            a.compare_dict({1: 1, 2: 2}, {1: 1, 3: 3}), "key '2' is missing from b []"
        )
        self.assertEqual(
            a.compare_dict(
                {
        ...
        self.assertEqual(
            ...
            "key '2' is missing from b []",
        )

        self.assertEqual(a.compare_dict({1: [3, 4], 2: 2}, {1: [3, 4], 2: 2}), "")
        self.assertEqual(
            a.compare_dict({1: [3, 4], 2: 2}, {1: [3, 5], 2: 2}),
            "key '1' is different: '4' != '5' [.1]",
        )
        self.assertEqual(
            a.compare_dict({1: [3], 2: 2}, {1: [3, 4], 2: 2}),
            "length are different: 1 != 2 [.1]",
        )
        self.assertEqual(
            a.compare_dict({1: [3, 4], 2: 2}, {1: [3], 2: 2}),
            "length are different: 2 != 1 [.1]",
        )

        self.assertEqual(a.compare_dict({1: {3: 4}, 2: 2}, {1: {3: 4}, 2: 2}), "")
        self.assertEqual(
            a.compare_dict({1: {3: 4}, 2: 2}, {1: {3: 5}, 2: 2}),
            "key '3' is different: '4' != '5' [.1]",
        )
        self.assertEqual(
            a.compare_dict({1: {3: 4}, 2: 2}, {1: {4: 5}, 2: 2}),
            "key '3' is missing from b [.1]",
        )
        self.assertEqual(
            a.compare_dict({1: {}, 2: 2}, {1: {3: 4}, 2: 2}),
            "key '3' is missing from a [.1]",
        )
        self.assertEqual(
            a.compare_dict({1: {3: 4}, 2: 2}, {1: {}, 2: 2}),
            "key '3' is missing from b [.1]",
        )
    ...
    def test_merge_doc(self):
        ...
        self.assertEqual(
            Analyser.merge_doc({"en": "1", "fr": "2"}, {"en": "3"}),
            {"en": "1\n\n3", "fr": "2\n\n3"},
        )
        self.assertEqual(
            Analyser.merge_doc({"en": "1", "fr": "2"}, {"en": "3", "fr": "4"}),
            {"en": "1\n\n3", "fr": "2\n\n4"},
        )

        self.assertEqual(
            Analyser.merge_docs({"detail": {"en": "a"}}, fix={"en": "b"}),
            {"detail": {"en": "a"}, "fix": {"en": "b"}},
        )
        self.assertEqual(
            Analyser.merge_docs({"detail": {"en": "a"}}, detail={"en": "b"}),
            {"detail": {"en": "a\n\nb"}},
        )
