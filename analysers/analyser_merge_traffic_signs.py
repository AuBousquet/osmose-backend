...
class Analyser_Merge_Traffic_Signs(Analyser_Merge_Dynamic):
    ...
    def check_not_only_for(self, not_for, only_for):
        ...
        if not_for:
            return not country or not any(
                map(lambda co: country.startswith(co), not_for)
            )
        return True
    ...
    def value_replace(self, v, key, replacement):
        if isinstance(v, dict):
            return dict(
                map(
                    lambda kv: [kv[0], self.value_replace(kv[1], key, replacement)],
                    v.items(),
                )
            )
    ...
    def dict_replace(self, d, f, r):
        return dict(
            map(
                lambda kv: [kv[0], kv[1] and self.value_replace(kv[1], f, r)], d.items()
            )
        )
    ...
    def __init__(self, config, logger=None):
        ...
        for r in mapingfile:
            if self.check_not_only_for(r.get("not_for"), r.get("only_for")):
                ...
                r["select_tags"] = list(
                    map(
                        lambda select: self.dict_replace(
                            select, "{speed_limit_unit}", unit
                        ),
                        r["select_tags"],
                    )
                )
                r["generate_tags"] = self.dict_replace(
                    r["generate_tags"], "{speed_limit_unit}", unit
                )

                self.classFactory(
                    SubAnalyser_Merge_Traffic_Signs,
                    r["class"],
                    r["item"],
                    r["class"],
                    r["level"],
    ...
    def __init__(
        ...
        for selection in selectTags:
            missing_tags.append(
                " + ".join(
                    [
                        "`{}={}`".format(kv[0], kv[1] if kv[1] else "*")
                        for kv in selection.items()
                    ]
                )
            )
        ...
        self.def_class_missing_official(
            ...
            id=classs,
            level=level,
            tags=["merge", "highway", "fix:picture", "fix:survey"] + tags,
            title=T_("Unmapped {0}", T_(title)),
            detail=T_(
                "Traffic sign ({1}) detected by Mapillary, but no nearby tagging of any:{0}",
                "\n\n- " + "\n- ".join(missing_tags),
                T_(title),
            ),
            fix=T_(
        ...
        self.init(
            ...
            CSV(
                Source(
                    attribution="Mapillary Traffic Signs",
                    fileUrl=f"http://proxy.osmose.openstreetmap.fr/mapillary/mapillary_signs-{config.osmosis_manager.db_schema}.csv.bz2",
                    bz2=True,
                )
            ),
