...
class Geocode_Addok_CSV(Source):
    ...
    def __init__(
        self, source, columns, logger, citycode=None, delimiter=",", encoding="utf-8"
    ):
        self.source = source
    ...
    def open(self):
        return open(
            downloader.update_cache(
                "geocoded://" + self.source.fileUrl, 60, fetch=self.fetch
            )
        )

    def fetch(self, url, tmp_file, date_string=None):
        service = "https://api-adresse.data.gouv.fr/search/csv/"
        ...
        for i in range(0, slices):
            ...
            r = downloader.requests_retry_session().post(
                url=service,
                data={
