...
class Analyser_Merge_Museum_FR(Analyser_Merge_Point):
    def __init__(self, config, logger=None):
        ...
        self.def_class_missing_official(
            ...
            id=31,
            level=3,
            tags=["merge", "tourism", "fix:survey", "fix:picture"],
        ...
        self.def_class_possible_merge(
            ...
            id=33,
            level=3,
            tags=["merge", "tourism", "fix:survey", "fix:picture", "fix:chair"],
        ...
        self.init(
            ...
            Conflate(
                select=Select(
                    types=["nodes", "ways", "relations"], tags={"tourism": "museum"}
                ),
                ...
                mapping=Mapping(
                    ...
                    mapping2={
                        "website": lambda res: (
                            ...
                            if not res["url_m"]
                                ...
                                res["url_m"]
                                if res["url_m"].startswith("http")
                                else "http://" + res["url_m"]
                            )
                        ),
                        "phone": lambda res: (
                            "+33 " + res["tel_m"][1:]
                            if res["tel_m"] and re_phone.match(res["tel_m"])
                            else None
                        ),
                        "name": lambda res: (
                            res["autnom"][0].upper() + res["autnom"][1:]
                            if res["autnom"]
                            else (
                                res["nomoff"][0].upper() + res["nomoff"][1:]
                                if res["nomoff"]
                                else None
                            )
                        ),
                        "official_name": lambda res: (
                            res["nomoff"][0].upper() + res["nomoff"][1:]
                            if res["autnom"]
                            and res["nomoff"].lower() != res["autnom"].lower()
                            else None
                        ),
                    },
                    text=lambda tags, fields: {
                        "en": " ".join(
                            filter(
                                lambda x: x,
                                [fields["adrl1_m"], fields["cp_m"], fields["ville_m"]],
                            )
                        )
                    },
                ),
