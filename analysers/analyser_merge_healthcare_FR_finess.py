...
from .Analyser_Merge import CSV, Conflate, Load_XY, Mapping, Select, SourceDataGouv
...
class Analyser_Merge_Healthcare_FR_Finess(Analyser_Merge_Dynamic):
    ...
    def __init__(self, config, logger=None):
        ...
        if config.db_schema == "france_guadeloupe":
            ...
            def is_in(dep):
                return dep not in ("9A", "9B", "9C", "9D")

        mapingfile = json.loads(
            open("merge_data/healthcare_FR_finess.mapping.json").read()
        )
        for r in mapingfile:
            self.classFactory(
                SubAnalyser_Merge_Healthcare_FR_Finess,
                r["classes"],
                srid,
                is_in,
                r["categories"],
                r["items"],
                r.get("missing_osm", True),
                r["classes"],
                r["level"],
                r["title:fr"],
                r["tags_select"],
    ...
    def __init__(
        ...
        if missing_osm is not False:
            self.def_class_missing_osm(
                ...
                id=classs + 2,
                level=level,
                tags=["merge", "public equipment", "fix:chair"],
        ...
        self.def_class_possible_merge(
            ...
            id=classs + 3,
            level=level,
            tags=["merge", "public equipment", "fix:chair", "fix:survey"],
        ...
        self.init(
            ...
            Conflate(
                select=Select(types=["nodes", "ways", "relations"], tags=tags_select),
...
class Source_Finess(SourceDataGouv):
    def open(self):
        ...
        structureet = [
            "nofinesset,nofinessej,rs,rslongue,complrs,compldistrib,numvoie,typvoie,voie,compvoie,\
            lieuditbp,commune,departement,libdepartement,ligneacheminement,telephone,telecopie,categetab,\
            libcategetab,categagretab,libcategagretab,siret,codeape,codemft,libmft,codesph,libsph,dateouv,\
            dateautor,datemaj,numuai,coordxet,coordyet,sourcecoordet,datemajcoord".split(
                ","
            )
        ]
        geolocalisation = {}
