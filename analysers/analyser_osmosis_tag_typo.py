...
class Analyser_Osmosis_Tag_Typo(Analyser_Osmosis):
    ...
    def __init__(self, config, logger=None):
        ...
        self.classs[1] = self.def_class(
            ...
            detail=T_("""The tag is misspelled. Detection is based on statistics."""),
            trap=T_(
    ...
    def get_keys_wiki_taginfo(self):
        ...
        keyset = set(
            map(lambda x: x["key"].split(":", 1)[0], json_entrylist)
        )  # Get all (unique) keys, discard :suffixes
        return list(
            filter(lambda x: len(x) > self.minKeyLength, keyset)
        )  # Ignore small keys / wildcards
    ...
    def analyser_osmosis_common(self):
        ...
        self.run(
            sql30.format(type="nodes", as_text="geom", table="nodes", geo="geom"),
            lambda res: {
                "class": 1,
                "subclass": stablehash64(res[1]),
                "data": [self.node_full, None, None, None, None, self.positionAsText],
                "text": T_(
                    "`{0}` is more common than `{1}`, is `{1}` a typo?",
                    res[1].replace(res[3], res[4], 1),
                    res[1],
                ),
                "fix": {
                    "-": [res[1]],
