...
class Analyser_Merge_Bridge_ES(Analyser_Merge_Point):
    def __init__(self, config, logger=None):
        ...
        self.def_class_missing_official(
            ...
            id=1,
            level=3,
            tags=["merge", "highway", "fix:survey", "fix:imagery"],
        ...
        self.init(
            ...
            SHP(
                Source(
                    ...
                    millesime="2023-05-22",
                    fileUrl="http://centrodedescargas.cnig.es/CentroDescargas/descargaDir",
                    post={
                        "secuencialDescDir": self.secuencialDescDir(),
            ...
            Load_XY(
                ("ST_X(ST_LineInterpolatePoint(ST_GeometryN(geom, 1), 0.5))",),
                ("ST_Y(ST_LineInterpolatePoint(ST_GeometryN(geom, 1), 0.5))",),
                select={"tipo_cruce": "26002"},
            ),  # Puente
            Conflate(
                select=Select(
                    types=["ways"],
                ...
                mapping=Mapping(
                    ...
                    text=lambda tags, fields: {
                        "en": ", ".join(
                            filter(lambda x: x and x != "None", [fields["nombre"]])
                        )
                    },
                ),
            ),
