...
`highway=secondary` should not become a residential."""
        ...
        self.classs[1] = self.def_class(
            ...
            **self.merge_docs(
                doc,
                detail={
                    "en": """motorway, motorway_link, trunk, trunk_link, primary,
primary_link"""
                },
            )
        )
        self.classs[2] = self.def_class(
            ...
            level=2,
            tags=["highway", "fix:chair"],
            **self.merge_docs(doc, detail={"en": """secondary, secondary_link"""})
        )
        self.classs[3] = self.def_class(
            item=1120,
            level=2,
            tags=["highway", "fix:chair"],
            **self.merge_docs(doc, detail={"en": """tertiary, tertiary_link"""})
        )
