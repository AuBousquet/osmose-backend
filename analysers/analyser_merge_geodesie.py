...
French survey point imported in OSM but not found."""
    ...
    fix=T_("""Restore node or relation."""),
    trap=T_(
...
class Analyser_Merge_Geodesie(Analyser_Merge_Point):
    def __init__(self, config, logger=None):
        ...
        self.def_class_missing_official(
            ...
            id=1,
            level=3,
            tags=["merge", "infrastructure", "fix:survey", "fix:picture"],
            title=T_("Missing survey point"),
            **doc
        )
        self.def_class_moved_official(
            item=8070,
            id=3,
            level=3,
            tags=["merge", "infrastructure", "fix:survey", "fix:picture"],
        ...
        self.init(
            ...
            Load_XY(
                "lon",
                "lat",
                where=lambda res: not (
                    "Point constaté détruit" in res["description"]
                    or "Point non retrouvé" in res["description"]
                ),  # No more unique ref
                create="""
...
class Analyser_Merge_Geodesie_Site(Analyser_Merge_Point):
    def __init__(self, config, logger=None):
        ...
        self.init(
            ...
            Conflate(
                select=Select(
                    types=["relations"], tags={"type": "site", "site": "geodesic"}
                ),
                osmRef="ref",
                mapping=Mapping(
                    static1={"type": "site", "site": "geodesic"},
                    static2={"source": self.source},
                    mapping1={"ref": "ref", "name": "name", "network": "network"},
                    mapping2={"note": "note"},
                    text=lambda tags, fields: {
                        "en": "Survey site {0} - {1}".format(
                            fields["ref"], fields["name"]
                        ),
                        "fr": "Site géodésique {0} - {1}".format(
                            fields["ref"], fields["name"]
                        ),
                        "es": "Sitio geodésico {0} - {1}".format(
                            fields["ref"], fields["name"]
                        ),
                    },
                ),
