...
class Analyser_Merge_Power_Substation_FR(Analyser_Merge_Point):
    def __init__(self, config, logger=None):
        ...
        self.def_class_missing_osm(
            ...
            title=T_(
                "Power substation is not known by the operator or misses substation=* value"
            ),
        )
        ...
        self.def_class_missing_official(
            ...
            id=1,
            level=3,
            tags=["merge", "power", "fix:survey", "fix:picture", "fix:imagery"],
                    ...
                    mapping2={
                        "voltage": lambda fields: (
                            str(
                                (
                                    int(
                                        float(fields["Tension (kV)"].replace("kV", ""))
                                        * 1000
                                    )
                                )
                            )
                            if fields["Tension (kV)"]
                            not in ("HORS TENSION", "<45kV", "COURANT CONTINU")
                            else None
                        )
                    },
                    text=lambda tags, fields: T_(
                        "Power substation of {0}", fields["Nom poste"]
                    ),
                ),
            ),
        )
