...
from modules import DictCursorUnicode, PointInPolygon, SourceVersion, downloader
from modules.OsmoseTranslation import T_
from modules.Stablehash import hexastablehash, stablehash64
...
GENERATE_DELETE_TAG = "DELETE TAG aechohve0Eire4ooyeyaey1gieme0xoo"
...
(
    ...
    SELECT
        ...
        CASE
            WHEN possible_merge.id IS NOT NULL
                THEN possible_merge.official_tags || possible_merge.osm_tags || missing_osm.tags
...
WHERE
    official.tags1
        ...
        - (SELECT coalesce(array_agg(key), array[]::text[])
           FROM jsonb_object_keys(osm_item.tags) AS t(key)
           WHERE official.tags1->>key != '"""
    + GENERATE_DELETE_TAG
    + """')
          -- Remove tags to delete not existing in OSM
        - (SELECT coalesce(array_agg(key), array[]::text[])
           FROM jsonb_each(official.tags1)
           WHERE value->>0 = '"""
    + GENERATE_DELETE_TAG
    + """' AND NOT osm_item.tags?key)
        - 'source'::text
        != '{{}}'::jsonb
"""
)
...
class Source:
    ...
    def zipFile(self):
        ...
        if self.file:
            ...
            f = downloader.urlopen(
                self.fileUrl, self.fileUrlCache, mode="rb", post=self.post
            )
    ...
    def time(self):
        if self.file:
            return int(os.path.getmtime(self.file) + 0.5)
        elif self.fileUrl:
            if self.zipFile():
                date_time = self.zipFile().date_time
                return int(time.mktime(date_time + (0, 0, -1)) + 0.5)
            else:
                return int(
                    downloader.urlmtime(self.fileUrl, self.fileUrlCache, self.post)
                    + 0.5
                )
    ...
    def _get_millesime(self) -> Optional[str]:
        if not self.millesime and self.fileUrl:
            cached_millesime = downloader.get_millesime(
    ...
    def as_tag_value(self):
        if "{0}" in self.attribution:
            ...
            return " - ".join(
                filter(
                    lambda x: x is not None, [self.attribution, self._get_millesime()]
                )
            )
...
class SourceDataGouv(Source):
    ...
    def __init__(
        ...
        dataset: str,
        resource: str,
        data_gouv_api_base: str = "https://www.data.gouv.fr/api/1",
        data_gouv_dataset_base: str = "https://www.data.gouv.fr/fr/datasets/r/",
        **kwargs,
    ):
    ...
    def get_millesime(self) -> datetime.datetime:
        response = downloader.request_get(
            f"{self.data_gouv_api_base}/datasets/{self.dataset}/resources/{self.resource}/"
        )
...
class SourcePublicLu(SourceDataGouv):
    def __init__(self, dataset: str, resource: str, **kwargs):
        super().__init__(
            dataset,
            resource,
            data_gouv_api_base="https://data.public.lu/api/1",
            data_gouv_dataset_base="https://data.public.lu/fr/datasets/r/",
            **kwargs,
        )
...
class SourceOpenDataSoft(Source):
    ...
    def __init__(self, url: str, format: str = "csv", **kwargs):
    ...
    def get_millesime(self) -> datetime.datetime:
        response = downloader.request_get(
            f"{self.base_url}/api/datasets/1.0/{self.dataset}"
        )
        response.raise_for_status()
        return datetime.datetime.fromisoformat(
            response.json()["metas"]["data_processed"]
        )


class SourceDataFair(Source):
    def __init__(self, url: str, file_name: str, format: str = "csv", **kwargs):
    ...
    def get_millesime(self) -> datetime.datetime:
        ...
        response.raise_for_status()
        return datetime.datetime.fromisoformat(
            next(filter(lambda f: f["name"] == self.file_name, response.json()))[
                "updatedAt"
            ][0:10]
        )
...
class SourceHttpLastModified(Source):
    ...
    def get_millesime(self):
        return datetime.datetime.strptime(
            downloader.requests_retry_session()
            .head(self.fileUrl)
...
class SourceIGN(Source):
    def __init__(self, dep_code, **kwargs):
        ...
        match = re.search(
            rf"https://wxs.ign.fr/859x8t863h6a09o9o6fy4v60/telechargement/prepackage/BDTOPOV3-TOUSTHEMES-DEPARTEMENT_GPKG_PACK_233\$BDTOPO_3-[0-9]+_TOUSTHEMES_GPKG_[A-Z0-9]+_D{dep_code}_[-0-9]+/file/BDTOPO_3-[0-9]+_TOUSTHEMES_GPKG_[A-Z0-9]+_D{dep_code}_([-0-9]+)\.7z",
            response.text,
        )
...
class CSV(Parser):
    ...
    def header(self):
        ...
        if self.have_header:
            return next(
                csv.reader(self.f, delimiter=self.separator, quotechar=self.quote)
            )
    ...
    def import_(self, table, osmosis):
        ...
        copy = "COPY {0} FROM STDIN WITH {1} {2} {3} {4} {5}".format(
            table,
            (
                ("DELIMITER AS '{0}'".format(self.separator))
        ...
        if self.fields:
            osmosis.run0(
                "CREATE TABLE {0}_fields AS SELECT {1} FROM {0}".format(
                    table, ", ".join(map(lambda field: '"' + field + '"', self.fields))
                )
            )
...
class JSON(Parser):
    ...
    def header(self):
        self.json = list(
            map(flattenjson, self.extractor(json.loads(self.source.open().read())))
        )
        columns = set()
    ...
    def import_(self, table, osmosis):
        self.json = self.json or map(
            flattenjson, self.extractor(json.loads(self.source.open().read()))
        )
        for row in self.json:
            osmosis.giscurs.execute(
                'insert into "{0}" ("{1}") values ({2})'.format(
                    table, '", "'.join(row.keys()), ("%s, " * len(row.keys()))[:-2]
                ),
...
class GeoJSON(Parser):
    ...
    def header(self):
        ...
        columns = list(columns)
        columns.append("geom_x")
        columns.append("geom_y")
        return columns
    ...
    def import_(self, table, osmosis):
        for row in self.json["features"]:
            if (
                row["geometry"]
                and row["geometry"]["coordinates"]
                and len(row["geometry"]["coordinates"]) > 0
            ):
                ...
                values = list(
                    map(
                        removequotesjson,
                        map(lambda column: row["properties"][column], columns),
                    )
                )
                columns.append("geom_x")
                columns.append("geom_y")
                if row["geometry"]["type"] in (
                    "Point",
                    "MultiPoint",
                    "LineString",
                    "MultiLineString",
                    ...
                    osmosis.giscurs.execute(
                        'insert into "{0}" ("{1}") values ({2})'.format(
                            table, '", "'.join(columns), ("%s, " * len(columns))[:-2]
                        ),
                        values,
                    )
...
class GDAL(Parser):
    def __init__(
        self, source, zip=None, layer=None, fields=None, srid: Optional[int] = None
    ):
        """
    ...
    def import_(self, table, osmosis):
        try:
            self.tmp_file = tempfile.NamedTemporaryFile(
                suffix=".zip" if self.zip else "", mode="wb", delete=False
            )
            shutil.copyfileobj(
                self.source.open(binary=True), self.tmp_file, 20 * 1024 * 1024
            )
            self.tmp_file.close()
            ...
            if not self._srid:
                match = re.search(
                    "EPSG:([0-9]+)",
                    subprocess.run(
                        ["gdalsrsinfo", "-e", self.source_layer[0]],
                ...
                if not match and len(self.source_layer) >= 2:
                    match = re.search(
                        "EPSG:([0-9]+)",
                        subprocess.run(
                            ["gdalsrsinfo", "-e", *self.source_layer],
                            stdout=subprocess.PIPE,
                        ).stdout.decode("utf-8"),
                    )
                if match:
        ...
        wkt = (
            PointInPolygon.PointInPolygon(self.polygon_id).polygon.as_simplified_wkt(
                self.source_srid(), self.proj
            )
            if self.polygon_id
            else None
        )
...
class Load(object):
    def __init__(
        ...
        select={},
        unique=None,
        where=lambda res: True,
        map=lambda i: i,
        geomFunction=lambda i: i,
        validationGeomSQL="1=1",
    ):
        """
    ...
    def run(self, osmosis, conflate, db_schema, default_table_base_name, version):
        ...
        country_hash = (
            osmosis.config.db_schema.split("_")[-1][0:10]
            + hexastablehash(osmosis.config.db_schema)[-4:]
        )
        if (
            len(default_table_base_name + "_" + country_hash) <= 63 - 2 - 3
        ):  # 63 max postgres relation name, 3 is index name prefix
            tableOfficial = default_table_base_name + "_" + country_hash + "_o"
        else:
            tableOfficial = (
                (default_table_base_name + "_" + country_hash)[-(63 - 2 - 3 - 4) :]
                + "_o"
                + hexastablehash(default_table_base_name)[-4:]
            )
        ...
        osmosis.run0(
            "SELECT bbox FROM meta WHERE name='{0}' AND bbox IS NOT NULL AND update IS NOT NULL AND update={1}".format(
                tableOfficial, version
            ),
            lambda res: setData(res),
        )
        ...
        if not self.data:
                ...
                if header:
                    if header is not True:
                        ...
                        for i, v in enumerate(header):
                            ...
                            header_without_duplicate.append(
                                v + str(count + 1)
                                if totalcount > 1 and count > 0
                                else v
                            )
                        self.create = ",".join(
                            map(
                                lambda c: '"{0}" VARCHAR'.format(
                                    DictCursorUnicode.identifier(c)
                                ),
                                header_without_duplicate,
                            )
                        )
            ...
            def insertOfficial(res):
                ...
                res = {
                    ...
                    if k not in ["_geom", "geom", "geometry"]
                }
                if geom and geom[0] and geom[1]:
                    for k in res.keys():
                        try:
                            res[k] = mult_space.sub(
                                " ", res[k].strip()
                    ...
                    giscurs.execute(
                        sql02.format(
                            official=tableOfficial,
                            proj=self.proj,
                            geom=self.spatialGeom(geom),
                        ),
                        {
                            "ref": (
                                tags[1].get(conflate.osmRef)
                                if conflate.osmRef != "NULL"
                                else None
                            ),
                            "tags": tags[1],
                            "tags1": tags[0],
            ...
            if self.unique:
                ...
                distinct = order_by = ""
            osmosis.run0(
                (sql01_ref if conflate.osmRef != "NULL" else sql01_geo).format(
                    table=table,
                    geom=self.geom,
                    validationGeomSQL=self.validationGeomSQL,
                    where=Select.where_attributes(self.select),
                    distinct=distinct,
                    order_by=order_by,
                ),
                insertOfficial,
            ...
            if self.parser.imported_srid():
                giscurs.execute(
                    "SELECT ST_AsText(ST_Transform(ST_Envelope(ST_SetSRID(ST_Extent(ST_Transform(ST_Expand(geom, {distance}), 4326)), 4326)), {proj})) FROM {table}".format(
                        table=tableOfficial,
                        proj=self.proj,
            ...
            if self.bbox is not None:
                osmosis.run(
                    "INSERT INTO meta VALUES ('{0}', {1}, '{2}')".format(
                        tableOfficial, version, self.bbox
                    )
                )
    ...
    def degree(val):
        ...
        if val is not None and "°" in val:
            ...
            return (
                reduce(
                    lambda sum, i: sum * 60 + i,
                    map(
                        lambda i: float(i.replace(",", ".")),
                        filter(lambda i: i != "", val.replace("°", "'").split("'")),
                    ),
                    0,
                )
                / 3600
...
class Load_XY(Load):
    ...
    def __init__(
        ...
        where=lambda res: True,
        map=lambda i: i,
        xFunction=lambda i: i,
        yFunction=lambda i: i,
    ):
        """
        Describ the conversion of data set loaded with COPY into the database into an other table more usable for processing.
...
"""
        super().__init__(
            ...
            where,
            map,
            self.geomFunctionPoint,
            validationGeomSQL,
        )

    def spatialGeom(self, geom):
        return (
            f"ST_Transform(ST_GeomFromEWKT('SRID={self.parser.imported_srid()};POINT({geom[0]} {geom[1]})'), {self.proj})"
            if self.parser.imported_srid()
            else "NULL::geometry"
        )
    ...
    def run(self, osmosis, conflate, db_schema, default_table_base_name, version):
        ...
        if not self.pip:
            self.pip = (
                PointInPolygon.PointInPolygon(self.polygon_id)
                if self.parser.imported_srid() and self.polygon_id
                else None
            )
            if self.pip:
                if Transformer:  # type: ignore
                    self.transformer = Transformer.from_crs(
                        self.parser.imported_srid(), 4326, always_xy=True
                    )
                else:  # pyproj < 2.1
        ...
        try:
            if self.pip and not self.transformer:
                self.giscurs_getpoint = osmosis.gisconn.cursor(
                    cursor_factory=psycopg2.extras.DictCursor
                )

            return super().run(
                osmosis, conflate, db_schema, default_table_base_name, version
            )
        finally:
        ...
        if not self.pip or (x and y):
            ...
            if self.pip:
                if self.transformer:
                    ...
                    self.giscurs_getpoint.execute(
                        "SELECT ST_AsText(ST_Transform(ST_SetSRID(ST_MakePoint({x}, {y}), {srid}), 4326))".format(
                            x=x, y=y, srid=self.parser.imported_srid()
                        )
                    )
                    lonLat = self.osmosis.get_points(
                        self.giscurs_getpoint.fetchone()[0]
                    )[0]
...
class LoadGeomCentroid(Load_XY):
    def __init__(self, *args, **kwargs):
        super(LoadGeomCentroid, self).__init__(
...
class Select:
    ...
    def where_tags(query):
        return Select._where(
            query, attribut_not_exists="NOT tags?'{}'", attribut_value="tags->'{}'"
        )

    @staticmethod
    def where_attributes(query):
        return Select._where(
            query, attribut_not_exists='"{}" IS NULL', attribut_value='"{}"'
        )
    ...
    def _where(query, attribut_not_exists, attribut_value):
        ...
        return (
            ...
            + (
                ") OR (".join(
                    map(
                        lambda x: Select._where_and(
                            x, attribut_not_exists, attribut_value
                        ),
                        query,
                    )
                )
            ...
            if v is False:
                ...
                if callable(v):
                    ...
                    cond = (
                        ...
                        + "::text, ';')"
                        + " && ARRAY['{}']".format(
                            "', '".join(map(lambda i: i.replace("'", "''"), actualList))
                        )
                    )
                    ...
                    if v is None or v is True:
                        ...
                        if "like" in v:
                            clauses.append(
                                k_value
                                + " LIKE '{}'".format(v["like"].replace("'", "''"))
                            )
                        elif "regex" in v:
                            clauses.append(
                                k_value
                                + " ~ '{}'".format(v["regex"].replace("'", "''"))
                            )
                    else:
                        clauses.append(
                            "'{}'".format(v.replace("'", "''"))
...
class Mapping:
    ...
    def tagFactoryGroup(self, res, static, mapping):
        ...
        return dict(
            map(
                lambda kv: [kv[0], kv[1] is not None and "{0}".format(kv[1]) or None],
                tags.items(),
            )
        )
...
class Conflate:
    def __init__(
        ...
        select=Select(),
        osmRef="NULL",
        conflationDistance=None,
        extraJoin=None,
        tag_keep_multiple_values=[],
        subclass_hash=lambda d: d,
        mapping=Mapping(),
    ):
        """
...
OpenStreetMap, then you can add it."""
        ...
        trap=T_("""Be sure that it does not already exist in another place."""),
    )
...
verification of this data."""
    ...
    def analyser_osmosis_common(self):
        self.run(
            "SET search_path TO {0}".format(
                self.config.db_schema_path
                or ",".join([self.config.db_user, self.config.db_schema, "public"])
            )
        )
        ...
        if not table:
            self.logger.log("Empty bbox, abort")
            return None
        ...
        self.logger.log("Retrieve OSM item")
        where = Select.where_tags(self.conflate.select.tags)
        self.run(
            "CREATE TEMP TABLE osm_item AS "
            + (
                "UNION ALL".join(
                    map(
                        lambda type: (
                    ...
                    WHERE"""
                            + (
                                """
                        {geomSelect} IS NOT NULL AND"""
                                if self.parser.imported_srid()
                                else ""
                            )
                            + (
                                """
                        ST_Transform(ST_SetSRID(ST_GeomFromText('{bbox}'), {proj}), 4326) && {geomSelect} AND"""
                                if self.load.bbox and self.parser.imported_srid()
                                else ""
                            )
                            + """
                        tags != ''::hstore AND
    ...
    def passTags(self, official):
        official = dict(
            filter(lambda kv: kv[1] != Mapping.delete_tag, official.items())
        )
        return {"+": official}
    ...
    def mergeTags(self, osm, official, ref, keep_multiple):
                        ...
                        if self.parser.source.attribution:
                            for s in osm[o].split(";"):
                                if self.parser.source.match_attribution(s):
                                    fix["~"][o] = osm[o].replace(
                                        s, self.parser.source.as_tag_value()
                                    )
                                    break
                        else:
        ...
        keys = [
            s
            for s in (list(fix["+"].keys()) + list(fix["~"].keys()))
            if s != "name" and not s.startswith("source")
        ]
        if (
            "name" in osm
...
open data and OSM."""
    ...
    def analyser_osmosis_common(self):
        ...
        if self.conflate.osmRef != "NULL":
            ...
            joinClause.append(
                "ST_DWithin(official.geom, osm_item.shape, {0})".format(
                    self.conflate.conflationDistance
                )
            )
        if self.conflate.extraJoin:
            joinClause.append(
                "official.tags->'{tag}' = osm_item.tags->'{tag}'".format(
                    tag=self.conflate.extraJoin
                )
            )
        joinClause = " AND\n".join(joinClause) + "\n"
        ...
        if self.missing_official:
            ...
            def ret(res):
                ...
                return {
                    ...
                    "text": self.conflate.mapping.text(
        ...
        if self.conflate.osmRef != "NULL":
            ...
            if self.missing_osm:
                ...
                def osm_missing(res):
                    ...
                    return {
                        ...
                        "subclass": (
                            str(
                                stablehash64("{0}{1}{2}".format(res[0], res[1], res[5]))
                            )
                            if res["ref"] is None
                            else None
                        ),
            ...
            if self.parser.imported_srid():
                possible_merge_joinClause.append(
                    "ST_DWithin(missing_official.geom, missing_osm.shape, {0})".format(
                        self.conflate.conflationDistance
                    )
                )
                possible_merge_orderBy = (
            ...
            if self.conflate.extraJoin:
                possible_merge_joinClause.append(
                    "missing_official.tags->'{tag}' = missing_osm.tags->'{tag}'".format(
                        tag=self.conflate.extraJoin
                    )
                )
            ...
            self.run(
                sql30.format(
                    joinClause=possible_merge_joinClause, orderBy=possible_merge_orderBy
                )
            )
            self.run(sql31)
                ...
                def ret(res):
                    ...
                    return {
                        ...
                        "fix": self.mergeTags(
                            res[5],
                            res[3],
                            self.conflate.osmRef,
                            self.conflate.tag_keep_multiple_values,
                        ),
                    }
            ...
            self.dumpCSV(
                ...
                "",
                ["lon", "lat"],
                lambda r, cc: list((r["lon"], r["lat"])) + cc,
            )
            ...
            self.dumpCSV(
                sql41,
                ".byOSM",
                ["osmose", "osm_id", "osm_type", "lon", "lat"],
                lambda r, cc: list(
                    (r["osmose"], r["osm_id"], r["osm_type"], r["lon"], r["lat"])
                )
                + cc,
            )

            file = io.open(
                "{0}/{1}.metainfo.csv".format(self.config.dst_dir, self.name),
                "w",
                encoding="utf8",
            )
            file.write(
                "file,origin,osm_date,count_official,count_osm,count_missing_official_in_osm,count_missing_osm_in_official,count_invalid_osm_ref,count_possible_merge\n"
            )
            file.write(
                f'"{self.name}","{self.parser.source.fileUrl or self.url}",FIXME,{count_official},{count_osm},{count_missing_official},{count_missing_osm},{count_invalid_osm_ref},{count_possible_merge}\n'
            )
            file.close()
        ...
        if self.moved_official:
            self.run(
                sql50.format(official=table, joinClause=joinClause),
                lambda res: {
        ...
        if self.update_official:
            self.run(
                sql60.format(official=table, joinClause=joinClause),
                lambda res: {
    ...
    def dumpCSV(self, sql, ext, head, callback):
        ...
        self.giscurs.execute(
            "SELECT array_agg(key) FROM (SELECT key, count(*) FROM DUMP, LATERAL jsonb_object_keys(tags) key GROUP BY key ORDER BY count(*) DESC) AS t(key)"
        )
        column = self.giscurs.fetchone()[0]
        if column is not None:
            column = list(
                filter(
                    lambda a: a != self.conflate.osmRef
                    and a not in self.conflate.select.tags[0],
                    column,
                )
            )
            column = (
                [self.conflate.osmRef]
                + list(self.conflate.select.tags[0].keys())
                + column
            )
        buffer = io.StringIO()
        ...
        with bz2.BZ2File(
            "{0}/{1}-{2}{3}.csv.bz2".format(
                self.config.dst_dir, self.name, self.__class__.__name__, ext
            ),
            mode="w",
        ) as csv_bz2_file:
...
class Test(TestAnalyserOsmosis):
    ...
    def test_where_formatter(self):
        ...
        self.assertEqual(
            Select.where_attributes({"a": None}), """((NOT "a" IS NULL))"""
        )
        self.assertEqual(
            Select.where_attributes({"a": True}), """((NOT "a" IS NULL))"""
        )
        self.assertEqual(Select.where_attributes({"a": False}), """(("a" IS NULL))""")
        self.assertEqual(
            Select.where_attributes({"a": {"like": "a%"}}),
            """((NOT "a" IS NULL AND "a" LIKE 'a%'))""",
        )
        self.assertEqual(
            Select.where_attributes({"a": "1"}),
            """((NOT "a" IS NULL AND '1' = ANY(string_to_array("a"::text, ';'))))""",
        )
        self.assertEqual(
            Select.where_attributes({"a": ["1", "2"]}),
            """((NOT "a" IS NULL AND string_to_array("a"::text, ';') && ARRAY['1', '2']))""",
        )
        self.assertEqual(
            Select.where_attributes({"a": ["1", False]}),
            """(((string_to_array("a"::text, ';') && ARRAY['1'] OR "a" IS NULL)))""",
        )
        self.assertEqual(
            Select.where_attributes({"a": "1", "b": "2"}),
            """((NOT "a" IS NULL AND '1' = ANY(string_to_array("a"::text, ';')) AND NOT "b" IS NULL AND '2' = ANY(string_to_array("b"::text, ';'))))""",
        )

        self.assertEqual(
            Select.where_tags({"a": "1"}),
            """((NOT NOT tags?'a' AND '1' = ANY(string_to_array(tags->'a'::text, ';'))))""",
        )
        self.assertEqual(Select.where_tags({"a": None}), """((NOT NOT tags?'a'))""")

        self.assertEqual(
            Select.where_attributes([{"a": "1"}, {"b": "2"}]),
            """((NOT "a" IS NULL AND '1' = ANY(string_to_array("a"::text, ';'))) OR (NOT "b" IS NULL AND '2' = ANY(string_to_array("b"::text, ';'))))""",
        )
