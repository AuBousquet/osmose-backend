...
on the wiki. Add a node or add tags if already existing."""
        ...
        self.missing_official = self.def_class(
            ...
            id=51,
            level=3,
            tags=["merge", "parking", "fix:imagery", "fix:survey"],
            title=T_("{0} parking not integrated", "BNLS"),
            detail=doc_detail,
        )
        self.possible_merge = self.def_class(
            item=8131,
            id=53,
            level=3,
            tags=["merge", "parking", "fix:imagery", "fix:chair"],
        ...
        self.update_official = self.def_class(
            item=8132,
            id=54,
            level=3,
            tags=["merge", "parking", "fix:chair", "fix:survey"],
        ...
        self.init(
            ...
            Conflate(
                select=Select(types=["nodes", "ways"], tags={"amenity": "parking"}),
                ...
                mapping=Mapping(
                    ...
                    mapping1={
                        ...
                        "access": lambda res: (
                            "yes"
                            if res["type_usagers"] == "tous"
                            else (
                                "customers"
                                if res["type_usagers"] == "abonnés"
                                else None
                            )
                        ),
                        "fee": lambda res: "yes" if res["gratuit"] == "0" else None,
                        "capacity": lambda res: (
                            res["nb_places"] if res["nb_places"] != "0" else None
                        ),
                        "capacity:disabled": lambda res: (
                            res["nb_pmr"] if res["nb_pmr"] != "0" else None
                        ),
                        "capacity:charging": lambda res: (
                            res["nb_voitures_electriques"]
                            if res["nb_voitures_electriques"] != "0"
                            else None
                        ),
                        "maxheight": lambda res: (
                            int(res["hauteur_max"]) / 100
                            if res["hauteur_max"] != "N/A"
                            else None
                        ),
                        "parking": lambda res: (
                            "surface"
                            if res["type_ouvrage"] == "enclos_en_surface"
                            else None
                        ),
                    },
                    text=lambda tags, fields: {
                        "en": "Parking {0}".format(tags["name"])
                    },
                ),
            ),
        )
