...
from .Analyser_Merge import CSV, Conflate, Load_XY, Mapping, Select, SourceOpenDataSoft
...
class SubAnalyser_Merge_Pitch_FR(SubAnalyser_Merge_Dynamic):
    def __init__(self, config, error_file, logger, classs, topic, osmTags, defaultTags):
        ...
        self.def_class_missing_official(
            ...
            id=classs,
            level=3,
            tags=["merge", "leisure", "fix:imagery", "fix:survey"],
                    ...
                    text=lambda tags, fields: {
                        "en": ", ".join(
                            filter(
                                lambda i: i != "None",
                                [
                                    fields["Numéro de l'installation sportive"],
                                    fields["Type d'équipement sportif"],
                                    fields["Nom de l'installation sportive"],
                                    fields["Nom de l'équipement sportif"],
                                ],
                            )
                        )
                    },
    ...
    def validLatLon(self, row):
        if (
            abs(float(row["Longitude (WGS84)"])) <= 180
            and abs(float(row["Latitude (WGS84)"])) <= 90
        ):
            return row
        else:
            return []
