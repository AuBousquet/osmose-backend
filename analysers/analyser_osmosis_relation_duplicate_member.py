...
class Analyser_Osmosis_Relation_Duplicate_Member(Analyser_Osmosis):
    ...
    def __init__(self, config, logger=None):
        ...
        self.callback10 = lambda res: {
            "class": 3,
            "data": [self.relation, self.positionAsText, self.array_id],
            "text": {"en": ", ".join(res[3]).lower()},
        }
...
class Test(TestAnalyserOsmosis):
    ...
    def setup_class(cls):
        ...
        cls.analyser_conf = cls.load_osm(
            "tests/osmosis_relation_duplicate_member.osm",
            config.dir_tmp + "/tests/osmosis_relation_duplicate_member.test.xml",
            {},
        )

    def test_classes(self):
        with Analyser_Osmosis_Relation_Duplicate_Member(
            self.analyser_conf, self.logger
        ) as a:
            a.analyser()
