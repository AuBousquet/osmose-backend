...
class _Generic_Analyser_Merge_Public_Services_FR_(Analyser_Merge_Point):
    def __init__(
        ...
        self.init(
            ...
            "Service-Public.fr",
            CSV(
                Public_Services_Source(
                    Source(
                        attribution="Service-Public.fr",
                        fileUrl="https://www.data.gouv.fr/fr/datasets/r/73302880-e4df-4d4c-8676-1a61bb997f3d",
                    )
                )
            ),
            Load_XY(
                "longitude", "latitude", select={"categorie": public_service_category}
            ),
            Conflate(
                select=Select(
                    types=["nodes", "ways", "relations"], tags=osm_select_tags
                ),
                ...
                mapping=Mapping(
                    ...
                    mapping1=dict(
                        {
                            ...
                            "wheelchair:description": lambda feature: (
                                feature["wheelchair:description"]
                                if feature["wheelchair"] == "limited"
                                else None
                            ),
                    ...
                    mapping2=dict(
                        {
                            "source": lambda feature: "Service-Public.fr - "
                            + feature["millesime"]
                        }
                    ),
                    text=lambda tags, fields: {
                        "en": "{0}, {1}".format(
                            fields["official_name"], fields["address_txt"]
                        )
                    },
                ),
            ),
...
class Analyser_Merge_Public_Services_Mairie(
    _Generic_Analyser_Merge_Public_Services_FR_
):
    def __init__(self, config, logger=None):
        _Generic_Analyser_Merge_Public_Services_FR_.__init__(
            self,
            config,
            logger,
            1,
...
class Analyser_Merge_ServicePublic_FR_Pole_Emploi(
    _Generic_Analyser_Merge_Public_Services_FR_
):
    def __init__(self, config, logger=None):
        _Generic_Analyser_Merge_Public_Services_FR_.__init__(
...
class Analyser_Merge_ServicePublic_FR_Tribunal(
    _Generic_Analyser_Merge_Public_Services_FR_
):
    def __init__(self, config, logger=None):
        _Generic_Analyser_Merge_Public_Services_FR_.__init__(
            ...
            logger,
            6,
            ["ta", "ti", "tribunal_commerce", "tgi", "te", "cour_appel"],
            {"amenity": "courthouse"},
            {"amenity": "courthouse"},
        )


class Analyser_Merge_ServicePublic_FR_Prison(
    _Generic_Analyser_Merge_Public_Services_FR_
):
    def __init__(self, config, logger=None):
        _Generic_Analyser_Merge_Public_Services_FR_.__init__(
...
class Analyser_Merge_ServicePublic_FR_Prefecture(
    _Generic_Analyser_Merge_Public_Services_FR_
):
    def __init__(self, config, logger=None):
        _Generic_Analyser_Merge_Public_Services_FR_.__init__(
...
class Analyser_Merge_ServicePublic_FR_Impots(
    _Generic_Analyser_Merge_Public_Services_FR_
):
    def __init__(self, config, logger=None):
        _Generic_Analyser_Merge_Public_Services_FR_.__init__(
    ...
    def __init__(self, config, logger=None):
        _Generic_Analyser_Merge_Public_Services_FR_.__init__(
            ...
            "caf",
            {
                ...
                "operator": "Caisse d'Allocations Familiales",
            },
            {
                "office": "government",
                "government": "social_welfare",
                "operator": "Caisse d'Allocations Familiales",
                "operator:wikidata": "Q1395254",
            },
        )
    ...
    def __init__(self, config, logger=None):
        _Generic_Analyser_Merge_Public_Services_FR_.__init__(
            ...
            "cpam",
            {
                ...
                "operator": "Caisse Primaire d'Assurance Maladie",
            },
            {
                "office": "government",
                "government": "social_security",
                "operator": "Caisse Primaire d'Assurance Maladie",
                "operator:wikidata": "Q2110238",
            },
        )


class Analyser_Merge_ServicePublic_FR_URSSAF(
    _Generic_Analyser_Merge_Public_Services_FR_
):
    def __init__(self, config, logger=None):
        _Generic_Analyser_Merge_Public_Services_FR_.__init__(
...
class Analyser_Merge_ServicePublic_FR_CCI(_Generic_Analyser_Merge_Public_Services_FR_):
    def __init__(self, config, logger=None):
        _Generic_Analyser_Merge_Public_Services_FR_.__init__(
            ...
            logger,
            14,
            ["chambre_agriculture", "chambre_metier", "cci"],
    ...
    def __init__(self, config, logger=None):
        _Generic_Analyser_Merge_Public_Services_FR_.__init__(
            ...
            "msap",
            {
                ...
                "operator": "France Services",
            },
            {
                "office": "government",
                "government": "public_service",
                "operator": "France Services",
                "operator:wikidata": "Q24931343",
            },
        )
...
class Public_Services_Source(Source):
    ...
    def open(self):
        return open(
            downloader.update_cache(
                "parse://" + self.source.fileUrl, 15, fetch=self.fetch
            )
        )
    ...
    def fetch(self, url, tmp_file, date_string=None):
        ...
        with tarfile.open(fileobj=BytesIO(dataset_dl.raw.read())) as archive:
            for content in archive.getmembers():
                if content.name.endswith("data.gouv_local.json"):
                    ...
                    millesime = datetime.datetime.strptime(
                        date_and_blabla[0:10], "%Y-%m-%d"
                    ).strftime("%m/%Y")
        ...
        wheelchair_mapping = {"ACC": "yes", "DEM": "limited", "NAC": "no", "": None}
        ...
        for feature in data["service"]:
            ...
            if elem["categorie"] in ["mairie", "mairie_com"]:
                ...
                elem["name"], elem["branch"] = retreat_France_Services_name(
                    feature["nom"]
                )
            elif elem["categorie"] in [
                "ti",
                "tribunal_commerce",
                "tgi",
                "te",
            ...
            if feature.get("telephone"):
                elem["contact:phone"] = retreat_phone_number(
                    feature["code_insee_commune"], feature["telephone"][0]["valeur"]
                )
        ...
        with open(tmp_file, "w") as out_file:
            csv_writer = csv.DictWriter(
                out_file, delimiter=",", fieldnames=export[0].keys()
            )
...
def parse_opening_hours(txt):
    ...
    for plage in txt:
        ...
        opening_hours += "{}-{}".format(
            plage["valeur_heure_debut_1"][0:5], plage["valeur_heure_fin_1"][0:5]
        )
        if plage["valeur_heure_debut_2"]:
            opening_hours += ",{}-{}".format(
                plage["valeur_heure_debut_2"][0:5], plage["valeur_heure_fin_2"][0:5]
            )
        opening_hours += "; "
...
def retreat_CPAM_name(official_name):
    return official_name.replace(
        "Caisse primaire d'assurance maladie (CPAM)", "CPAM"
    ).split(" - ")[0]


def retreat_Caf_name(official_name):
    return official_name.replace("Caisse d'allocations familiales (Caf)", "Caf").split(
        " - "
    )[0]
...
def retreat_CIO_name(official_name):
    return (
        official_name.replace("Centre d’information et d’orientation (CIO)", "CIO")
        .replace("Centre d'information et d'orientation (CIO)", "CIO")
        .split(" - ")[0]
    )
