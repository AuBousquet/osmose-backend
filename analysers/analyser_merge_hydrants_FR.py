...
class _Analyser_Merge_Afigeo_Hydrants(Analyser_Merge_Point):
    def __init__(self, config, source_url, dataset_name, source, osmRef, logger=None):
        ...
        self.def_class_missing_official(
            ...
            id=11,
            level=3,
            tags=["merge", "emergency", "fix:imagery", "fix:picture", "fix:survey"],
        ...
        self.init(
            source_url,
            dataset_name,
            GeoJSON(source, extractor=lambda geojson: geojson),
            Load_XY("geom_x", "geom_y"),
            Conflate(
                select=Select(types=["nodes"], tags={"emergency": "fire_hydrant"}),
                osmRef=osmRef,
                ...
                mapping=Mapping(
                    ...
                    mapping1={
                        ...
                        "fire_hydrant:type": lambda res: (
                            "pillar" if res.get("type_pei") == "PI" else None
                        ),
                        ...
                        "start_date": lambda res: (
                            parse(res.get("date_ct")).date()
                            if res.get("date_ct")
                            else None
                        ),
                        "survey:date": lambda res: (
                            parse(res.get("date_ro")).date()
                            if res.get("date_ro")
                            else None
                        ),
