...
class Analyser_Merge_Police_FR_gn(Analyser_Merge_Point):
    def __init__(self, config, logger=None):
        ...
        self.def_class_missing_official(
            ...
            id=1,
            level=3,
            tags=["merge", "amenity", "fix:survey", "fix:picture"],
        ...
        self.init(
            ...
            Load_XY(
                "geocodage_x_GPS",
                "geocodage_y_GPS",
                where=lambda row: "Centre d'information et de recrutement"
                not in row["service"]
                and "motorisé" not in row["service"],
            ),
            Conflate(
                select=Select(types=["nodes", "ways"], tags={"amenity": "police"}),
                ...
                mapping=Mapping(
                    ...
                    mapping1={
                        ...
                        "seasonal": lambda fields: (
                            "yes" if "Poste provisoire" in fields["service"] else None
                        ),
                    },
                    mapping2={
                        "phone": "telephone",
        ...
        def parse_opening_hours(line):
            ...
            for a_day in OSM_DAYS_FR:
                ...
                if line[a_day + "_plage1_fin"]:
                    hours += "{}-{}".format(
                        line[a_day + "_plage1_debut"], line[a_day + "_plage1_fin"]
                    )
                if line[a_day + "_plage2_fin"]:
                    hours += ",{}-{}".format(
                        line[a_day + "_plage2_debut"], line[a_day + "_plage2_fin"]
                    )
                if line[a_day + "_plage3_fin"]:
                    hours += ",{}-{}".format(
                        line[a_day + "_plage3_debut"], line[a_day + "_plage3_fin"]
                    )
                hours_list.append(
                    {"hours": hours.replace("h", ":"), "day": OSM_DAYS[a_day]}
                )
                ...
                for j in range(7):
                    ...
                    if hours_list[j]["hours"] == hours_list[i]["hours"]:
                        if hours_list[j]["hours"]:
                            if i == j:
                                current_hours_txt = "{} {}; ".format(
                                    hours_list[i]["day"], hours_list[i]["hours"]
                                )
                            else:
                                current_hours_txt = "{}-{} {}; ".format(
                                    hours_list[i]["day"],
            ...
            if hours_list[7]["hours"]:
                hours_text += "{} {}".format(
                    hours_list[7]["day"], hours_list[7]["hours"]
                )
