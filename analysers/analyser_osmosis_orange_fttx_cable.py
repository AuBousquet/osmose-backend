...
FROM {1}.cable_fttx_rip left join {1}.site_support_rip as site_orig on (site_orig.code = cable_fttx_rip.code_site_)
...
FROM {1}.cable_fttx_rip left join {1}.site_support_rip as site_dest on (site_dest.code = cable_fttx_rip.code_sit_1)
WHERE NOT (site_dest.geom = st_endpoint(st_geometryn(cable_fttx_rip.geom, st_numgeometries(cable_fttx_rip.geom))));
"""

class Analyser_Orange_Cable_Fttx(Analyser_Osmosis):

    requires_tables_common = ['cable_fttx_rip', 'site_support_rip']
    ...
    def analyser_osmosis_common(self):
        self.run(sql00.format(self.config.db_schema), self.callback00)
        self.run(sql10.format(self.config.db_schema), self.callback10)
