...
class Analyser_Merge_Power_Plant_FR(Analyser_Merge_Point):
    def __init__(self, config, logger=None):
        ...
        self.def_class_missing_official(
            ...
            id=1,
            level=3,
            tags=["merge", "power", "fix:survey", "fix:imagery"],
        ...
        self.init(
            ...
            Conflate(
                select=Select(types=["ways", "relations"], tags={"power": "plant"}),
                ...
                mapping=Mapping(
                    ...
                    mapping1={
                        ...
                        "plant:source": lambda fields: self.filiere[fields["filiere"]][
                            fields["combustible"]
                        ],
                        "plant:output:electricity": lambda fields: (
                            None
                            if not fields["puisMaxRac"]
                            else str(float(fields["puisMaxRac"]) / 1000).rstrip(".0")
                            + " MW"
                        ),
                    },
                    mapping2={
                        "start_date": lambda fields: (
                            None
                            if not fields["dateMiseEnService"]
                            else (
                                fields["dateMiseEnService"][6:10]
                                if fields["dateMiseEnService"].startswith("01/01/")
                                or fields["dateMiseEnService"].startswith("31/12/")
                                else "-".join(
                                    fields["dateMiseEnService"].split("/")[::-1]
                                )
                            )
                        )
                    },
