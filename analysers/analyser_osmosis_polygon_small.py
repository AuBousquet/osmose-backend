...
Usually a park contains grass, other vegetation, walking paths. A park often has a name.""",
                "`, `".join(
                    ("landuse=grass", "natural=shrubbery", "landuse=flowerbed")
                ),
            ),
            trap=trapmsg,
        )
        ...
        self.checks = [
            # Objects to be checked. Requires: key, val(ue) of tag, min(imum)area (in m2) of the object, and class
            {
                ...
                "minarea": 20,
                "class": 1,
            },  # 20m2 is roughly 1 big tree of 5m diameter
            {
                "key": "landuse",
                "val": "forest",
                "minarea": 20,
        ...
        for item in self.checks:
            self.run(
                ...
                lambda res: {
                    ...
                    "text": T_(
                        "{0} with an area of {1} m2",
                        "`{0}={1}`".format(item["key"], item["val"]),
                        round(res[2]),
                    ),
                },
            )
