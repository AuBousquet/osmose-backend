...
class Analyser_Merge_Power_Substation_minor_FR(Analyser_Merge_Point):
    def __init__(self, config, logger=None):
        ...
        self.def_class_missing_official(
            ...
            id=11,
            level=3,
            tags=["merge", "power", "fix:survey", "fix:picture"],
            title=T_("Minor distribution power substation missing in OSM"),
            detail=T_(
                "A power substation that directly feeds consumers, known from operator, does not exist in OSM."
            ),
        )
        self.def_class_possible_merge(
            ...
            detail=T_(
                "This existing power substation can be integrated with official values."
            ),
        )
            ...
            Conflate(
                select=Select(
                    ...
                    tags=[
                        {
                            "power": "substation",
                            "substation": ["distribution", "minor_distribution"],
                        },
                ...
                mapping=Mapping(
                    ...
                    static2={
                        ...
                        "voltage": "20000",
                    },  # Currently lawful default value as there is no opendata to define it. Mappers may be knowledgeable
                    mapping2={
                        "operator": "GRD",
                        "name": lambda fields: (
                            fields["Nom poste"]
                            if fields["Nom poste"] != ""
                            and fields["GRD"] != "Strasbourg Électricité Réseaux"
                            else None
                        ),
                        "ref": lambda fields: (
                            fields["Nom poste"]
