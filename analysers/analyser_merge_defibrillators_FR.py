...
class Analyser_merge_defibrillators_FR(Analyser_Merge_Point):
    def normalizeEtage(self, etg):
        if etg is None:
            ...
            etg = unidecode.unidecode(
                etg.lower().replace("-", "").replace(".", "").replace(" ", "")
            )
            if etg == "0":
                return None
    ...
    def __init__(self, config, logger=None):
        ...
        self.def_class_missing_official(
            ...
            trap=T_(
                "Location of defibrillators from this dataset can be very approximative. Check carefully the position before adding to OSM."
            ),
        )
        self.def_class_possible_merge(
            ...
            id=13,
            level=3,
            tags=["merge", "emergency", "fix:chair", "fix:picture"],
        ...
        self.init(
            ...
            Conflate(
                select=Select(types=["nodes"], tags={"emergency": "defibrillator"}),
                ...
                mapping=Mapping(
                    ...
                    mapping1={
                        ...
                        "indoor": lambda res: (
                            "yes"
                            if res["c_acc"] == "Intérieur"
                            else "no" if res["c_acc"] == "Extérieur" else None
                        ),
                        "access": lambda res: (
                            "yes"
                            if res["c_acc_lib"] == "t"
                            else "permissive" if res["c_acc_lib"] == "f" else None
                        ),
                        "level": lambda res: self.normalizeEtage(res["c_acc_etg"]),
                        "opening_hours": lambda res: self.normalizeHours(
                            res["c_disp_j"], res["c_disp_h"]
                        ),
                    },
                    mapping2={
                        "name": lambda res: (
                            reaccentue.reaccentue(res["c_nom"])
                            if res["c_nom"] and res["c_acc_complt"]
                            else None
                        ),
                        "operator": lambda res: (
                            reaccentue.reaccentue(res["c_expt_rais"])
                            if "c_expt_rais" in res
                            else None
                        ),
                        "source": lambda res: (
                            "Direction Générale de la Santé - "
                            + res["c__edit_datemaj|timePosition"].split("T")[0]
                        ),
                        "defibrillator:location": lambda res: (
                            res["c_acc_complt"]
                            if "c_acc_complt" in res
                            else (
                                reaccentue.reaccentue(res["c_nom"])
                                if res["c_nom"]
                                else None
                        ...
                        "security_desk": lambda res: (
                            "yes"
                            if res["c_acc_pcsec"] == "t"
                            else "no" if res["c_acc_pcsec"] == "f" else None
                        ),
                        "reception_desk": lambda res: (
                            "yes"
                            if res["c_acc_acc"] == "t"
                            else "no" if res["c_acc_acc"] == "f" else None
                        ),
                        "operator:ref:FR:SIREN": lambda res: (
                            res["c_expt_siren"] if "c_expt_siren" in res else None
                        ),
                    },
                            ...
                            filter(
                                ...
                                [
                                    ...
                                    (
                                        ...
                                        + " "
                                        + fields["c_disp_h"][1:-1]
                                        if fields["c_disp_j"] and fields["c_disp_h"]
                                        else None
                                    ),
                                    " ".join(
                                        filter(
