...
from .Analyser_Merge import GDAL, Load, Mapping, Select, SourceOpenDataSoft
...
class Analyser_Merge_Power_Line_FR(Analyser_Merge_Network):
    def __init__(self, config, logger=None):
        ...
        self.init(
            ...
            Load("geom", select={"tension": lambda t: f"{t} != 'HORS TENSION'"}),
            ConflateNetwork(
                select=Select(
                    types=["ways"], tags=[{"power": "line"}, {"disused:power": "line"}]
                ),
                conflationDistance=30,
                mapping=Mapping(
                    static1={"power": "line", "operator": "RTE"},
                    static2={"operator:wikidata": "Q2178795", "source": self.source},
                    mapping1={
                        "voltage": lambda fields: (
                            str(
                                (int(float(fields["tension"].replace("kV", "")) * 1000))
                            )
                            if fields["tension"]
                            not in ("HORS TENSION", "<45kV", "COURANT CONTINU")
                            else None
                        ),
                    },
                    text=lambda tags, fields: {
                        "en": ", ".join(
                            filter(
                                lambda res: res and res != "None", [fields["tension"]]
                            )
                        )
                    },
                ),
