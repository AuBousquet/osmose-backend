...
`railway=rail` crudely drawn."""
        ...
        self.callback10 = lambda res: {
            ...
            "text": T_("railway={0} with suspicious angle {1}°", res[3], int(res[2])),
        }
    ...
    def analyser_osmosis_full(self):
        self.run(
            sql10.format(
                table_prefix="",
                tag="railway",
                tag_values="', '".join(self.tags),
                srid=self.config.options.get("proj"),
            ),
            self.callback10,
        )
