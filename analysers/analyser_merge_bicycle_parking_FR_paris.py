...
tag_mapping = {
    "capacity": (
        lambda res: (
            None
            if res["Nombre places calculées"] in (None, "0", "-1")
            else res["Nombre places calculées"]
        )
    ),
    "bicycle_parking": (lambda res: STANDS_TYPES.get(res["Type mobilier"], None)),
}
    ...
    def __init__(self, config, logger=None):
        ...
        self.init(
            ...
            Conflate(
                select=Select(
                    types=["nodes", "ways"],
                    tags={"amenity": ["bicycle_parking", "motorcycle_parking"]},
                ),
                conflationDistance=20,
                mapping=Mapping(
