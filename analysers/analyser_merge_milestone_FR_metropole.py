...
Sometimes, a small white line perpendicular to the road on the emergency stop strip or the left flared strip can be seen on satellite images or Mapillary's photos."""
        ...
        self.def_class_missing_official(
            ...
            id=41,
            level=3,
            tags=["merge", "highway", "fix:picture", "fix:survey"],
        ...
        self.init(
            ...
            Conflate(
                select=Select(types=["nodes"], tags=[{"highway": "milestone"}]),
                osmRef="nat_ref",
                conflationDistance=150,
                mapping=Mapping(
                    static1={"highway": "milestone"},
                    static2={"source:nat_ref": self.source},
                    mapping1={"distance": "pr", "nat_ref": "nom_plo"},
                ),
            ),
        )
    ...
    def is_milestone(self, row):
        if [
            ele
            for ele in ("P", "N1", "N2", "A9", "N9", "A8", "N8")
            if ele in row["route"]
        ]:
