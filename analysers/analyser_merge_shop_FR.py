        ...
        self.init(
            ...
            CSV(
                SourceHttpLastModified(
                    attribution="INSEE",
                    gzip=True,
                    fileUrl="http://data.cquest.org/geo_sirene/v2019/last/dep/geo_siret_{0}.csv.gz".format(
                        dep_code
                    ),
                )
            ),
            Load_XY(
                ...
                where=lambda res: (
                    float(res["geo_score"]) > 0.9
                    and (
                        not res["complementAdresseEtablissement"]
                        or (
                            " APP" not in res["complementAdresseEtablissement"]
                            and " CHEZ " not in res["complementAdresseEtablissement"]
                        )
                    )
                    and (
                        not trancheEffectifs
                        or (
                            res["trancheEffectifsEtablissement"]
                            and res["trancheEffectifsEtablissement"] != "NN"
                            and int(res["trancheEffectifsEtablissement"])
                            > trancheEffectifs
                        )
                    )
                ),
            ...
            Conflate(
                select=Select(types=["nodes", "ways"], tags=selectTags),
                ...
                mapping=Mapping(
                    ...
                    mapping1={
                        ...
                        #    None,
                        "name": lambda fields: (
                            reaccentue.reaccentue(fields["enseigne1Etablissement"])
                            if fields["enseigne1Etablissement"]
                            else (
                                reaccentue.reaccentue(
                                    fields["denominationUsuelleEtablissement"]
                                )
                    ...
                    text=lambda tags, fields: {
                        "en": ", ".join(
                            filter(
                                lambda f: f and f != "None",
                                [
                                    fields["enseigne1Etablissement"]
                                    or fields["denominationUsuelleEtablissement"]
                                ]
                                + list(
                                    map(
                                        lambda k: fields[k],
