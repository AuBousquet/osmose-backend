...
class Name_Saint_FR(Plugin):
    ...
    def init(self, logger):
        ...
        self.errors[3033] = self.def_class(
            item=3033,
            level=3,
            tags=["name", "fix:chair"],
            title={"en": "Saint"},
...
thing ("Saint-*" in French)."""
            ...
            fix=T_("""Add a hyphen."""),
            trap=T_("""This rule does not apply in particular to the Belgium."""),
            example=T_(
        ...
        if tags.get("name") not in (None, "Saint Algue"):
            ...
            if r:
                return {
                    "class": 3033,
                    "subclass": 1,
                    "text": T_('Missing hyphen after "Saint(e)"'),
                    "fix": {
                        "name": tags["name"].replace(
                            r.group(1), "{0}-".format(r.group(2))
                        )
                    },
                }
