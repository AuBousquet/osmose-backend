...
class TagRemove_Fixme(Plugin):
    ...
    def init(self, logger):
        ...
        self.errors[40610] = self.def_class(
            item=4061,
            level=3,
            tags=["fixme", "fix:chair"],
            title=T_("Object needs review"),
        )
        self.errors[40611] = self.def_class(
            item=4061,
            level=2,
            tags=["fixme", "fix:chair", "highway"],
                ...
                err.append(
                    {
                        ...
                        "subclass": stablehash64(
                            t + self.currentYear
                        ),  # Reset false positives every year
                        "text": {"en": tags[t]},
                    }
                )
        return err
    ...
    def way(self, data, tags, nds):
        ...
        if tags.get("highway") == "road":
            ret.append(
                {
                    "class": 40611,
                    "subclass": stablehash64("highway=road" + self.currentYear),
                }
            )
