...
class TagFix_Postcode(Plugin):
    ...
    def list_postcode(self):
        ...
        data = filter(
            ...
            map(
                lambda x: list(map(lambda y: y.strip(), x.split("|")))[5:8],
                data.split("|-")[1:-1],
            ),
        )
        postcode = {}
...
[Wikipedia](https://en.wikipedia.org/wiki/List_of_postal_codes)"""
        ...
        if self.Country in postcode:
            if (
                "street" in postcode[self.Country]
                and postcode[self.Country]["street"] is not None
            ):
    ...
    def node(self, data, tags):
        err = []
        if (
            self.CountryPostcodeArea
            and "postal_code" in tags
            and not self.CountryPostcodeArea.match(
                tags["postal_code"].replace(" ", "").replace("-", "").replace(".", "")
            )
        ):
            err.append(
                {
                    ...
                    "subclass": 1,
                    "text": T_(
                        "Invalid area postcode {postcode} for country code {country}",
        ...
        if (
            self.CountryPostcodeStreet
            and "addr:postcode" in tags
            and not self.CountryPostcodeStreet.match(
                tags["addr:postcode"].replace(" ", "").replace("-", "").replace(".", "")
            )
        ):
            err.append(
                {
                    ...
                    "subclass": 2,
                    "text": T_(
                        "Invalid street level postcode {postcode} for country code {country}",
