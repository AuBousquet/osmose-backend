...
class TagFix_Access(Plugin):
    def init(self, logger):
        ...
        self.accessValuesSpecial = {
            "bicycle": ["optional_sidepath", "dismount"],
        ...
        self.errors[30404] = self.def_class(
            item=3040,
            level=3,
            tags=["highway", "fix:chair"],
            title=T_("Uncommon access value"),
            detail=T_(
                """The value of the access tag is not one of the common access values: `{0}`.""",
                ", ".join(self.accessValuesGeneral),
            ),
            resource="https://wiki.openstreetmap.org/wiki/Key:access",
            trap=T_(
                """If there is no other tag (or combination of tags) that properly describes the access permissions, custom tags may be used."""
            ),
        )
        self.errors[30405] = self.def_class(
            item=3040,
            level=3,
            tags=["highway", "fix:chair"],
            title=T_("Transport mode in access value"),
            detail=T_(
                """The value of the access tag is a transport mode (such as `access=foot`). Consider replacing it with a more specific tag listing the transport mode first, for example `access=no` + `foot=yes`."""
            ),
            trap=T_(
                """Ensure that the access remains the same and does not conflict with other tags. This is especially likely if access tags are combined with directional and/or conditional access tags, or when transport modes are mixed with regular access values."""
            ),
            resource="https://wiki.openstreetmap.org/wiki/Key:access",
        )
    ...
    def checkAccessKeys(self, tags, suffixes):
        ...
        for accesskey in self.accessKeys:
            for suffix in suffixes:
                if accesskey + suffix in tags:
                    accessTags[accesskey + suffix] = {
                        "transportMode": accesskey,
                        "value": tags[accesskey + suffix],
                        "suffix": suffix,
                    }
        ...
        for tag in accessTags:
            ...
            for accessVal in set(values):
                ...
                if (
                    transportMode in self.accessValuesSpecial
                    and accessValue in self.accessValuesSpecial[transportMode]
                ):
                    continue
                if accessValue not in self.accessValuesGeneral:
                    if (
                        accessValue in self.accessKeys or accessValue == "emergency"
                    ) and accessValue != transportMode:
                        propose = (
                            tag
                            + " = ## + "
                            + accessValue
                            + accessTags[tag]["suffix"]
                            + " = yes"
                        )
                        if len(values) > 1 or isConditional:
                            propose = propose.replace(
                                "###", "..."
                            )  # i.e. access=bus;destination should become access=destination + bus=yes instead of access=no + bus=yes
                        else:
                            propose = propose.replace(
                                "###", "no"
                            )  # assume 'no' holds for all other transport modes
                        if isConditional:
                            propose = (
                                propose + " @ (...)"
                            )  # conditional may need to change
                        err.append(
                            {
                                "class": 30405,
                                "subclass": 0 + stablehash64(tag + "|" + accessVal),
                                "text": T_(
                                    'Access value "{0}" for key "{1}" is a transport mode. Consider using "{2}" instead',
                                    accessValue,
                                    tag,
                                    propose,
                                ),
                        ...
                        err.append(
                            {
                                ...
                                "subclass": 0 + stablehash64(tag + "|" + accessVal),
                                "text": T_(
                                    'Unknown access value "{0}" for key "{1}"',
                                    accessValue,
                                    tag,
                                ),
                            }
...
class Test(TestPluginCommon):
    def test(self):
        ...
        for t in [
            {"amenity": "parking", "vehicle": "no"},
            {"amenity": "parking", "vehicle:conditional": "no @ wet"},
            {"access": "agricultural", "agricultural": "designated"},
            {"agricultural": "agricultural"},
            {
                "highway": "residential",
                "hgv:conditional": "no @ (Mo-Fr 00:00-12:00;Sa 0:00-19:00); yes @ (Mo-Fr 12:00-24:00;Sa 19:00-24:00)",
            },
        ...
        for t in [
            {"highway": "residential", "vehicle:forward": "customers"},
            {
                "highway": "residential",
                "vehicle:backward:conditional": "customers @ (wet); destination @ (snow)",
            },
        ...
        for t in [
            {"amenity": "parking", "vehicle": "nope"},
            {"amenity": "parking", "vehicle:conditional": "nope @ wet"},
            {
                "highway": "residential",
                "canoe:conditional": "no @ (Mo-Fr 06:00-11:00;Sa 03:30-19:00); nope @ (snow)",
            },
            {
                "highway": "residential",
                "tank:conditional": "dismount @ (Mo-Fr 06:00-11:00;Sa 03:30-19:00); no @ (snow)",
            },
            {"bicycle": "leashed"},
        ]:
        ...
        for t in [
            {"highway": "residential", "vehicle:forward": "nope"},
        ...
        for t in [
            {"highway": "residential", "access": "foot"},
