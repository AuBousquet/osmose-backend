...
class indoor(PluginMapCSS):
    ...
    def init(self, logger):
        ...
        self.errors[50] = self.def_class(
            item=1300,
            level=3,
            tags=mapcss.list_("indoor", "geom") + mapcss.list_("fix:survey"),
            title=mapcss.tr("This indoor feature should be a closed and valid polygon"),
        )
        self.errors[51] = self.def_class(
            item=1300,
            level=3,
            tags=mapcss.list_("indoor", "geom") + mapcss.list_("fix:survey"),
            title=mapcss.tr("This indoor feature should have a level"),
        )
        self.errors[52] = self.def_class(
            item=1300,
            level=3,
            tags=mapcss.list_("indoor", "geom") + mapcss.list_("fix:survey", "shop"),
            title=mapcss.tr("This indoor shop should probably be inside a room"),
        )
        self.errors[53] = self.def_class(
            item=1300,
            level=2,
            tags=mapcss.list_("indoor", "geom"),
            title=mapcss.tr("This indoor room should have a door"),
        )
        self.errors[21201] = self.def_class(
            item=2120,
            level=3,
            tags=mapcss.list_("indoor", "geom"),
            title=mapcss.tr(
        ...
        self.errors[21202] = self.def_class(
            item=2120,
            level=3,
            tags=mapcss.list_("indoor", "geom"),
            title=mapcss.tr(
    ...
    def node(self, data, tags):
        ...
        if ("indoor" in keys) or ("room" in keys):
            ...
            if not match:
                ...
                try:
                    match = (
                        mapcss.regexp_test(
                            mapcss._value_capture(capture_tags, 0, self.re_2a047336),
                            mapcss._tag_capture(capture_tags, 0, tags, "indoor"),
                        )
                    ) and (mapcss.inside(self.father.config.options, "DE,CH,FR"))
                except mapcss.RuleAbort:
                    pass
            if not match:
                capture_tags = {}
                try:
                    match = (mapcss._tag_capture(capture_tags, 0, tags, "room")) and (
                        mapcss.inside(self.father.config.options, "DE,CH,FR")
                    )
                except mapcss.RuleAbort:
                    pass
            if match:
                ...
                err.append(
                    {
                        ...
                        "subclass": 0,
                        "text": mapcss.tr(
                            "This indoor feature should be a closed and valid polygon"
                        ),
                    }
                )
        ...
        if "indoor" in keys:
            ...
            if not match:
                ...
                try:
                    match = (
                        (mapcss._tag_capture(capture_tags, 0, tags, "indoor"))
                        and (not mapcss._tag_capture(capture_tags, 1, tags, "level"))
                        and (
                            not mapcss._tag_capture(capture_tags, 2, tags, "repeat_on")
                        )
                        and (
                            mapcss._tag_capture(capture_tags, 3, tags, "indoor")
        ...
        if "room" in keys:
            ...
            if not match:
                ...
                try:
                    match = (
                        (mapcss._tag_capture(capture_tags, 0, tags, "room"))
                        and (not mapcss._tag_capture(capture_tags, 1, tags, "indoor"))
                        and (
                            not mapcss._tag_capture(
                                capture_tags, 2, tags, "buildingpart"
                            )
                        )
                    )
                except mapcss.RuleAbort:
    ...
    def way(self, data, tags, nds):
        ...
        if ("indoor" in keys) or ("room" in keys):
                        ...
                        (
                            mapcss.regexp_test(
                                mapcss._value_capture(
                                    capture_tags, 1, self.re_2a047336
                                ),
            ...
            if not match:
                ...
                try:
                    match = (
                        (mapcss._tag_capture(capture_tags, 1, tags, "room"))
                        and (mapcss.inside(self.father.config.options, "DE,CH,FR"))
                        and (nds[0] != nds[-1])
                    )
                except mapcss.RuleAbort:
                ...
                err.append(
                    {
                        "class": 53,
                        "subclass": 0,
                        "text": mapcss.tr("This indoor room should have a door"),
                    }
                )
            ...
            if not match:
                ...
                try:
                    match = (
                        ...
                        and (not mapcss._tag_capture(capture_tags, 2, tags, "level"))
                        and (mapcss.inside(self.father.config.options, "DE,CH,FR"))
                        and (nds[0] == nds[-1])
                    )
                except mapcss.RuleAbort:
                ...
                err.append(
                    {
                        "class": 51,
                        "subclass": 0,
                        "text": mapcss.tr("This indoor feature should have a level"),
                    }
                )
            ...
            if not match:
                ...
                try:
                    match = (
                        (
                            mapcss._tag_capture(capture_tags, 1, tags, "indoor")
                            == mapcss._value_capture(capture_tags, 1, "area")
                        )
            ...
            if not match:
                ...
                try:
                    match = (
                        (
                            mapcss._tag_capture(capture_tags, 1, tags, "indoor")
                            == mapcss._value_capture(capture_tags, 1, "corridor")
                        )
                        and (mapcss._tag_capture(capture_tags, 2, tags, "shop"))
                        and (mapcss.inside(self.father.config.options, "DE,CH,FR"))
                        and (nds[0] == nds[-1])
                ...
                err.append(
                    {
                        ...
                        "subclass": 0,
                        "text": mapcss.tr(
                            "This indoor shop should probably be inside a room"
                        ),
                        "allow_fix_override": True,
                        "fix": {"+": dict([["indoor", "room"]])},
                    }
...
class Test(TestPluginMapcss):
    def test(self):
        ...
        with with_options(n, {"country": "FR"}):
            self.check_err(
                n.node(data, {"indoor": "room"}), expected={"class": 50, "subclass": 0}
            )
        with with_options(n, {"country": "DE"}):
            self.check_err(
                n.node(data, {"room": "shop"}), expected={"class": 50, "subclass": 0}
            )
        self.check_err(
            n.node(data, {"indoor": "room"}), expected={"class": 21201, "subclass": 1}
        )
        self.check_err(
            n.node(data, {"room": "office"}), expected={"class": 21202, "subclass": 2}
        )
        with with_options(n, {"country": "DE"}):
            self.check_not_err(
                n.way(data, {"indoor": "room", "level": "-0.5"}, [0]),
                expected={"class": 51, "subclass": 0},
            )
        with with_options(n, {"country": "FR"}):
            self.check_err(
                n.way(data, {"indoor": "room", "room": "shop"}, [0]),
                expected={"class": 51, "subclass": 0},
            )
        with with_options(n, {"country": "DE"}):
            self.check_err(
                n.way(data, {"indoor": "area", "level": "3", "shop": "florist"}, [0]),
                expected={"class": 52, "subclass": 0},
            )
        with with_options(n, {"country": "FR"}):
            self.check_err(
                n.way(data, {"indoor": "corridor", "shop": "tickets"}, [0]),
                expected={"class": 52, "subclass": 0},
            )
        with with_options(n, {"country": "DE"}):
            self.check_not_err(
                n.way(data, {"indoor": "room", "room": "shop", "shop": "florist"}, [0]),
                expected={"class": 52, "subclass": 0},
            )
