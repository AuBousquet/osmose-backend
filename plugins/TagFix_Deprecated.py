...
class TagFix_Deprecated(Plugin):
    def deprecated_list(self):
        ...
        data = re.sub(
            r"\[\[(.+?)\]\]",
            lambda x: "[{}]({}/{})".format(
                x.group(1), wikiRoot, x.group(1).replace(" ", "_")
            ),
            data,
        )
...
features](https://wiki.openstreetmap.org/wiki/Deprecated_features)"""
        ...
        self.errors[4010] = self.def_class(
            item=4010,
            level=2,
            tags=["deprecated", "tag", "fix:chair"],
            title=T_("Deprecated tag"),
            detail=detail,
        )
        self.errors[40102] = self.def_class(
            item=4010,
            level=2,
            tags=["deprecated", "value", "fix:chair"],
...
class Test(TestPluginCommon):
    def test(self):
        ...
        for d in [{"onway": "yes"}, {"waterway": "stream"}, {"highway": "primary"}]:
