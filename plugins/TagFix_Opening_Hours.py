...
try:
    from PyKOpeningHours.PyKOpeningHours import Error, OpeningHours
...
class TagFix_Opening_Hours(Plugin):
    ...
    def init(self, logger):
        ...
        self.errors[32501] = self.def_class(
            item=3250,
            level=3,
    ...
    def sanitize_openinghours(self, openinghours_value):
        ...
        if (
            parser.error() == Error.SyntaxError
            or parser.error() == Error.IncompatibleMode
        ):
        ...
        simplify = (
            lambda s: s.replace(" ", "").replace("24:00", "00:00").replace("0", "")
        )
                    ...
                    err.append(
                        {
                            ...
                            "text": T_(
                                "The `{0}` value is invalid and could not be parsed",
                                tag,
                            ),
                        }
                    )
...
class Test(TestPluginCommon):
    def test(self):
        ...
        for k in ["opening_hours", "opening_hours:kitchen"]:
            ...
            self.check_err(
                a.node(None, {k: "mo-fr 10h - 19h00"}),
                expected={"class": 32501, "fix": {k: "Mo-Fr 10:00-19:00"}},
            )
            self.check_err(
                a.node(None, {k: "Monday to Friday 8:00AM to 4:30PM"}),
                expected={"class": 32501, "fix": {k: "Mo-Fr 08:00-16:30"}},
            )

            # These return a parse error
            self.check_err(
                a.node(None, {k: 'mo-fr 10h - 19h00"'}), expected={"class": 32501}
            )
            self.check_err(
                a.node(None, {k: "09:00-21:00 TEL/072(360)3200"}),
                expected={"class": 32501},
            )
            ...
            assert not a.node(
                None,
                {
                    k: "Mo-Tu,Th-Fr 09:30-12:00; We 15:00-17:00; Dec 24,31 off; Sa,Su off; PH off"
                },
            )
            assert not a.node(
                None,
                {
                    k: "Mo off, Tu-Th 09:00-18:00; Fr 09:00-19:00; Sa 08:00-18:00; Su off"
                },
            )
        ...
        assert not a.node(
            None,
            {"opening_hours": "Tu-Fr 11:00-19:00; Sa 10:00-22:15; 14:00-15:00 closed"},
        )
