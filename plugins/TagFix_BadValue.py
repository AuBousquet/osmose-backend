...
class TagFix_BadValue(Plugin):
    ...
    def init(self, logger):
        ...
        self.errors[3040] = self.def_class(
            item=3040,
            level=1,
 ...
 (lowercase alphanumeric characters with no spaces). It's most likely not an expected tagging."""
            ...
            fix=T_(
                "Check the value of the tag in question and update the tagging to reflect what this feature is."
            ),
            trap=T_(
...
However, this should probably still conform to the typical format used for values of the given tag."""
        ...
        self.errors[40613] = self.def_class(
            item=4061,
            level=3,
            tags=["highway", "fix:chair"],
            title=T_("Unspecific value"),
            detail=T_(
                """The value of the tag is very unspecific. Replace it by a meaningful value."""
            ),
        )
        ...
        self.exceptions_open = {
            "type": ("associatedStreet", "turnlanes:lengths", "turnlanes:turns", "TMC"),
    ...
    def check(self, data, tags, check_list_open):
        ...
        for k in keys:
            if not self.Values_open.match(
                tags[k]
            ) and (  # value has a non-standard character
                k not in self.exceptions_open
                or any(  # no exceptions exist for the key
                    map(
                        lambda val: val not in self.exceptions_open[k]
                        and not self.Values_open.match(val),
                        tags[k].split(";"),
                    )
                )  # Check each value in a multiple-value key for not being whitelisted (or normal)
            ):
                err.append(
                    {
                        "class": 3040,
