...
class TagFix_Brand(Plugin):
    ...
    def init(self, logger):
        ...
        self.errors[31301] = self.def_class(
            item=3130,
            level=3,
            tags=["brand"],
            title=T_(
                "This name is very common, is this store part of the brand network?"
            ),
            detail=T_(
...
If not, see if you can improve the [name-suggestion-index project](https://github.com/osmlab/name-suggestion-index/blob/master/CONTRIBUTING.md) which is used to link frequent names to brands and their tags."""
        ...
        self.errors[31302] = self.def_class(
            item=3130,
            level=3,
            tags=["operator"],
...
If not, see if you can improve the [name-suggestion-index project](https://github.com/osmlab/name-suggestion-index/blob/master/CONTRIBUTING.md) which is used to register common operators."""
        ...
        if not self.father.config.options.get("country"):
            return False
        self.country_code = (
            self.father.config.options.get("country").split("-")[0].lower()
        )

        nsi = download_nsi()
        self.brands_from_nsi = self._parse_category_from_nsi(nsi, "brands/", "brand")
        self.operators_from_nsi = self._parse_category_from_nsi(
            nsi, "operators/", "operator"
        )
            ...
            if tag.startswith(nsiprefix) and "items" in details:
                ...
                for preset in details["items"]:
                    if "locationSet" in preset:
                        if (
                            "include" in preset["locationSet"]
                            and self.country_code
                            not in preset["locationSet"]["include"]
                            and "001" not in preset["locationSet"]["include"]
                        ):
                            continue
                        if (
                            "exclude" in preset["locationSet"]
                            and self.country_code in preset["locationSet"]["exclude"]
                        ):
                            continue
                    if "matchTags" in preset:
                        for additional_tag in preset["matchTags"]:
                            nsi_key = "{}|{}".format(
                                additional_tag, preset["tags"][key]
                            )
                    ...
                    if "name" in preset["tags"]:
                        additional_presets[
                            "{}|{}".format(nsi_name, preset["tags"]["name"]).lower()
                        ] = preset
                    additional_presets[
                        "{}|{}".format(nsi_name, preset["displayName"]).lower()
                    ] = preset
        return additional_presets

    def node(self, data, tags):
        if "name" in tags and ("brand" not in tags or "brand:wikidata" not in tags):
            for main_key in ["shop", "amenity"]:
                if main_key in tags:
                    if "brand" in tags:
                        nsi_key = "{}/{}|{}".format(
                            main_key, tags[main_key], tags["brand"]
                        ).lower()
                    else:
                        nsi_key = "{}/{}|{}".format(
                            main_key, tags[main_key], tags["name"]
                        ).lower()
                    if nsi_key in self.brands_from_nsi:
                        ...
                        if tags_to_add and (
                            "brand:wikidata" not in brands_tags
                            or "not:brand:wikidata" not in tags
                            or brands_tags["brand:wikidata"]
                            != tags["not:brand:wikidata"]
                        ):
                            return {
                                "class": 31301,
                                "subclass": 0,
        ...
        if "operator" in tags and "operator:wikidata" not in tags:
            for main_key in ["shop", "amenity", "emergency"]:
                if main_key in tags:
                    nsi_key = "{}/{}|{}".format(
                        main_key, tags[main_key], tags["operator"]
                    ).lower()
                    if nsi_key in self.operators_from_nsi:
                        ...
                        if tags_to_add and (
                            "operator:wikidata" not in operators_tags
                            or "not:operator:wikidata" not in tags
                            or operators_tags["operator:wikidata"]
                            != tags["not:operator:wikidata"]
                        ):
                            return {
                                "class": 31302,
                                "subclass": 0,
    ...
    def test_FR(self):
        ...
        assert not a.node(
            None,
            {
                ...
                "shop": "clothes",
                "name": "Kiabi",
                "not:brand:wikidata": "Q3196299",
            },
        )
        assert not a.node(
            None, {"shop": "clothes", "name": "Kiabi", "not:brand:wikidata": "Q3196299"}
        )
        assert not a.node(
            None,
            {
                ...
                "name": "Kiabi",
                "not:brand:wikidata": "Q3196299",
                "brand:wikidata": "Q1234567",
            },
        )
        assert not a.node(
            None, {"name": "National Bank", "amenity": "bank", "atm": "yes"}
        )
        ...
        assert a.node(
            None,
            {
                "name": "Beautify fire station",
                "amenity": "fire_station",
                "operator": "Bataillon de marins-pompiers de Marseille",
            },
        )
        assert not a.node(
            None,
        ...
        assert not a.node(
            None,
            {
                "name": "Beautify fire station",
                "amenity": "fire_station",
                "not:operator:wikidata": "Q2891011",
            },
        )
        assert not a.node(
            None,
