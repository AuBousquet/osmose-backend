...
class Name_Cadastre_FR(PluginMapCSS):
    ...
    def init(self, logger):
        ...
        self.errors[50801] = self.def_class(
            ...
            title=mapcss.tr(
                "Hamlet or Locality name suffix Nord, Sud, Est, Ouest, Centre should be removed from Cadastre name. Place should be integrated only once."
            ),
        )
                ...
                try:
                    match = (
                        (
                            ...
                            == mapcss._value_capture(capture_tags, 0, "locality")
                        )
                        and (
                            mapcss.regexp_test(
                                mapcss._value_capture(
                                    capture_tags, 1, self.re_5d724bf1
                                ),
            ...
            if not match:
                ...
                try:
                    match = (
                        (
                            mapcss._tag_capture(capture_tags, 0, tags, "place")
                            == mapcss._value_capture(capture_tags, 0, "hamlet")
                        )
                        and (
                            mapcss.regexp_test(
                                mapcss._value_capture(
...
class Test(TestPluginMapcss):
    def test(self):
        ...
        with with_options(n, {"country": "FR"}):
            self.check_not_err(
                n.node(data, {"name": "ZA Sud Loire", "place": "hamlet"}),
                expected={"class": 50801, "subclass": 0},
            )
        with with_options(n, {"country": "FR"}):
            self.check_not_err(
                n.node(data, {"name": "Kerbrest", "place": "hamlet"}),
                expected={"class": 50801, "subclass": 0},
            )
        with with_options(n, {"country": "FR"}):
            self.check_err(
                n.node(data, {"name": "Montdésert-Sud", "place": "hamlet"}),
                expected={"class": 50801, "subclass": 0},
            )
