...
class Name_Multiple(Plugin):
    ...
    def init(self, logger):
        ...
        self.errors[705] = self.def_class(
            item=5030,
            level=1,
...
the tag `name:left=*` and `name:right=*`."""
        ...
        self.errors[50301] = self.def_class(
            item=5030,
            level=2,
            tags=["name", "fix:chair"],
...
The tag `name` should have the value `name:right / name:left` or `name:left / name:right`."""
        ...
        if self.father.config.options.get("country"):
            self.NoExtra = any(
                map(
                    lambda c: self.father.config.options.get("country").startswith(c),
                    ["DE", "US", "CA"],
                )
            )

            self.HighwayOnly = self.father.config.options.get("country").startswith(
                "BY"
            )
        ...
        self.streetSubNumberRe = re.compile(".*[0-9๐๑๒๓๔๕๖๗๘๙]/[0-9๐๑๒๓๔๕๖๗๘๙].*")
    ...
    def way(self, data, tags, nds):
        ...
        if ";" in tags["name"]:
            return {
                "class": 705,
                "subclass": 0,
                "text": {"en": "name={0}".format(tags["name"])},
            }

        if self.HighwayOnly and "highway" not in tags:
            return
        if self.NoExtra:
            return

        if "+" in tags["name"][0:-1] and "P+R" not in tags["name"]:
            return {
                "class": 705,
                "subclass": 2,
                "text": {"en": "name={0}".format(tags["name"])},
        ...
        if "/" in tags["name"] and not self.allowSlash:
            ...
            if "public_transport" in tags and tags["public_transport"] in [
                "platform",
                "stop_position",
            ]:
                return
            ...
            if "name:left" in tags and "name:right" in tags:
                ...
                if (
                    tags["name:left"] in nameparts
                    and tags["name:right"] in nameparts
                    and len(nameparts) == 2
                ):
                    return
                    ...
                    return {
                        ...
                        "fix": {
                            "~": {
                                "name": tags["name:right"] + " / " + tags["name:left"]
                            }
                        },
                    }
...
class Test(TestPluginCommon):
    def test(self):
        ...
        with with_options(p, {"country": "TH"}):
            ...
            assert not p.way(
                None, {"name": "aueuie + ueuaeuie", "aeroway": "yes"}, None
            )
            assert not p.way(None, {"name": "Profil+"}, None)
            assert not p.way(None, {"name": "บ้านแพะแม่คือ ซอย 5/10"}, None)
            assert not p.way(None, {"name": "บ้านแพะแม่คือ ซอย 5/๓๔๕"}, None)
            assert not p.way(
                None, {"name": "streetA/streetB", "public_transport": "platform"}, None
            )
            assert not p.way(None, {"name": "Gas station no. 21/2356"}, None)
        ...
        with with_options(p, {"country": "US-TX"}):
            p.init(None)
            assert not p.way(None, {"name": "County Route 7/2"}, None)
            assert not p.way(None, {"name": "16 5/10 Road"}, None)

        with with_options(p, {"country": "BY"}):
            p.init(None)
            assert not p.way(None, {"name": "д/с №68"}, None)
            assert p.way(None, {"name": "д/с №68", "highway": "terciary"}, None)

        with with_options(p, {"country": "CH"}):
            p.init(None)
            assert not p.way(None, {"name": "Waffenplatz-/Bederstrasse"}, None)

        with with_options(p, {"country": "DJ"}):
            p.init(None)
            assert not p.way(None, {"name": "Avenue 17 / جادة 17"}, None)
