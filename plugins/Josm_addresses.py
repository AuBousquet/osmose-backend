...
class Josm_addresses(PluginMapCSS):
    ...
    def init(self, logger):
        ...
        self.errors[9000003] = self.def_class(
            item=9000,
            level=3,
            tags=["tag", "addr"],
            title=mapcss.tr(
        ...
        self.errors[9000004] = self.def_class(
            item=9000,
            level=3,
            tags=["tag", "addr"],
            title=mapcss.tr(
                "{0} without number", mapcss._tag_uncapture(capture_tags, "{0.key}")
            ),
        )
            ...
            if not match:
                ...
                try:
                    match = (
                        ...
                        and (
                            mapcss._tag_capture(capture_tags, 1, tags, "addr:housename")
                        )
                        and (
                            mapcss._tag_capture(
                                capture_tags, 2, tags, "addr:housenumber"
                            )
                            == mapcss._value_capture(
                                capture_tags, 2, mapcss.tag(tags, "addr:housename")
                            )
                        )
                    )
        ...
        if "addr:housenumber" in keys:
            ...
            if not match:
                ...
                try:
                    ...
                    ) and (mapcss.inside(self.father.config.options, "BA"))
                except mapcss.RuleAbort:
                    pass
            if match:
                # set ok_housenumber
...
class Test(TestPluginMapcss):
    def test(self):
        ...
        self.check_err(
            n.node(data, {"addr:housename": "1", "addr:housenumber": "1"}),
            expected={"class": 9000003, "subclass": 1820984183},
        )
        self.check_not_err(
            n.node(data, {"addr:housename": "1", "addr:housenumber": "2"}),
            expected={"class": 9000003, "subclass": 1820984183},
        )
        self.check_not_err(
            n.way(data, {"addr:housenumber": "0"}, [0]),
            expected={"class": 9000004, "subclass": 1053226919},
        )
        self.check_not_err(
            n.way(data, {"addr:housenumber": "5/1"}, [0]),
            expected={"class": 9000004, "subclass": 1053226919},
        )
        self.check_not_err(
            n.way(data, {"addr:housenumber": "5a"}, [0]),
            expected={"class": 9000004, "subclass": 1053226919},
        )
        self.check_err(
            n.way(data, {"addr:housenumber": "?"}, [0]),
            expected={"class": 9000004, "subclass": 1053226919},
        )
        self.check_err(
            n.way(data, {"addr:housenumber": "Palace of Westminster"}, [0]),
            expected={"class": 9000004, "subclass": 1053226919},
        )
        self.check_err(
            n.way(data, {"addr:housenumber": "S/N"}, [0]),
            expected={"class": 9000004, "subclass": 1053226919},
        )
        self.check_err(
            n.way(data, {"addr:housenumber": "unknown"}, [0]),
            expected={"class": 9000004, "subclass": 1053226919},
        )
