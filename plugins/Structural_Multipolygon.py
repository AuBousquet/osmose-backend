...
class Structural_Multipolygon(Plugin):
    ...
    def init(self, logger):
        ...
        self.errors[11703] = self.def_class(
            ...
            fix=T_("""Find the way outside, it may be deleted, check the history."""),
        )
        self.errors[11704] = self.def_class(
            ...
            detail=T_("""Multipolygon relation actually defines a simple polygon."""),
        )
    ...
    def relation(self, data, tags, members):
        ...
        if len(err_roles) > 0:
            err.append(
                {"class": 11701, "subclass": 1, "text": {"en": ", ".join(err_roles)}}
            )
        if len(err_members) > 0:
            err.append(
                {"class": 11702, "subclass": 1, "text": {"en": ", ".join(err_members)}}
            )
