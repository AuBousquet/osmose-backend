...
class TagFix_IntervalConditional(Plugin):
    ...
    def init(self, logger):
        ...
        self.errors[32502] = self.def_class(
            item=3250,
            level=3,
            tags=["value", "fix:chair", "public_transport"],
    ...
    def check_tags(self, tags):
        ...
        if parsedData["defaultInterval"] == "invalid":
            return {
                "class": 32502,
                "subclass": 1,
                "text": T_("Invalid interval tag format"),
            }
        ...
        if parsedData["otherIntervals"] == "invalid":
            return {
                "class": 32502,
                "subclass": 2,
                "text": T_("Conditional interval tag is not valid"),
            }

        # Check combination of opening_hours, interval and interval:conditional
        if (
            "opening_hours" in tags
            and type(parsedData["opens"]) is dict
            and parsedData["allComputedIntervals"] == "invalid"
        ):
            return {
                "class": 32502,
                "subclass": 3,
                "text": T_("Conditional interval does not fit into opening hours"),
            }
...
class Test(TestPluginCommon):
    def test(self):
        ...
        self.check_err(
            a.relation(
                None,
                {"interval": "01:30", "interval:conditional": "Invalid conditional"},
                None,
            )
        )
        self.check_err(
            a.relation(
                None,
                {
                    "opening_hours": "Mo-Fr 05:00-23:00",
                    "interval": "15",
                    "interval:conditional": "5 @ (Mo-Sa 12:00-15:00)",
                },
                None,
            )
        )
        ...
        assert not a.relation(
            None,
            {"interval": "00:10", "interval:conditional": "15 @ (Mo-Sa 15:00-17:00)"},
            None,
        )
        assert not a.relation(
            None,
            {
                ...
                "interval": "00:10",
                "interval:conditional": "15 @ (Mo-Sa 15:00-17:00)",
            },
            None,
        )
