...
class Website(Plugin):
    ...
    def init(self, logger):
        ...
        self.errors[30933] = self.def_class(
            item=3093, level=3, tags=["value", "fix:chair"], title=T_("Invalid URL")
        )
        self.errors[30934] = self.def_class(
            item=3093,
            level=3,
            tags=["value", "fix:chair"],
            title=T_("Tracking parameter in URL"),
            fix=T_(
                "Strip the tracking parameter from the URL. Verify that the URL still works afterwards."
            ),
    ...
    def _load_trackingparameter_list(self):
        ...
        csvlines = urlread(url, 30).splitlines()[
            1:
        ]  # Discard first line, it contains headers. Use urlread to allow caching the external file
        csvparsed = csv.reader(csvlines, delimiter=",")
        # Filter entries of len<=2 for safety. First column contains the parameter
        return set(
            filter(
                lambda p: len(p) > 2,
                map(lambda linesplit: linesplit[0].strip(), csvparsed),
            )
        )
                    ...
                    err.append(
                        {
                            "class": 30933,
                            "subclass": stablehash64(tag),
                            "text": T_("Bad URL in `{0}`: {1}", tag, str(e)),
                        }
                    )
                    continue
                queryparams = parsed_url.query.split(
                    "&"
                )  # not parse_qs/parse_qsl because we don't want to change whether i.e. + is encoded in the fix
                if any(
                    map(
                        lambda qs: qs.split("=")[0] in self.strippable_queryparameters,
                        queryparams,
                    )
                ):
                    stripped_query = "&".join(
                        list(
                            filter(
                                lambda qs: qs.split("=")[0]
                                not in self.strippable_queryparameters,
                                queryparams,
                            )
                        )
                    )
            ...
            err.append(
                {
                    "class": 30932,
                    "subclass": stablehash64(tag),
                    "fix": [{tag: "https://" + url}, {tag: "http://" + url}],
                }
            )
...
class Test(TestPluginCommon):
    def test(self):
        ...
        for bad in (
            ...
            self.assertEqual(
                err[0]["fix"][0]["website"], "https://{0}".format(test_url)
            )
        ...
        err = p.node(
            None,
            {
                "website": "https://osmose.osmose/osmose?osmose=osmose&fbclid=abcdefghijkl&osmose2=test+%2Btest%20test&osmose3=&osmose4"
            },
        )
        self.check_err(err)
        self.assertEqual(
            err[0]["fix"]["~"]["website"],
            "https://osmose.osmose/osmose?osmose=osmose&osmose2=test+%2Btest%20test&osmose3=&osmose4",
        )
        err = p.node(
            None,
            {
                "website": "https://osmose.osmose/osmose/osmose/?utm_campaign=abcdefghijkl#osmose"
            },
        )
        self.check_err(err)
        self.assertEqual(
            err[0]["fix"]["~"]["website"], "https://osmose.osmose/osmose/osmose/#osmose"
        )

        # Verify we get no error for other correct URLs
        for good in (
            "ftp://{0}".format(test_url),
        ...
        assert not p.node(
            None,
            {
                "url": "http://ancien-geodesie.ign.fr/fiche_point_OM.asp?num_site=9712301&#38;no_ptg=04"
            },
        )
        assert not p.node(
            None,
            {"url": "https://osmose.osmose/osmose/osmose?osmose=osmose&name=osmose"},
        )  # Ensure we strip the header line of the CSV file
