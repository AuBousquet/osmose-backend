...
class TagFix_MultipleTag(Plugin):
    ...
    def init(self, logger):
        ...
        self.errors[30323] = self.def_class(
            item=3032,
            level=3,
            tags=["tag", "fix:chair"],
            title=T_("Watch multiple tags"),
        )
        self.errors[20803] = self.def_class(
            item=2080,
            level=2,
            tags=["tag", "highway", "fix:chair"],
            title=T_("Tag highway missing for tracktype or lanes"),
        )
        self.errors[21101] = self.def_class(
            item=2110,
            level=2,
            tags=["tag"],
            title=T_("Untagged named object"),
            detail=T_(
                "The object is missing any tag which defines what kind of feature it is. This is unexpected for something with a `name` tag."
            ),
            fix=self.merge_doc(
                T_(
                    "Add a top level tag to state what this feature is. Considered top level tags are (with derived `disused:`, `abandoned:` and `historic:` variants):"
                ),
                {"en": ", ".join(map(lambda x: "`{}`".format(x), sorted(main_tags)))},
            ),
            trap=T_(
                "It may be more appropriate to remove the object completely if it isn't useful."
            ),
        )
        self.errors[1050] = self.def_class(
            item=1050,
            level=1,
...
Clockwise rotation."""
        ...
        self.errors[32200] = self.def_class(
            item=3220,
            level=2,
...
For further detail, see [the wiki](https://wiki.openstreetmap.org/wiki/Key:access#Transport_mode_restrictions)."""
        ...
        self.errors[32201] = self.def_class(
            item=3220,
            level=2,
            tags=["highway", "fix:chair"],
            title=T_("Overly permissive access"),
...
For further detail, see [the wiki](https://wiki.openstreetmap.org/wiki/Key:access#Transport_mode_restrictions)."""
        ...
        self.driving_side_right = not (
            self.father.config.options.get("driving_side") == "left"
        )
        self.driving_direction = (
            "anticlockwise" if self.driving_side_right else "clockwise"
        )
        name_parent = []
        for i in main_tags:
    ...
    def common(self, tags, key_set):
        ...
        if (
            tags.get("name")
            and len(key_set & self.name_parent) == 0
            and tags.get("naptan:verified") != "no"
        ):
    ...
    def node(self, data, tags):
        ...
        if tags.get("highway") == "mini_roundabout" and "direction" in tags:
            ...
            if (self.driving_side_right and clockwise) or (
                not self.driving_side_right and anticlockwise
            ):
                err.append(
                    {
                        "class": 1050,
                        "subclass": 1000,
                        "text": T_(
                            'mini roundabout direction in this country is usually "{0}"',
                            self.driving_direction,
                        ),
                        "fix": {"-": ["direction"]},
                    }
    ...
    def way(self, data, tags, nds):
        ...
        if (
            (tags.get("tracktype") or tags.get("lanes"))
            and not tags.get("highway")
            and not tags.get("disused:highway")
            and not tags.get("abandoned:highway")
            and not tags.get("construction:highway")
            and not tags.get("proposed:highway")
            and not tags.get("planned:highway")
            and not tags.get("leisure") == "track"
        ):
...
class Test(TestPluginCommon):
    def test(self):
        ...
        assert not a.node(
            ...
            {
                "name": "foo",
                "traffic_sign:forward": "city_limit;DE:310",
                "traffic_sign:backward": "city_limit;DE:311",
            },
        )
