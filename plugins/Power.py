            ...
            if not match:
                ...
                try:
                    match = (
                        mapcss._tag_capture(capture_tags, 0, tags, "power")
                        == mapcss._value_capture(capture_tags, 0, "transformer")
                    ) and (not mapcss._tag_capture(capture_tags, 1, tags, "frequency"))
                except mapcss.RuleAbort:
                    pass
            if match:
                # -osmoseTags:list("tag")
                ...
                err.append(
                    {
                        ...
                        "subclass": 0,
                        "text": mapcss.tr(
                            "Power Transformers should always be on a node"
                        ),
                    }
                )
...
class Test(TestPluginMapcss):
    def test(self):
        ...
        self.check_not_err(
            n.node(data, {"frequency": "50", "power": "transformer"}),
            expected={"class": 91003, "subclass": 0},
        )
        self.check_err(
            n.node(data, {"power": "transformer"}),
            expected={"class": 91003, "subclass": 0},
        )
