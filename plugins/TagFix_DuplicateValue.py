...
Ensure the interpretation of the tag does not change when you delete one item."""
        ...
        self.errors[3060] = self.def_class(
            item=3060,
            level=3,
        ...
        self.errors[30601] = self.def_class(
            item=3060,
            level=3,
            tags=["value", "fix:chair"],
        ...
        self.WhitelistSimilarEqual = set(
            (  # Keys that can have similar and even equal values
                "CLC:id",
                "GNS:id",
                "tmc",
                "tiger:cfcc",
                ...
                "note",
                "description",
                "inscription",  # Text; not ;-separated values
            )
        )
        self.WhitelistSimilar = set(
            (  # Keys that can have similar, but not equal values
        ...
        self.WhitelistSimilarEqualRegex = set(
            (  # Regexes for keys that can have similar and even equal values
                re.compile("seamark:.+:colour"),
                re.compile(".+_ref"),
                re.compile("ref:.+"),
                re.compile("destination:.+"),
                re.compile("AND_.+"),
                re.compile("AND:.+"),  # Netherlands
                re.compile("[Nn][Hh][Dd]:.+"),
                re.compile("massgis:.+"),
                re.compile("lacounty:.+"),
                ...
                re.compile(
                    ".+:conditional"
                ),  # More thorough check in ConditionalRestrictions plugin
            )
        )
        self.WhitelistSimilarRegex = set(
            (  # Regexes for keys that can have similar, but not equal values
                re.compile(".+_name"),
                re.compile(".+:date"),
                re.compile(".+:colour"),
    ...
    def node(self, data, tags):
        ...
        for k in keys:
            ...
            vs = list(
                filter(lambda w: len(w) > 0, map(lambda w: w.strip(), v.split(";")))
            )
                        ...
                        err.append(
                            {
                                ...
                                "subclass": stablehash64(k),
                                "text": T_(
                                    "Similar values {v1} and {v2} in {key}={val}",
                                    v1=v1,
                                    v2=v2,
                                    key=k,
                                    val=tags[k],
...
class Test(TestPluginCommon):
    def test(self):
        ...
        for t in [
            ...
            {
                "source": "cadastre-dgi-fr source : Direction Générale des Impôts - Cadastre ; mise à jour : 2013;cadastre-dgi-fr source : Direction Générale des Impôts - Cadastre ; mise à jour : 2013"
            },
            {
                "source": "cadastre-dgi-fr source : Direction Générale des Impôts - Cadastre ; mise à jour : 2010;cadastre-dgi-fr source : Direction Générale des Impôts - Cadastre ; mise à jour : 2013"
            },
        ...
        for t in [
            {"ref": "E 05; E 70; E 05;E 70; E 05;E 70; E 05;E 70; E 05;E 70"},
