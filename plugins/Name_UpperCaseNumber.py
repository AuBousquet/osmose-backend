...
class Name_UpperCaseNumber(Plugin):
    ...
    def init(self, logger):
        ...
        self.errors[905] = self.def_class(
            item=5010,
            level=1,
            tags=["name", "fix:chair"],
            title=T_("Abbreviation of number should be in lowercase"),
            detail=T_("""Number written in capital letters: "N°" in place of "n°"."""),
        )

        import re

        self.ReNUpperCase = re.compile("^(|.* )N(°[0-9]+)(| .*)$")
        ...
        if "name" in tags:
            ...
            if r:
                return {
                    "class": 905,
                    "fix": {
                        "name": "{0}n{1}{2}".format(r.group(1), r.group(2), r.group(3))
                    },
                }
