...
class Highway_Sides(Plugin):
    def init(self, logger):
        ...
        self.errors[33601] = self.def_class(
            item=3360,
            level=2,
    ...
    def way(self, data, tags, nds):
        ...
        for tag in tags:
            if tag[-5:] == ":both":
                tag_default = tag[
                    0:-5
                ]  # special handling because :both_ways also exists
            else:
                tag_default = (
                    tag.replace(":both:", ":")
                    .replace(":left", "")
            ...
            if tag_default in tags:
                ...
                if tt != ttd and ttd not in allowedAlternativeValues:
                    err.append(
                        {
                            "class": 33601,
                            "subclass": 1 + stablehash64(tag),
                            "text": T_(
                                'Conflicting values of "{0}" and "{1}"',
                                tag_default,
                                tag,
                            ),
                        }
