...
class Covid19(PluginMapCSS):
    ...
    def init(self, logger):
        ...
        self.errors[202005] = self.def_class(
            item=4010,
            level=2,
            tags=mapcss.list_("tag", "fix:survey", "deprecated"),
            title=mapcss.tr(
                "This store offered a take-away service during the lockdown. Does it still offer take-away in usual times"
            ),
        )
        self.errors[202006] = self.def_class(
            item=4010,
            level=2,
            tags=mapcss.list_("tag", "fix:survey", "deprecated"),
            title=mapcss.tr(
                "This store offered a delivery service during the lockdown. Does it still offer delivery in usual times"
            ),
        )
        self.errors[202007] = self.def_class(
            item=4010,
            level=2,
            tags=mapcss.list_("tag", "fix:survey", "deprecated"),
            title=mapcss.tr("The lockdown is over. Has this place reopened?"),
        )
        self.errors[202008] = self.def_class(
            item=4010,
            level=2,
            tags=mapcss.list_("tag", "fix:survey", "deprecated"),
            title=mapcss.tr(
                "This store was on an adapted schedule during the lockdown. Are these opening hours still in effect?"
            ),
        )
            ...
            if not match:
                ...
                try:
                    match = (
                        (
                            mapcss._tag_capture(
                                capture_tags, 0, tags, "takeaway:covid19"
                            )
            ...
            if not match:
                ...
                try:
                    match = (
                        (
                            mapcss._tag_capture(
                                capture_tags, 0, tags, "delivery:covid19"
                            )
                            == mapcss._value_capture(capture_tags, 0, "yes")
                        )
                        and (
            ...
            if not match:
                ...
                try:
                    match = (
                        (
                            mapcss._tag_capture(
                                capture_tags, 0, tags, "opening_hours:covid19"
                            )
                        )
                        and (
                            mapcss.regexp_test(
                                mapcss._value_capture(
                                    capture_tags, 1, self.re_3f390088
                                ),
                                mapcss._tag_capture(
                                    capture_tags, 1, tags, "opening_hours:covid19"
                                ),
                            )
                        )
                        and (mapcss.inside(self.father.config.options, "FR"))
...
class Test(TestPluginMapcss):
    def test(self):
        ...
        with with_options(n, {"country": "FR"}):
            self.check_err(
                n.node(data, {"takeaway": "no", "takeaway:covid19": "yes"}),
                expected={"class": 202005, "subclass": 0},
            )
        with with_options(n, {"country": "FR"}):
            self.check_err(
                n.node(data, {"delivery": "no", "delivery:covid19": "yes"}),
                expected={"class": 202006, "subclass": 0},
            )
        with with_options(n, {"country": "FR"}):
            self.check_not_err(
                n.way(data, {"takeaway:covid19": "no"}, [0]),
                expected={"class": 202005, "subclass": 0},
            )
        with with_options(n, {"country": "FR"}):
            self.check_not_err(
                n.way(data, {"takeaway:covid19": "only"}, [0]),
                expected={"class": 202005, "subclass": 0},
            )
        with with_options(n, {"country": "FR"}):
            self.check_not_err(
                n.way(data, {"takeaway": "yes", "takeaway:covid19": "yes"}, [0]),
                expected={"class": 202005, "subclass": 0},
            )
        with with_options(n, {"country": "FR"}):
            self.check_err(
                n.way(data, {"takeaway:covid19": "yes"}, [0]),
                expected={"class": 202005, "subclass": 0},
            )
        with with_options(n, {"country": "FR"}):
            self.check_not_err(
                n.way(data, {"delivery:covid19": "no"}, [0]),
                expected={"class": 202006, "subclass": 0},
            )
        with with_options(n, {"country": "FR"}):
            self.check_not_err(
                n.way(data, {"delivery:covid19": "only"}, [0]),
                expected={"class": 202006, "subclass": 0},
            )
        with with_options(n, {"country": "FR"}):
            self.check_not_err(
                n.way(data, {"delivery": "yes", "delivery:covid19": "yes"}, [0]),
                expected={"class": 202006, "subclass": 0},
            )
        with with_options(n, {"country": "FR"}):
            self.check_err(
                n.way(data, {"delivery:covid19": "yes"}, [0]),
                expected={"class": 202006, "subclass": 0},
            )
        with with_options(n, {"country": "FR"}):
            self.check_not_err(
                ...
                expected={"class": 202007, "subclass": 0},
            )
        with with_options(n, {"country": "FR"}):
            self.check_err(
                n.way(data, {"opening_hours:covid19": "restricted"}, [0]),
                expected={"class": 202007, "subclass": 0},
            )
        with with_options(n, {"country": "FR"}):
            self.check_not_err(
                n.way(data, {"opening_hours:covid19": "same"}, [0]),
                expected={"class": 202007, "subclass": 0},
            )
        with with_options(n, {"country": "FR"}):
            self.check_not_err(
                ...
                expected={"class": 202008, "subclass": 0},
            )
        with with_options(n, {"country": "FR"}):
            self.check_err(
                n.way(data, {"opening_hours:covid19": "Mo-Su 09:00-20:00"}, [0]),
                expected={"class": 202008, "subclass": 0},
            )
