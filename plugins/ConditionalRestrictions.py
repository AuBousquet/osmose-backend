...
class ConditionalRestrictions(Plugin):
    def init(self, logger):
        ...
        self.ReTimeOpeningH = re.compile(
            r"\d\D[\d-]|sun[sr][ei][ts]"
        )  # i.e. 5:30 or 5h30 or 5h-8h
        # Workaround https://bugs.kde.org/show_bug.cgi?id=452236
        self.kOpeningHours452236 = re.compile(
            r"((?P<yyyy>20\d\d) [A-Z][a-z][a-z] \d\d?\s?-\s?)(?P=yyyy) ([A-Z][a-z][a-z] \d\d?)"
        )

        self.getDuplicatesFromList = lambda lst: [
        ...
        self.errors[33501] = self.def_class(
            item=3350,
            level=2,
...
Parentheses `()` must be used around the condition if the condition itself contains semicolons `;`, i.e. `value @ (date;date)`."""
        ...
        self.errors[33502] = self.def_class(
            item=3350,
            level=3,
            tags=["highway", "fix:chair"],
            title=T_("Improve style of conditional"),
            detail=T_(
                """Although valid, it is recommended to format conditional restrictions with:
- spaces around the `@`;
...
For example, use `no @ (weight > 5 AND wet)` rather than `no@weight>5 and wet`."""
        ...
        self.errors[33503] = self.def_class(
            item=3350,
            level=3,
            tags=["highway", "fix:chair"],
        ...
        self.errors[33504] = self.def_class(
            item=3350,
            level=3,
            tags=["highway", "fix:chair"],
            title=T_("Invalid date/time span"),
            detail=T_(
                """A date/time-based condition is invalid or poorly formatted. Time-based conditions of a conditional restriction use the same syntax as the key `opening_hours`."""
            ),
        )
        self.errors[33505] = self.def_class(
            item=3350,
            level=3,
            tags=["highway", "fix:chair"],
            title=T_("Repeated condition in conditional restriction"),
            detail=T_(
                """Two different values are set to apply under the same condition or a condition is repeated. The first value is overruled by the second value."""
            ),
            example=T_(
...
`maxspeed:conditional = 40 @ (12:00-24:00); 60 @ (00:00-12:00)`."""
        ...
        for tag in tags_conditional:
            ...
            if "@" not in tag_value:
                err.append(
                    {
                        "class": 33501,
                        "subclass": 0 + stablehash64(tag + "|" + tag_value),
                        "text": T_('Missing `@` in "{0}"', tag),
                    }
                )
                continue
                ...
                if c == "@" and not past_parentheses:
                    if len(tmp_str.strip()) == 0:
                        err.append(
                            {
                                "class": 33501,
                                "subclass": 1 + stablehash64(tag + "|" + tag_value),
                                "text": T_(
                                    'Missing value for the condition in "{0}"', tag
                                ),
                            }
                        )
                        bad_tag = True
                    ...
                    if parentheses == 1 and tmp_str.lstrip() != "":
                        err.append(
                            {
                                "class": 33501,
                                "subclass": 7 + stablehash64(tag + "|" + tag_value),
                                "text": T_(
                                    'Unexpected "{0}" before or after parentheses in "{1}"',
                                    tmp_str.strip(),
                                    tag,
                                ),
                            }
                    ...
                    if parentheses == -1:
                        err.append(
                            {
                                "class": 33501,
                                "subclass": 2 + stablehash64(tag + "|" + tag_value),
                                "text": T_(
                                    'Mismatch in the number of parentheses in "{0}"',
                                    tag,
                                ),
                            }
                        )
                    ...
                    if len(tmp_str) == 0:
                        err.append(
                            {
                                "class": 33501,
                                "subclass": 3 + stablehash64(tag + "|" + tag_value),
                                "text": T_(
                                    'Missing condition, `@` or parentheses in "{0}"',
                                    tag,
                                ),
                            }
                        )
                    ...
                    err.append(
                        {
                            ...
                            "subclass": 2 + stablehash64(tag + "|" + tag_value),
                            "text": T_(
                                'Mismatch in the number of parentheses in "{0}"', tag
                            ),
                        }
                    )
                    continue
            ...
            for d in self.getDuplicatesFromList(conditions):
                err.append(
                    {
                        "class": 33505,
                        "subclass": 0 + stablehash64(tag + "|" + tag_value + "|" + d),
                        "text": T_("Repeated condition `{0}` in `{1}`", d, tag),
                    }
                )
            ...
            if not bad_tag:
                ...
                for condition in set(conditions):
                    condition_ANDsplitted = list(
                        map(str.strip, self.ReAND.split(condition))
                    )
                        ...
                        err.append(
                            {
                                "class": 33501,
                                "subclass": 4
                                + stablehash64(tag + "|" + tag_value + "|" + condition),
                                "text": T_(
                                    'Missing condition before or after AND combinator in "{0}"',
                                    tag,
                                ),
                            }
                        )
                        ...
                        err.append(
                            {
                                ...
                                "subclass": 0
                                + stablehash64(
                                    tag + "|" + tag_value + "|" + condition + "|" + d
                                ),
                                "text": T_(
                                    "Repeated condition `{0}` in `{1}` (in tag `{2}`)",
                                    d,
                                    condition,
                                    tag,
                                ),
                ...
                for c in set(allANDsplittedConditions):
                    ...
                    if self.isLikelyOpeningHourSyntax(c):
                        sanitized = self.sanitize_openinghours(
                            self.kOpeningHours452236.sub(r"\1\3", c)
                        )
                                ...
                                err.append(
                                    {
                                        "class": 33504,
                                        "subclass": 6
                                        + stablehash64(tag + "|" + tag_value + "|" + c),
                                        "text": T_(
                                            'Involves "{0}" in "{1}". Consider using "{2}"',
                                            c,
                                            tag,
                                            sanitized["fix"],
                                        ),
                        ...
                        if (
                            c[0] in self.comparisonOperatorChars
                            or c[-1] in self.comparisonOperatorChars
                        ):
                            err.append(
                                {
                                    "class": 33501,
                                    "subclass": 5
                                    + stablehash64(tag + "|" + tag_value + "|" + c),
                                    "text": T_('Unexpected <, = or > in "{0}"', tag),
                                }
                            )
                            bad_tag = True
                            break
                    ...
                    err.append(
                        {
                            ...
                            + stablehash64(tag + "|" + tag_value + "|" + condition),
                            "text": T_(
                                'Condition "{0}" in "{1}" was only valid until {2}',
                                condition,
                                tag,
                                maxYear,
                            ),
            ...
            if tag_value.count("(") < len(conditions):
                if not (
                    tag_value.count("(") == len(conditions) - 1
                    and not tag_value[-1] == ")"
                    and re.search(self.ReSimpleCondition, conditions[-1])
                ):
                    ...
                    err.append(
                        {
                            ...
                            "subclass": 0 + stablehash64(tag + "|" + tag_value),
                            "text": T_(
                                'Add parentheses around the condition(s) in "{0}"', tag
                            ),
                        }
                    )
    ...
    def isLikelyOpeningHourSyntax(self, condition):
        ...
        for s in self.comparisonOperatorChars:
            if s in condition:
                score -= (
                    1  # weight < 25 or so, no meaning in date/time-based conditionals
                )
        if score >= threshold:
            return True
        if score < 0:
        ...
        for r in [
            self.ReYear,
            self.ReWeekdayMonthOpeningH,
            self.ReTimeOpeningH,
            self.ReMonthDayOpeningH,
        ]:
...
class Test(TestPluginCommon):
    def test(self):
        ...
        for t in [
            ...
            },  # note: remove if we start warning about missing spaces around @
            {
                "highway": "residential",
                "access:conditional": "no @ wet",
                "source:access:conditional": "survey",
            },
            {"highway": "residential", "maxspeed:conditional": "20 @ 06:00-19:00"},
            {"highway": "residential", "maxspeed:conditional": "20 @ (06:00-19:00)"},
            {
                "highway": "residential",
                "maxspeed:conditional": "20 @ (06:00-20:00); 100 @ (22:00-06:00)",
            },
            {
                "highway": "residential",
                "access:forward:conditional": "delivery @ (Mo-Fr 06:00-11:00,17:00-19:00;Sa 03:30-19:00)",
            },
            {
                "highway": "residential",
                "access:forward:conditional": "no @ (10:00-18:00 AND length>5)",
            },
            {
                "highway": "residential",
                "access:forward:conditional": "no @ (Mo-Fr 10:00-18:00 and wet)",
            },  # suboptimal but valid: and instead of AND
            {
                "highway": "residential",
                "access:forward:conditional": "no @ (Mo-Fr 10:00-18:00 and wet AND weight > 5)",
            },  # suboptimal but valid: and instead of AND
            {"highway": "residential", "access:conditional": "no @ 2099"},
            {
                "highway": "residential",
                "access:conditional": "no @ (weight >= 12020 AND length < 20200)",
            },
            {
                "highway": "residential",
                "access:conditional": "no @ (2099 May 22-2099 Oct 07 Mo-Su 09:00-18:00)",
            },
            {
                "highway": "residential",
                "access:conditional": "no @ (2098 May 22-2099 May 27 Mo-Su 09:00-18:00)",
            },
            {
                "highway": "residential",
                "access:conditional": "no @ (2010 May 22-2099 Oct 07)",
            },
            {
                "highway": "residential",
            ...
            {
                "highway": "service",
                "access:conditional": 'customers @ (Mo-Fr || "by appointment and >5 persons")',
            },
            {
                "highway": "residential",
                "maxspeed:conditional": "20 @ (06:00-19:00); 40 @ (06:00-19:00 AND length < 2)",
            },
        ]:
        ...
        for t in [
            {"highway": "residential", "access:forward:conditional": "no @ 2020"},
            {
                "highway": "residential",
                "access:conditional": "no @ (2018 May 22-2020 Oct 07)",
            },
            {
                "highway": "residential",
                "access:conditional": "no @ (2018 May 22-2020 Oct 07); delivery @ 2099",
            },
            {
                "highway": "residential",
                "access:conditional": "no @ (2018 May 22-2020 Oct 07); destination @ (length < 4)",
            },
            {
                "highway": "residential",
                "access:conditional": "no @ (2018 May 22-2020 Oct 07 AND weight > 5)",
            },
        ]:
        ...
        for t in [
            {"highway": "residential", "access:conditional": "no @ (weight >)"},
            {
                "highway": "residential",
                "access:conditional": "no @ (foggy AND weight <= AND wet); destination @ snow",
            },
            {"highway": "residential", "access:conditional": "no @ (<=4 axles)"},
            {
                "highway": "residential",
                "access:conditional": "no @ (2098-05-22 - 2099-10-7)",
            },
            {
                "highway": "residential",
                "access:conditional": "no @ (22 mei 2099 - 07 okt 2099)",
            },
            {"highway": "residential", "access:conditional": "no @ (JUL 01-JAN 31)"},
            {"highway": "residential", "access:conditional": "no @ (6h00-19h00)"},
            {
                "highway": "residential",
                "access:conditional": "no @ (Ma-Vr 18:00-20:00); destination @ (length < 4)",
            },
            {
                "highway": "residential",
                "access:conditional": "no @ (Mei 22 - Okt 7 AND weight > 5)",
            },
            {
                "highway": "residential",
                "access:conditional": "no @ (weight > 5 AND mei 22 - okt 7)",
            },
        ]:
            assert a.way(None, t, None), a.way(None, t, None)
        ...
        for t in [
            {
                "highway": "residential",
                "access:conditional": "no @ (weight >= 12020 AND length < 20200 AND weight >= 12020)",
            },
            {
                "highway": "residential",
                "maxspeed:conditional": "20 @ (06:00-19:00); 40 @ (06:00-19:00)",
            },
            {
                "highway": "residential",
                "maxspeed:conditional": "40 @ (06:00-19:00); 40 @ (06:00-19:00)",
            },
        ]:
            assert a.way(None, t, None), a.way(None, t, None)
        ...
        for t in [
            {"highway": "residential", "access:conditional": "no"},
            ...
            {
                "highway": "residential",
                "access:conditional": "delivery @ Mo-Fr 06:00-11:00,17:00-19:00;Sa 03:30-19:00);yes@wet",
            },
            {
                "highway": "residential",
                "access:conditional": "delivery @ (Mo-Fr 06:00-11:00,17:00-19:00;Sa 03:30-19:00;yes@wet",
            ...
            {
                "highway": "residential",
                "bicycle:conditional": "designated @ (Fr-Mo 22:00-00:00); (Fr-Mo 22:00-24:00)",
            },
            {
                "highway": "residential",
                "access:conditional": "no @ (Mo-Fr 22:00-00:00);",
            },
            ...
            {
                "highway": "residential",
                "access:conditional": "no @ (abc)_; no @ (06:00-19:00)",
            },
            {
                "highway": "residential",
                "access:conditional": "no @ (abc); no @ (06:00-19:00)_",
            },
            {
                "highway": "residential",
                "access:conditional": "no @ (2099 May 22 AND AND 2099 Oct 07)",
            },
            {
                "highway": "residential",
                "access:conditional": "no @ (2099 May 22 AND 2099 Oct 07 AND); delivery @ wet",
            },
            {
                "highway": "residential",
                "maxweight:conditional": "27000 lbs (axles=2); 41400 lbs @ (axles=3); 48600 lbs @ (axles>=4)",
            },
        ]:
            assert a.way(None, t, None), a.way(None, t, None)
        ...
        # Nodes
        assert not a.node(
            None, {"barrier": "lift_gate", "access:conditional": "no @ wet"}
        )
        assert a.node(
            None,
            {
                "barrier": "lift_gate",
                "access:conditional": "no @ Mo-Fr 06:00-11:00,17:00-19:00;Sa 03:30-19:00",
            },
        )

        # Relations
        assert not a.relation(
            None,
            {
                "type": "restriction",
