...
and `potential`."""
        ...
        if country == "CH":
            ...
            self.housenumber = re.compile(
                r"^[1-9][0-9]{0,3}([A-Z]){0,3}(-[1-9][0-9]{0,3}([A-Z]){0,3})?$"
            )
        elif country == "IT":
    ...
    def node(self, data, tags):
        err = []
        if "addr:housenumber" in tags and (
            len(tags["addr:housenumber"]) == 0
            or not (self.housenumber.match(tags["addr:housenumber"]))
        ):
    ...
    def way(self, data, tags, nds):
        ...
        if interpolation:
            if any(
                filter(
                    lambda x: x.startswith("addr:")
                    and x not in ("addr:interpolation", "addr:inclusion"),
                    tags.keys(),
                )
            ):
                err.append({"class": 14, "subclass": 1})
            if (
                interpolation not in ("even", "odd", "all", "alphabetic")
                and not interpolation.isdigit()
            ):
                err.append(
                    {
                        "class": 15,
                        "subclass": 1,
                        "text": {"en": "addr:interpolation={0}".format(interpolation)},
                    }
                )
            ...
            err.append(
                {
                    "class": 15,
                    "subclass": 2,
                    "text": {"en": "addr:inclusion={0}".format(inclusion)},
                }
            )
...
class Test(TestPluginCommon):
    def test(self):
        ...
        assert not a.way(
            None, {"addr:interpolation": "4", "addr:inclusion": "actual"}, None
        )
        assert a.way(None, {"addr:interpolation": "invalid"}, None)

        assert a.way(
            None, {"addr:housenumber": "ev.387"}, None
        )  # CZ housenumbers not valid outside CZ
    ...
    def test_NL(self):
        ...
        # Trolls, bogus input.
        assert a.node(
            None, {"addr:housenumber": "1234567890abcdefghijklmnopqrstuvwxyz"}
        )
