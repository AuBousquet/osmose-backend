...
class Construction2(PluginMapCSS):
    ...
    def init(self, logger):
        ...
        self.errors[40701] = self.def_class(
            item=4070,
            level=1,
            tags=mapcss.list_("tag", "highway", "fix:survey"),
            title=mapcss.tr(
    ...
    def way(self, data, tags, nds):
        ...
        if (
            ("construction" in keys and "highway" in keys)
            or ("construction" in keys and "railway" in keys)
            or ("highway" in keys and "proposed" in keys)
            or ("proposed" in keys and "railway" in keys)
        ):
            match = False
            if not match:
                capture_tags = {}
                try:
                    match = (
                        ...
                        and (
                            mapcss._tag_capture(capture_tags, 3, tags, "construction")
                            != mapcss._value_const_capture(
            ...
            if not match:
                ...
                try:
                    match = (
                        (mapcss._tag_capture(capture_tags, 0, tags, "highway"))
                        and (mapcss._tag_capture(capture_tags, 1, tags, "proposed"))
                        and (
                            mapcss._tag_capture(capture_tags, 2, tags, "highway")
                            != mapcss._value_const_capture(
                                capture_tags, 2, "proposed", "proposed"
                            )
                        )
                    )
                except mapcss.RuleAbort:
                ...
                try:
                    match = (
                        ...
                        and (
                            ...
                            != mapcss._value_const_capture(
                                capture_tags, 3, "abandoned", "abandoned"
                            )
                        )
                        and (
                            mapcss._tag_capture(capture_tags, 4, tags, "railway")
                            != mapcss._value_const_capture(
                                capture_tags, 4, "razed", "razed"
                            )
                        )
                        and (
                        ...
                        and (
                            mapcss._tag_capture(capture_tags, 6, tags, "construction")
                            != mapcss._value_const_capture(
                ...
                try:
                    match = (
                        ...
                        and (
                            ...
                            != mapcss._value_const_capture(
                                capture_tags, 6, "disused", "disused"
                            )
                        )
                    )
                except mapcss.RuleAbort:
...
class Test(TestPluginMapcss):
    def test(self):
        ...
        self.check_not_err(
            n.way(data, {"construction": "primary", "highway": "construction"}, [0]),
            expected={"class": 40701, "subclass": 0},
        )
        self.check_not_err(
            n.way(data, {"construction": "minor", "highway": "primary"}, [0]),
            expected={"class": 40701, "subclass": 0},
        )
        self.check_err(
            n.way(data, {"construction": "primary", "highway": "primary"}, [0]),
            expected={"class": 40701, "subclass": 0},
        )
