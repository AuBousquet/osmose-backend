...
class Bicycle(PluginMapCSS):
    ...
    def init(self, logger):
        ...
        self.errors[20301] = self.def_class(
            item=2030,
            level=1,
            tags=mapcss.list_("tag", "highway")
            + mapcss.list_("cycleway", "fix:survey"),
            title=mapcss.tr("Opposite cycleway without oneway"),
        )
        self.errors[20302] = self.def_class(
            item=2030,
            level=1,
            tags=mapcss.list_("tag", "highway")
            + mapcss.list_("cycleway", "fix:survey"),
            title=mapcss.tr("Opposite or opposite lane in the same way of the oneway"),
        )
        self.errors[20805] = self.def_class(
            item=2080,
            level=3,
            tags=mapcss.list_("tag", "highway") + mapcss.list_("footway", "fix:chair"),
            title=mapcss.tr(
                "{0} without {1}", "footway=sidewalk", "highway=footway|construction"
            ),
        )
        self.errors[30328] = self.def_class(
            item=3032,
            level=2,
            tags=mapcss.list_("tag", "highway") + mapcss.list_("cycleway", "fix:chair"),
            title=mapcss.tr("{0} with {1}", "highway=cycleway", "cycleway=track"),
        )
        self.errors[30329] = self.def_class(
            item=3032,
            level=2,
            tags=mapcss.list_("tag", "highway") + mapcss.list_("fix:chair"),
            title=mapcss.tr(
                "{0} with {1}",
        ...
        self.errors[40101] = self.def_class(
            item=4010,
            level=2,
            tags=mapcss.list_("tag", "highway") + mapcss.list_("fix:chair"),
            title=mapcss.tr("{0} is preferred to {1}", "psv", "service=psv"),
        )
        self.errors[40301] = self.def_class(
            item=4030,
            level=2,
            tags=mapcss.list_("tag", "highway") + mapcss.list_("cycleway", "fix:chair"),
            title=mapcss.tr(
                "{0} with {1} and {2}", "cycleway", "cycleway:right", "cycleway:left"
            ),
        )
    ...
    def way(self, data, tags, nds):
            ...
            if not match:
                ...
                try:
                    match = (
                        ...
                        and (
                            mapcss._tag_capture(capture_tags, 2, tags, "cycleway:left")
                        )
                    )
            ...
            if not match:
                ...
                try:
                    match = (
                        ...
                        and (
                            mapcss._tag_capture(capture_tags, 1, tags, "service")
                ...
                err.append(
                    {
                        ...
                        "subclass": 0,
                        "text": mapcss.tr(
                            "{0} with {1}", "highway=cycleway", "cycleway=track"
                        ),
                        "allow_fix_override": True,
                        "fix": {"-": (["cycleway"])},
                    }
        ...
        if (
            ("bicycle" in keys and "cycleway" in keys)
            or ("bicycle" in keys and "cycleway:left" in keys)
            or ("bicycle" in keys and "cycleway:right" in keys)
        ):
            match = False
            if not match:
                capture_tags = {}
                        ...
                        (
                            mapcss.regexp_test(
                                mapcss._value_capture(
                                    capture_tags, 0, self.re_5b286a0d
                                ),
        ...
        if ("cycleway" in keys) or ("cycleway" in keys and "oneway" in keys):
            ...
            if not match:
                ...
                try:
                    ...
                    ) and (not mapcss._tag_capture(capture_tags, 1, tags, "oneway"))
                except mapcss.RuleAbort:
                    pass
            if not match:
                capture_tags = {}
                try:
                    match = (
                        ...
                        mapcss._tag_capture(capture_tags, 1, tags, "oneway")
                ...
                err.append(
                    {
                        "class": 20301,
                        "subclass": 0,
                        "text": mapcss.tr("Opposite cycleway without oneway"),
                    }
                )
        ...
        if ("cycleway:left" in keys and "oneway" in keys) or (
            "cycleway:right" in keys and "oneway" in keys
        ):
            match = False
            if not match:
                capture_tags = {}
                try:
                    match = (
                        (
                            mapcss.regexp_test(
                                mapcss._value_capture(
                                    capture_tags, 1, self.re_67b51e41
                                ),
                                mapcss._tag_capture(
                ...
                err.append(
                    {
                        ...
                        "subclass": 0,
                        "text": mapcss.tr(
                            "Opposite or opposite lane in the same way of the oneway"
                        ),
                    }
                )
...
class Test(TestPluginMapcss):
    def test(self):
        ...
        self.check_err(
            n.way(
                data,
                {"cycleway": "a", "cycleway:left": "c", "cycleway:right": "b"},
                [0],
            ),
            expected={"class": 40301, "subclass": 0},
        )
        self.check_not_err(
            n.way(data, {"area:highway": "footway", "footway": "sidewalk"}, [0]),
            expected={"class": 20805, "subclass": 0},
        )
        self.check_not_err(
            n.way(
                data,
        ...
        self.check_not_err(
            n.way(data, {"footway": "sidewalk", "highway": "footway"}, [0]),
            expected={"class": 20805, "subclass": 0},
        )
        self.check_err(
            n.way(data, {"footway": "sidewalk", "highway": "path"}, [0]),
            expected={"class": 20805, "subclass": 0},
        )
        self.check_err(
            n.way(data, {"highway": "service", "psv": "no", "service": "psv"}, [0]),
            expected={"class": 40101, "subclass": 0},
        )
        self.check_not_err(
            n.way(data, {"highway": "service", "psv": "yes", "service": "psv"}, [0]),
            expected={"class": 40101, "subclass": 0},
        )
        self.check_not_err(
            n.way(data, {"bicycle": "no", "cycleway:right": "no"}, [0]),
            expected={"class": 30329, "subclass": 0},
        )
        self.check_err(
        ...
        self.check_not_err(
            n.way(data, {"cycleway": "lane", "oneway": "yes"}, [0]),
            expected={"class": 20301, "subclass": 0},
        )
        self.check_not_err(
            n.way(data, {"cycleway": "opposite", "oneway": "yes"}, [0]),
            expected={"class": 20301, "subclass": 0},
        )
        self.check_err(
            n.way(data, {"cycleway": "opposite"}, [0]),
            expected={"class": 20301, "subclass": 0},
        )
        self.check_err(
            n.way(data, {"cycleway:right": "opposite", "oneway": "yes"}, [0]),
            expected={"class": 20302, "subclass": 0},
        )
        self.check_not_err(
            n.way(data, {"cycleway": "opposite", "oneway": "yes"}, [0]),
            expected={"class": 20302, "subclass": 0},
