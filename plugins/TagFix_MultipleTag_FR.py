...
class TagFix_MultipleTag_FR(Plugin):
    ...
    def init(self, logger):
        ...
        self.errors[30321] = self.def_class(
            item=3032,
            level=1,
            tags=["tag", "fix:chair"],
            title=T_("Watch multiple tags"),
        )
        self.errors[50201] = self.def_class(
            item=5020,
            level=2,
            tags=["tag", "name", "fix:chair"],
...
written in a form actually used."""
        ...
        self.errors[30324] = self.def_class(
            item=3032,
            level=2,
            tags=["highway", "maxspeed", "fix:survey"],
            title=T_("incoherent maxspeed"),
        )
        self.errors[30325] = self.def_class(
            item=3032,
            level=2,
            tags=["highway", "ref", "fix:chair"],
            title=T_("Invalid reference"),
        )
        self.errors[206013] = self.def_class(
            item=2060,
            level=3,
            tags=["addr", "fix:chair"],
        ...
        if country and country.startswith("NC"):
            self.Ref = re.compile(
                r"^(RT|RP|VU|VE|VDE|RPN|RM|CR)[-\s]?[0-9]?", re.IGNORECASE
            )
        elif country and (country.startswith("FR-2A") or country.startswith("FR-2B")):
            self.Ref = re.compile(
                r"^([ANDCVR]|RN|RD|VC|CR|CE|EV|V|T)[-\s]?[0-9]?", re.IGNORECASE
            )
        else:  # "FR"
            self.Ref = re.compile(
                r"^([ANDMCVR]|RN|RD|VC|CR|CE|EV|V)[-\s]?[0-9]?", re.IGNORECASE
            )
            ...
            err.append(
                {
                    ...
                    "subclass": 5,
                    "text": T_(
                        "Needs tag amenity=nursery|kindergarten|school besides on school:FR"
                    ),
                }
            )
        if (
            "name" in tags
            and tags.get("amenity") == "school"
            and "school:FR" not in tags
        ):
            canonicalSchool = self.ToolsStripAccents(tags["name"]).lower()
            matches = list(
                filter(lambda kv: kv[0] in canonicalSchool, self.school.keys())
            )
            if len(matches) == 1:
                err.append(
                    {
                        "class": 30321,
                        "subclass": 6,
                        "text": T_("Add school:FR tag"),
                        "fix": {"+": {"school:FR": self.school[matches[0]]}},
                    }
                )
            elif len(matches) > 1:
                err.append(
                    {"class": 30321, "subclass": 6, "text": T_("Add school:FR tag")}
                )

        if (
            "addr:housenumber" not in tags
            and "ref:FR:FANTOIR" in tags
            and len(tags["ref:FR:FANTOIR"]) == 10
        ):
                    ...
                    err.append(
                        {
                            "class": 206013,
                            "subclass": 1,
                            "text": T_("FANTOIR numeric type is for ways"),
                        }
                    )
            # elif fantoir_key == "A":
                ...
                if (
                    ...
                    and tags.get("railway") != "station"
                    and tags.get("leisure") not in ("park", "garden")
                ):
                    err.append(
                        {
                            "class": 206013,
                            "subclass": 1,
                            "text": T_(
                                "FANTOIR B to W type is for locality, hamlet, isolated_dwelling, islet or neighbourhood"
                            ),
                        }
                    )
    ...
    def way(self, data, tags, nds):
            ...
            err.append(
                {
                    ...
                    "fix": {
                        "~": {
                            "name": tags["name"].replace("Chemin Rural dit ", "Chemin ")
                        }
                    },
                }
            )
        ...
        if "highway" in tags:
            if tags["highway"] == "living_street" and tags.get("zone:maxspeed") not in (
                None,
                "FR:20",
            ):
                err.append(
                    {
                        "class": 30324,
                        "subclass": 0,
                        "text": T_("A living_street in France is a Zone 20"),
                    }
                )
            elif (
                tags.get("zone:maxspeed") == "FR:20"
                ...
                err.append(
                    {
                        "class": 30324,
                        "subclass": 1,
                        "text": T_("A Zone 20 in France is a living_street"),
                    }
                )
                    ...
                    err.append(
                        {
                            "class": 30324,
                            "subclass": 3,
                            "text": T_("A Zone 20 is limited to 20 km/h"),
                        }
                    )
                    ...
                    err.append(
                        {
                            "class": 30324,
                            "subclass": 4,
                            "text": T_("A zone 30 is limited to 30 km/h"),
                        }
                    )
        ...
        if (
            ...
            and "ref" in tags
            and not self.Ref.match(tags["ref"])
        ):
...
class Test(TestPluginCommon):
    def test_FR(self):
        ...
        a.init(None)
        for t in [
            {"amenity": "school", "name": "École maternelle Clos Montesquieu"},
            {"amenity": "school", "name": "École élémentaire"},
            {"name": "Chemin Rural dit de la Borne Trouée"},
    ...
    def test_NC(self):
        ...
        for t in [
            ...
            {"ref:FR:FANTOIR": "90123D123D", "highway": "residential"},
            {"highway": "trunk", "ref": "D\u20073"},
            {"school:FR": "maternelle", "ref": "1989898"},
        ]:
            self.check_err(a.way(None, t, None), t)

        for t in [
            {"highway": "trunk", "ref": "RPN 73"},
            {"highway": "primary", "zone:maxspeed": "FR:50"},
            {"highway": "primary", "zone:maxspeed": "FR:30", "maxspeed": "30"},
            {"highway": "living_street"},
            {"highway": "living_street", "zone:maxspeed": "FR:20", "maxspeed": "20"},
