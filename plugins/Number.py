...
class Number(Plugin):
    ...
    def init(self, logger):
        ...
        self.errors[3091] = self.def_class(
            item=3091,
            level=2,
...
(with valid units if required)."""
        ...
        self.errors[3092] = self.def_class(
            item=3091,
            level=2,
            tags=["value", "fix:chair"],
...
be used if the value is valid."""
        ...
        self.errors[3093] = self.def_class(
            item=3091,
            level=2,
            tags=["value", "fix:chair"],
            title=T_("Suspicious value"),
            detail=T_("""The tag expects a positive, round number without unit."""),
            fix=T_(
                """Check that the value is accurate. Consider whether another tag should
be used if the value is valid."""
            ),
        )
        self.errors[3094] = self.def_class(
            item=3091,
            level=2,
            tags=["value", "fix:chair"],
            title=T_("Unknown unit"),
            detail=T_("""The tag uses an unexpected unit."""),
            fix=T_(
        ...
        self.tag_number_integer = [
            "admin_level",
            "capital",
            "heritage",
            "population",
            "step_count",
        ]  # Only positive integers (no units) allowed
        tag_number_directional = [
            "maxaxleload",
            "maxheight",
            "maxheight:physical",
            "maxlength",
        ...
        for i in ["", ":forward", ":backward"]:
            self.tag_number.extend(
                list(map(lambda tag: tag + i, tag_number_directional))
            )
        ...
        self.units = [
            ...
            "km",
            "mi",
            "nmi",  # distance excluding feet'inch"
            "km/h",
            "mph",
            "knots",  # speed
            "t",
        ...
        self.Number = re.compile(
            '^((?:-?[0-9]+(?:[.][0-9]+)?)|(?:[.][0-9]+))(?: ?([a-zA-Z23/]{1,5})|\'(?:[0-9]*(?:[.][0-9]+)?")?|")?$'
        )
        self.MaxspeedExtraValue = [
            "none",
            "default",
            "signals",
            "national",
        ...
        self.MaxspeedClassValue = re.compile("^[A-Z]*:")
        self.MaxheightExtraValue = [
            "default",
            "below_default",
            "no_indications",
            "no_sign",
    ...
    def node(self, data, tags):
        for i in self.tag_number:
            if i in tags:
                ...
                if (
                    not m
                    and not (i == "width" and tags[i] == "narrow")
                    and not (i == "capital" and tags[i] == "yes")
                    and not (i == "heritage" and tags[i] == "yes")
                    and not (
                        ("maxspeed" in i or "minspeed" in i)
                        and (
                            tags[i] in self.MaxspeedExtraValue
                            or self.MaxspeedClassValue.match(tags[i])
                            or (
                                tags[i] == "implicit"
                                and ("traffic_sign" in tags)
                                and "maxspeed" in tags["traffic_sign"].split(";")
                            )
                        )
                    )
                    and not (i == "maxheight" and tags[i] in self.MaxheightExtraValue)
                ):
                    return {
                        "class": 3091,
                        "subclass": 1,
                        "text": T_("Concerns tag: `{0}`", "=".join([i, tags[i]])),
                    }
                if not m:
                    continue
                ...
                if (
                    i in self.tag_number_integer
                    and str(int(abs(float(m.group(1))))) != tags[i]
                ):
                    ...
                    "maxspeed" in i and float(m.group(1)) < 5 and "waterway" not in tags
                ):
                    return {
                        "class": 3092,
                        "subclass": 3,
                        "text": T_("`{0}` is really slow", "maxspeed=" + m.group(1)),
                    }
                elif (
                    i == "width" and float(m.group(1)) <= 0 and "highway" in tags
                ):  # seems to be an old iD bug
                    return {
                        "class": 3092,
                        "subclass": 5,
                        "text": T_("Concerns tag: `{0}`", "=".join([i, tags[i]])),
