...
class Josm_highway(PluginMapCSS):
    ...
    def init(self, logger):
        ...
        self.errors[9004009] = self.def_class(
            item=9004, level=3, tags=["tag", "highway"], title=mapcss.tr("missing tag")
        )
        self.errors[9004010] = self.def_class(
            ...
            tags=["tag", "highway"],
            title=mapcss.tr(
                "{0} on a node", mapcss._tag_uncapture(capture_tags, "{0.tag}")
            ),
        )
        ...
        self.errors[9004012] = self.def_class(
            ...
            level=3,
            tags=["tag", "highway"],
            title=mapcss.tr("questionable value (ending with a number)"),
        )
        ...
        self.errors[9004014] = self.def_class(
            item=9004,
            level=3,
            tags=["tag", "highway"],
            title=mapcss.tr(
        ...
        self.re_23c50386 = re.compile(
            r"^(|none|((sharp_|slight_|merge_to_|slide_)?(left|right)|reverse|through)(;((sharp_|slight_|merge_to_|slide_)?(left|right)|reverse|through))*)(\|(|none|((sharp_|slight_|merge_to_|slide_)?(left|right)|reverse|through)(;((sharp_|slight_|merge_to_|slide_)?(left|right)|reverse|through))*))*$"
        )
        self.re_3092b7ac = re.compile(r"^.*_link$")
        self.re_33052a50 = re.compile(
            r"^(none|((sharp_|slight_|merge_to_|slide_)?(left|right)|reverse|through)(;((sharp_|slight_|merge_to_|slide_)?(left|right)|reverse|through))*)$"
        )
        ...
        self.re_447f4d65 = re.compile(
            r"motorway|trunk|primary|secondary|tertiary|unclassified|residential|service|living_street|pedestrian|track|path|footway|cycleway|busway|bus_guideway|bridleway"
        )
        ...
        self.re_5757d731 = re.compile(
            r"^((motorway|trunk|primary|secondary|tertiary)(_link)?|residential|unclassified)$"
        )
    ...
    def node(self, data, tags):
        ...
        keys = tags.keys()
        err = []
        set_fixable_footway = set_link_road = set_major_road = set_minor_road = (
            set_not_fixable_footway
        ) = False
                        ...
                        (
                            mapcss.regexp_test(
                                mapcss._value_capture(
                                    capture_tags, 0, self.re_447f4d65
                                ),
                ...
                err.append(
                    {
                        "class": 9004008,
                        "subclass": 224371448,
                        "text": mapcss.tr("wrong highway tag on a node"),
                    }
                )
                ...
                err.append(
                    {
                        ...
                        "allow_fix_override": True,
                        "fix": {"+": dict([["railway", "level_crossing"]])},
                    }
                )
            ...
            if not match:
                ...
                try:
                    match = mapcss.regexp_test(
                        mapcss._value_capture(capture_tags, 0, self.re_55b03910),
                ...
                err.append(
                    {
                        "class": 9004012,
                        "subclass": 291757414,
                        "text": mapcss.tr("{0} with trailing number", "paving_stones"),
                    }
                )
    ...
    def way(self, data, tags, nds):
            ...
            if not match:
                ...
                try:
                    match = (
                        ...
                        mapcss.regexp_test(
                            mapcss._value_capture(capture_tags, 1, self.re_4186cb68),
        ...
        if "highway" in keys:
            ...
            if not match:
                ...
                try:
                    match = mapcss._tag_capture(
                        capture_tags, 0, tags, "highway"
                    ) == mapcss._value_capture(capture_tags, 0, "road")
                except mapcss.RuleAbort:
                    pass
            if match:
        ...
        if (
            ("bicycle" in keys and "highway" in keys)
            or ("cycleway" in keys and "highway" in keys)
            or ("foot" in keys and "highway" in keys)
            or ("highway" in keys and "maxspeed" in keys)
        ):
            match = False
            if not match:
                capture_tags = {}
        ...
        if "footway" in keys:
            ...
            if not match:
                ...
                try:
                    ...
                    ) and (mapcss._tag_capture(capture_tags, 1, tags, self.re_4dcdb354))
                except mapcss.RuleAbort:
                    pass
            if not match:
                capture_tags = {}
            ...
            if not match:
                ...
                try:
                    match = (
                        mapcss._tag_capture(capture_tags, 0, tags, "footway")
                        == mapcss._value_capture(capture_tags, 0, "separate")
                    ) and (mapcss._tag_capture(capture_tags, 1, tags, self.re_4dcdb354))
                except mapcss.RuleAbort:
                    pass
            if not match:
        ...
        if "footway" in keys:
            ...
            if not match:
                capture_tags = {}
                try:
                    match = (not set_not_fixable_footway) and (
                        mapcss._tag_capture(capture_tags, 0, tags, "footway")
                        == mapcss._value_capture(capture_tags, 0, "left")
                    )
                except mapcss.RuleAbort:
                ...
                err.append(
                    {
                        ...
                        "allow_fix_override": True,
                        "fix": {
                            "+": dict([["sidewalk", mapcss.tag(tags, "footway")]]),
                            "-": (["footway"]),
                        },
                    }
                )
        ...
        if (
            ("turn" in keys)
            or ("turn:backward" in keys)
            or ("turn:both_ways" in keys)
            or ("turn:both_ways:backward" in keys)
            or ("turn:both_ways:forward" in keys)
            or ("turn:forward" in keys)
            or ("turn:lanes" in keys)
            or ("turn:lanes:backward" in keys)
            or ("turn:lanes:both_ways" in keys)
            or ("turn:lanes:both_ways:backward" in keys)
            or ("turn:lanes:both_ways:forward" in keys)
            or ("turn:lanes:forward" in keys)
        ):
            match = False
            if not match:
                capture_tags = {}
                try:
                    match = (mapcss._tag_capture(capture_tags, 0, tags, "turn")) and (
                        not mapcss.regexp_test(
                            mapcss._value_const_capture(
                                capture_tags,
                                1,
                                self.re_33052a50,
                                "^(none|((sharp_|slight_|merge_to_|slide_)?(left|right)|reverse|through)(;((sharp_|slight_|merge_to_|slide_)?(left|right)|reverse|through))*)$",
                            ),
                            mapcss._tag_capture(capture_tags, 1, tags, "turn"),
                        )
                    )
                except mapcss.RuleAbort:
                    pass
            if not match:
                ...
                try:
                    match = (
                        mapcss._tag_capture(capture_tags, 0, tags, "turn:forward")
                    ) and (
                        not mapcss.regexp_test(
                            mapcss._value_const_capture(
                                capture_tags,
                            ...
                            mapcss._tag_capture(capture_tags, 1, tags, "turn:forward"),
                        )
                    )
                except mapcss.RuleAbort:
                    pass
            if not match:
                ...
                try:
                    match = (
                        mapcss._tag_capture(capture_tags, 0, tags, "turn:backward")
                    ) and (
                        not mapcss.regexp_test(
                            mapcss._value_const_capture(
                                capture_tags,
                            ...
                            mapcss._tag_capture(capture_tags, 1, tags, "turn:backward"),
                        )
                    )
                except mapcss.RuleAbort:
                    pass
            if not match:
                ...
                try:
                    match = (
                        mapcss._tag_capture(capture_tags, 0, tags, "turn:both_ways")
                    ) and (
                        not mapcss.regexp_test(
                            mapcss._value_const_capture(
                                capture_tags,
                            ...
                            mapcss._tag_capture(
                                capture_tags, 1, tags, "turn:both_ways"
                            ),
                        )
                    )
                except mapcss.RuleAbort:
            ...
            if not match:
                ...
                try:
                    match = (
                        mapcss._tag_capture(
                            capture_tags, 0, tags, "turn:both_ways:forward"
                        )
                    ) and (
                        not mapcss.regexp_test(
                            mapcss._value_const_capture(
                            ...
                            mapcss._tag_capture(
                                capture_tags, 1, tags, "turn:both_ways:forward"
                            ),
                        )
                    )
                except mapcss.RuleAbort:
            ...
            if not match:
                ...
                try:
                    match = (
                        mapcss._tag_capture(
                            capture_tags, 0, tags, "turn:both_ways:backward"
                        )
                    ) and (
                        not mapcss.regexp_test(
                            mapcss._value_const_capture(
                            ...
                            mapcss._tag_capture(
                                capture_tags, 1, tags, "turn:both_ways:backward"
                            ),
                        )
                    )
                except mapcss.RuleAbort:
            ...
            if not match:
                ...
                try:
                    match = (
                        mapcss._tag_capture(capture_tags, 0, tags, "turn:lanes")
                    ) and (
                        not mapcss.regexp_test(
                            mapcss._value_const_capture(
                                capture_tags,
                                1,
                                self.re_23c50386,
                                "^(|none|((sharp_|slight_|merge_to_|slide_)?(left|right)|reverse|through)(;((sharp_|slight_|merge_to_|slide_)?(left|right)|reverse|through))*)(\|(|none|((sharp_|slight_|merge_to_|slide_)?(left|right)|reverse|through)(;((sharp_|slight_|merge_to_|slide_)?(left|right)|reverse|through))*))*$",
                            ),
                            mapcss._tag_capture(capture_tags, 1, tags, "turn:lanes"),
                        )
                    )
                except mapcss.RuleAbort:
                    pass
            if not match:
                ...
                try:
                    match = (
                        mapcss._tag_capture(capture_tags, 0, tags, "turn:lanes:forward")
                    ) and (
                        not mapcss.regexp_test(
                            mapcss._value_const_capture(
                                capture_tags,
                            ...
                            mapcss._tag_capture(
                                capture_tags, 1, tags, "turn:lanes:forward"
                            ),
                        )
                    )
                except mapcss.RuleAbort:
            ...
            if not match:
                ...
                try:
                    match = (
                        mapcss._tag_capture(
                            capture_tags, 0, tags, "turn:lanes:backward"
                        )
                    ) and (
                        not mapcss.regexp_test(
                            mapcss._value_const_capture(
                            ...
                            mapcss._tag_capture(
                                capture_tags, 1, tags, "turn:lanes:backward"
                            ),
                        )
                    )
                except mapcss.RuleAbort:
            ...
            if not match:
                ...
                try:
                    match = (
                        mapcss._tag_capture(
                            capture_tags, 0, tags, "turn:lanes:both_ways"
                        )
                    ) and (
                        not mapcss.regexp_test(
                            mapcss._value_const_capture(
                            ...
                            mapcss._tag_capture(
                                capture_tags, 1, tags, "turn:lanes:both_ways"
                            ),
                        )
                    )
                except mapcss.RuleAbort:
            ...
            if not match:
                ...
                try:
                    match = (
                        mapcss._tag_capture(
                            capture_tags, 0, tags, "turn:lanes:both_ways:forward"
                        )
                    ) and (
                        not mapcss.regexp_test(
                            mapcss._value_const_capture(
                            ...
                            mapcss._tag_capture(
                                capture_tags, 1, tags, "turn:lanes:both_ways:forward"
                            ),
                        )
                    )
                except mapcss.RuleAbort:
            ...
            if not match:
                ...
                try:
                    match = (
                        mapcss._tag_capture(
                            capture_tags, 0, tags, "turn:lanes:both_ways:backward"
                        )
                    ) and (
                        not mapcss.regexp_test(
                            mapcss._value_const_capture(
                            ...
                            mapcss._tag_capture(
                                capture_tags, 1, tags, "turn:lanes:both_ways:backward"
                            ),
                        )
                    )
                except mapcss.RuleAbort:
        ...
        if "area" in keys and "highway" in keys:
            ...
            if not match:
                ...
                try:
                    match = (
                        mapcss.regexp_test(
                            mapcss._value_capture(capture_tags, 0, self.re_5757d731),
                            mapcss._tag_capture(capture_tags, 0, tags, "highway"),
                        )
                    ) and (
                ...
                err.append(
                    {
                        ...
                        "subclass": 610375152,
                        "text": mapcss.tr(
                            "Area with {0} above {1} is invalid",
                            "highway=*",
                            "highway=service",
                        ),
                    }
...
class Test(TestPluginMapcss):
    def test(self):
        ...
        self.check_not_err(
            ...
            expected={"class": 9004008, "subclass": 224371448},
        )
        self.check_not_err(
            n.node(data, {"highway": "crossing"}),
            expected={"class": 9004008, "subclass": 224371448},
        )
        self.check_not_err(
            n.node(data, {"highway": "emergency_access_point"}),
            expected={"class": 9004008, "subclass": 224371448},
        )
        self.check_not_err(
            n.node(data, {"highway": "give_way"}),
            expected={"class": 9004008, "subclass": 224371448},
        )
        self.check_not_err(
            n.node(data, {"highway": "mini_roundabout"}),
            expected={"class": 9004008, "subclass": 224371448},
        )
        self.check_not_err(
            n.node(data, {"highway": "motorway_junction"}),
            expected={"class": 9004008, "subclass": 224371448},
        )
        self.check_not_err(
            n.node(data, {"highway": "passing_place"}),
            expected={"class": 9004008, "subclass": 224371448},
        )
        self.check_err(
            n.node(data, {"highway": "primary"}),
            expected={"class": 9004008, "subclass": 224371448},
        )
        self.check_err(
            n.node(data, {"highway": "primary_link"}),
            expected={"class": 9004008, "subclass": 224371448},
        )
        self.check_not_err(
            n.node(data, {"highway": "rest_area"}),
            expected={"class": 9004008, "subclass": 224371448},
        )
        self.check_not_err(
            n.node(data, {"highway": "services"}),
            expected={"class": 9004008, "subclass": 224371448},
        )
        self.check_not_err(
            n.node(data, {"highway": "speed_camera"}),
            expected={"class": 9004008, "subclass": 224371448},
        )
        self.check_not_err(
            n.node(data, {"highway": "stop"}),
            expected={"class": 9004008, "subclass": 224371448},
        )
        self.check_not_err(
            n.node(data, {"highway": "street_lamp"}),
            expected={"class": 9004008, "subclass": 224371448},
        )
        self.check_not_err(
            n.node(data, {"highway": "traffic_calming"}),
            expected={"class": 9004008, "subclass": 224371448},
        )
        self.check_not_err(
            n.node(data, {"highway": "traffic_signals"}),
            expected={"class": 9004008, "subclass": 224371448},
        )
        self.check_not_err(
            n.node(data, {"highway": "turning_circle"}),
            expected={"class": 9004008, "subclass": 224371448},
        )
        self.check_err(
            n.way(data, {"highway": "unclassified", "name": "Bou Blvd."}, [0]),
            expected={"class": 9004001, "subclass": 1120623403},
        )
        self.check_err(
            n.way(data, {"highway": "unclassified", "name": "Bou blvd."}, [0]),
            expected={"class": 9004001, "subclass": 1120623403},
        ...
        self.check_err(
            n.way(data, {"highway": "unclassified", "name": "Foo Ave"}, [0]),
            expected={"class": 9004001, "subclass": 1120623403},
        )
        self.check_err(
            n.way(data, {"highway": "unclassified", "name": "Foo Ave."}, [0]),
        ...
        self.check_err(
            n.way(data, {"highway": "crossing"}, [0]),
            expected={"class": 9004002, "subclass": 1549110307},
        )
        self.check_not_err(
            n.way(data, {"highway": "residential"}, [0]),
            expected={"class": 9004004, "subclass": 1729022478},
        )
        self.check_err(
            n.way(data, {"highway": "road"}, [0]),
            expected={"class": 9004004, "subclass": 1729022478},
        )
        self.check_not_err(
            n.way(data, {"bicycle": "yes", "highway": "cycleway"}, [0]),
            expected={"class": 9004013, "subclass": 469607562},
        )
        self.check_err(
            n.way(data, {"cycleway": "lane", "highway": "cycleway"}, [0]),
            expected={"class": 9004013, "subclass": 469607562},
        )
        self.check_not_err(
            n.way(data, {"highway": "cycleway"}, [0]),
            expected={"class": 9004013, "subclass": 469607562},
        )
        self.check_not_err(
            n.way(data, {"highway": "cycleway"}, [0]),
        ...
        self.check_not_err(
            n.way(data, {"foot": "yes", "highway": "footway"}, [0]),
            expected={"class": 9004013, "subclass": 469607562},
        )
        self.check_err(
            n.way(data, {"highway": "footway", "maxspeed": "20"}, [0]),
            expected={"class": 9004013, "subclass": 469607562},
        )
        self.check_not_err(
            n.way(data, {"highway": "footway"}, [0]),
            expected={"class": 9004013, "subclass": 469607562},
        )
        self.check_not_err(
            n.way(data, {"highway": "footway"}, [0]),
        ...
        self.check_not_err(
            n.way(data, {"cycleway": "lane", "highway": "residential"}, [0]),
            expected={"class": 9004013, "subclass": 469607562},
        )
        self.check_not_err(
            n.way(data, {"highway": "residential", "maxspeed": "20"}, [0]),
            expected={"class": 9004013, "subclass": 469607562},
        )
        self.check_err(
            n.way(data, {"footway": "both", "footway:surface": "asphalt"}, [0]),
            expected={"class": 9004006, "subclass": 1255595246},
        )
        self.check_err(
            n.way(data, {"footway": "separate", "footway:surface": "asphalt"}, [0]),
            expected={"class": 9004006, "subclass": 1255595246},
        )
        self.check_not_err(
            n.way(data, {"footway": "no", "footway:surface": "asphalt"}, [0]),
            expected={"class": 9004006, "subclass": 2016837729},
        )
        self.check_err(
            n.way(data, {"footway": "none", "footway:surface": "asphalt"}, [0]),
            expected={"class": 9004006, "subclass": 2016837729},
        )
        self.check_not_err(
            n.way(
                data,
                {
                    "turn:lanes:forward": "sharp_left;left||left;through;slight_right|slight_right;right|"
                },
                [0],
            ),
            expected={"class": 9004014, "subclass": 1634496690},
        )
        self.check_err(
            n.way(data, {"turn:lanes:forward": "slight_reverse|right"}, [0]),
            expected={"class": 9004014, "subclass": 1634496690},
        )
        self.check_err(
            n.way(data, {"turn:lanes:forward": "straight|right"}, [0]),
            expected={"class": 9004014, "subclass": 1634496690},
        )
        self.check_err(
            n.way(data, {"turn:lanes": "left;none|right"}, [0]),
            expected={"class": 9004014, "subclass": 1634496690},
        )
        self.check_err(
            n.way(data, {"turn": "slight_reverse"}, [0]),
            expected={"class": 9004014, "subclass": 1634496690},
        )
        self.check_err(
            n.way(data, {"turn": "straight"}, [0]),
            expected={"class": 9004014, "subclass": 1634496690},
        )
        self.check_not_err(
            n.way(data, {"turn": "through;right"}, [0]),
            expected={"class": 9004014, "subclass": 1634496690},
        )
        self.check_err(
            n.way(data, {"turn": "through|right"}, [0]),
        ...
        self.check_not_err(
            n.way(data, {"area": "yes", "highway": "service"}, [0]),
            expected={"class": 9004011, "subclass": 610375152},
        )
        self.check_err(
            n.way(data, {"area": "yes", "highway": "trunk"}, [0]),
            expected={"class": 9004011, "subclass": 610375152},
        )
        self.check_not_err(
            n.way(data, {"highway": "trunk"}, [0]),
            expected={"class": 9004011, "subclass": 610375152},
        )
        self.check_err(
            n.relation(data, {"highway": "trunk", "type": "multipolygon"}, []),
            expected={"class": 9004011, "subclass": 2126629809},
        )
