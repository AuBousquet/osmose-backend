...
class TagFix_Destination(PluginMapCSS):
    ...
    def init(self, logger):
        ...
        self.errors[316010] = self.def_class(
            item=3160,
            level=3,
            tags=mapcss.list_("tag", "highway", "waterway"),
            title=mapcss.tr(
                "Pipe characters should not be used in destination tag, only in destination:lanes"
            ),
        )
    ...
    def way(self, data, tags, nds):
        ...
        if (
            ("destination" in keys and "highway" in keys)
            or ("destination" in keys and "waterway" in keys)
            or ("highway" in keys)
            or ("waterway" in keys)
        ):
            match = False
            if not match:
                ...
                try:
                    match = (
                        ...
                        and (mapcss._tag_capture(capture_tags, 1, tags, "destination"))
                        and (
                            mapcss.string_contains(
                                mapcss._tag_capture(
                                    capture_tags, 2, tags, "destination"
                                ),
            ...
            if not match:
                ...
                try:
                    match = (
                        (mapcss._tag_capture(capture_tags, 0, tags, "highway"))
                        and (
                            mapcss._tag_capture(capture_tags, 1, tags, self.re_53d7e349)
                        )
                        and (
...
class Test(TestPluginMapcss):
    def test(self):
        ...
        self.check_not_err(
            n.way(data, {"destination:colour": "Red", "highway": "primary"}, [0]),
            expected={"class": 316010, "subclass": 0},
        )
        self.check_err(
            n.way(
                data, {"destination:colour": "Red|Yellow", "highway": "primary"}, [0]
            ),
            expected={"class": 316010, "subclass": 0},
        )
        self.check_not_err(
            n.way(
                data,
                {"destination:lanes:backward": "A8|Centre|Plage", "highway": "primary"},
                [0],
            ),
            expected={"class": 316010, "subclass": 0},
        )
        self.check_not_err(
            n.way(
                data,
                {"destination:lanes": "A8|Centre|Plage", "highway": "primary"},
                [0],
            ),
            expected={"class": 316010, "subclass": 0},
        )
        self.check_not_err(
            n.way(
                data, {"destination:ref:lanes": "A8|A10|A23", "highway": "primary"}, [0]
            ),
            expected={"class": 316010, "subclass": 0},
        )
        self.check_not_err(
            n.way(data, {"destination": "A8", "highway": "primary"}, [0]),
            expected={"class": 316010, "subclass": 0},
        )
        self.check_err(
            n.way(data, {"destination": "A8|Centre|Plage", "highway": "primary"}, [0]),
            expected={"class": 316010, "subclass": 0},
        )
        self.check_not_err(
            n.way(
                data,
                {"destination:ref:lanes:backward": "B 3|B 3", "highway": "tertiary"},
                [0],
            ),
            expected={"class": 316010, "subclass": 0},
        )
        self.check_not_err(
            n.way(
                data, {"destination:ref:to:lanes": "A 7|", "highway": "tertiary"}, [0]
            ),
            expected={"class": 316010, "subclass": 0},
        )
        self.check_not_err(
            n.way(data, {"destination": "East", "waterway": "river"}, [0]),
            expected={"class": 316010, "subclass": 0},
        )
        self.check_err(
            n.way(data, {"destination": "East|West", "waterway": "river"}, [0]),
            expected={"class": 316010, "subclass": 0},
        )
