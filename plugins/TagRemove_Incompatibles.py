...
class TagRemove_Incompatibles(Plugin):
    ...
    def init(self, logger):
        ...
        self.errors[900] = self.def_class(
            item=4030,
            level=1,
            tags=["tag", "fix:chair"],
            title=T_("Tag conflict"),
            detail=T_("""The object contains two incompatible tags."""),
            trap=T_("""Sometimes the object needs both tags."""),
        )
            ...
            if len(conflict) > 1:
                ...
                if len(conflict) > 1:
                    return {
                        ...
                        "subclass": 1,
                        "text": T_(
                            "Conflict between tags: {0}", (", ".join(sorted(conflict)))
                        ),
                    }
