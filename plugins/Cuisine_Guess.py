...
class Cuisine_Guess(Plugin):
    ...
    def node(self, data, tags):
        ...
        cuisine_guess = self.taster.guess(
        ...
        if cuisine_guess:
            ...
            if "cuisine" in tags:
                ...
                if max_score < 0.9:
                    ...
                    tasty_cuisines = cuisines.intersection(
                        set(map(lambda c: c[0], cuisine_guess.items()))
                    )

            if not tasty_cuisines:
                return {
                    "class": 1 if "cuisine" in tags else 2,
                    "text": T_(
                        "Guess with probability: {0}",
                        ", ".join(
                            map(
                                lambda cs: "{0} ({1}%)".format(
                                    cs[0], round(cs[1] * 100)
                                ),
                                cuisine_guess.items(),
                            )
                    ...
                    "fix": [
                        {"~": {"cuisine": cuisine[0]}}
                        for cuisine in cuisine_guess.items()
                    ],
                }
