...
class Name_MisspelledWordByRegex_Lang_fa(PluginMapCSS):
    ...
    def init(self, logger):
        ...
        self.errors[50109001] = self.def_class(
            item=5010,
            level=2,
            tags=mapcss.list_("name", "fix:chair"),
            title=mapcss.tr("Arabic letter detected in Farsi name"),
        )
        ...
        if "name" in keys:
            ...
            if not match:
                ...
                try:
                    match = (
                        mapcss.regexp_test(
                            mapcss._value_capture(capture_tags, 0, self.re_5eeade1c),
                            mapcss._tag_capture(capture_tags, 0, tags, "name"),
                        )
                    ) and (mapcss.language(self.father.config.options, "fa"))
            ...
            if not match:
                ...
                try:
                    match = mapcss.regexp_test(
                        mapcss._value_capture(capture_tags, 0, self.re_5eeade1c),
                        mapcss._tag_capture(capture_tags, 0, tags, "name:fa"),
                    )
                except mapcss.RuleAbort:
                    pass
            if match:
        ...
        if "name" in keys:
            ...
            if not match:
                ...
                try:
                    match = (
                        mapcss.regexp_test(
                            mapcss._value_capture(capture_tags, 0, self.re_4234bf3b),
                            mapcss._tag_capture(capture_tags, 0, tags, "name"),
                        )
                    ) and (mapcss.language(self.father.config.options, "fa"))
                except mapcss.RuleAbort:
...
class Test(TestPluginMapcss):
    def test(self):
        ...
        with with_options(n, {"language": "fa"}):
            self.check_err(
                n.node(data, {"name": "روابط عمومي مجتمع مس شهربابك"}),
                expected={"class": 50109001, "subclass": 10},
            )
        self.check_not_err(
            n.node(data, {"name": "روابط عمومي مجتمع مس شهربابك"}),
            expected={"class": 50109001, "subclass": 10},
        ...
        self.check_err(
            n.node(data, {"name:fa": "روابط عمومي مجتمع مس شهربابك"}),
            expected={"class": 50109001, "subclass": 11},
        )
        with with_options(n, {"language": "fa"}):
            self.check_err(
                n.node(data, {"name": "روابط عمومي مجتمع مس شهربابك"}),
                expected={"class": 50109001, "subclass": 20},
            )
        self.check_not_err(
            n.node(data, {"name": "روابط عمومي مجتمع مس شهربابك"}),
            expected={"class": 50109001, "subclass": 20},
        ...
        self.check_err(
            n.node(data, {"name:fa": "روابط عمومي مجتمع مس شهربابك"}),
            expected={"class": 50109001, "subclass": 21},
        )
