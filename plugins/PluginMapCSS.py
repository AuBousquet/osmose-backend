    ...
    def def_class(self, **kwargs):
        if "source" not in kwargs:
            ...
            if hasattr(self, "MAPCSS_URL") and self.MAPCSS_URL:
                ...
                caller = getframeinfo(stack()[1][0]).filename.replace(
                    ".py", ".validator.mapcss"
                )
                kwargs["source"] = "{0}/plugins/{1}{2}".format(
                    config
                    and hasattr(config, "source_url")
                    and config.source_url
                    or None,
    ...
    def check_err(self, errors, **kwargs):
        ...
        for error in errors:
            if "text" in error and "en" in error["text"]:
                assert not any(
                    c in disallowed_str_in_text for c in error["text"]["en"]
                ), (
                    "Encountered any of '"
                    + "".join(disallowed_str_in_text)
                    + "' in text: "
                    + error["text"]["en"]
                )
