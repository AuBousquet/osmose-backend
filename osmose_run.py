...
def get_version():
    ...
    if not version:
        try:
            version = (
                subprocess.check_output("git describe --long --dirty".split())
                .decode("utf-8")
                .strip()
            )
        except:
    ...
    if options.minimum_free_space:
        for i in dirs:
            ...
            if free_space < needed_space:
                err_msg = "directory '%s' has %.2f GB free instead of %.2f GB " % (
                    i,
...
def execc(conf, logger, analysers, options, osmosis_manager):
    ...
    if not options.skip_init:
        if (
            options.change
            and osmosis_manager.check_change(conf)
            and not options.change_init
        ):
                ...
                try:
                    if options.pbf_update_tool == "osmosis":
                        if osmosis_manager.check_osmosis_diff(conf):
                            (status, xml_change) = osmosis_manager.run_osmosis_diff(
                                conf
                            )
                    else:
            ...
            if not newer:
                logger.log(logger.log_av_r + "downloading" + logger.log_ap)
                newer = download.dl(
                    conf.download["url"],
                    conf.download["dst"],
                    logger.sub(),
                ...
                if newer and options.diff:
                    ...
                    if (
                        "/minute/" in conf.download["diff"]
                        or "/hour/" in conf.download["diff"]
                    ):
        ...
        try:
            ...
            for name, obj in inspect.getmembers(analysers[analyser]):
                if (
                    ...
                    resume = options.resume or (
                        options.resume_analyser and analyser in options.resume_analyser
                    )
                    ...
                    if not options.skip_analyser:
                        with obj(analyser_conf, logger.sub()) as analyser_obj:
                            ...
                            if not options.skip_frontend_check:
                                url = (
                                    modules.config.url_frontend_update
                                    + "/../../control/status/%s/%s?%s"
                                    % (
                                        conf.country,
                                        analyser_name,
                                ...
                                if not resp.ok:
                                    logger.sub().err(
                                        "Fails to get status from frontend: {0}".format(
                                            resp.status_code
                                        )
                                    )
                                else:
                                    try:
                                        status = resp.json()
                                        remote_timestamp = (
                                            dateutil.parser.parse(status["timestamp"])
                                            if status
                                            else None
                                        )
                            ...
                            if (
                                analyser_obj.timestamp()
                                and remote_timestamp
                                and analyser_obj.timestamp() <= remote_timestamp
                                and analyser_obj.analyser_version()
                            ...
                            while not update_finished and nb_iter < 3:
                                ...
                                try:
                                    ...
                                    r = requests.post(
                                        ...
                                        files={
                                            "content": open(
                                                analyser_conf.error_file.dst, "rb"
                                            )
                                        },
                                    )
                                    r.raise_for_status()
...
def clean(conf, logger, options, osmosis_manager):
    logger.log(logger.log_av_r + "cleaning : " + logger.log_ap)

    if options.change:
        pass
    else:
        if osmosis_manager:
            osmosis_manager.clean_database(
                conf, options.no_clean or not conf.clean_at_end
            )
    ...
    try:
        ...
        if hasattr(conf, "db_base") and conf.db_base:
            try:
                ...
                traceback.print_exc()
                logger.err("error in database initialisation")
                return 0x10
...
def main(options):
    ...
    if os.getenv("SENTRY_DSN"):
        sentry_sdk.init(
            dsn=os.getenv("SENTRY_DSN"), traces_sample_rate=1.0, release=get_version()
        )
    ...
    if options.analyser:
        options.analyser = list(
            map(
                lambda analyser: (
                    analyser[len("analyser_") :]
                    if analyser.startswith("analyser_")
                    else analyser
                ),
                options.analyser,
            )
    ...
    logger.log(logger.log_av_green + "end of analyses" + logger.log_ap)
    return err_code
