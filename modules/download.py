...
from modules import OsmoseLog, config, downloader
...
def dl(url, local, logger=OsmoseLog.logger(), min_file_size=10 * 1024):
    ...
    if answer.status_code == 304:
        logger.log("not newer")
        return False
    if not answer.ok:
        logger.log("got error %d" % answer.status_code)
    ...
    if not answer.headers.get("Content-Encoding") and file_size != os.path.getsize(
        file_dl
    ):
        logger.log(
            "error: Download file (%d) not of the expected size (%d) for %s"
    ...
    # convert pbf to osm
    if convert_pbf:
        logger.log("osmconvert")
        subprocess.check_output(
            "{} {} > {}".format(config.bin_osmconvert, file_dl, local), shell=True
        )
        os.remove(file_dl)
