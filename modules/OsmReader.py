...
def open(
    osm: str, logger=dummylog(), state_file: Optional[OsmState] = None
) -> OsmReader:
    if osm.endswith(".pbf"):
        from .OsmPbf import OsmPbfReader

        return OsmPbfReader(osm, logger, state_file)
    elif osm.endswith(".osc") or osm.endswith(".osc.gz") or osm.endswith(".osc.bz2"):
        from .OsmSax import OscSaxReader

        return OscSaxReader(osm, logger, state_file)
    elif osm.endswith(".osm") or osm.endswith(".osm.gz") or osm.endswith(".osm.bz2"):
