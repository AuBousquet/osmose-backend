...
class OsmOsisManager:
    ...
    def __init__(
        ...
        self.db_string += "options=--search_path=%s,public " % (
            self.conf.db_schema_path or self.db_schema
        )
    ...
    def osmosis(self, schema_path=True):
        if not hasattr(self, "_osmosis"):
            if schema_path:
                self._osmosis = OsmOsis(
                    self.db_string, self.conf.db_schema_path or self.db_schema
                )
            else:
    ...
    def set_pgsql_schema(self, reset=False):
        ...
        self.psql_c(
            ...
            % (self.db_user, self.db_base, self.conf.db_schema_path or self.db_schema)
        )
    ...
    def check_database(self):
        ...
        for extension in ["hstore"] + self.conf.db_extension_check:
            ...
            if giscurs.rowcount != 1 or giscurs.fetchone()[0] is None:
                self.logger.err("missing extension: " + extension)
                return False
    ...
    def init_database(self, conf, options):
        ...
        # schema
        self.logger.log(
            self.logger.log_av_r + "import osmosis schema" + self.logger.log_ap
        )
        ...
        self.logger.log(
            self.logger.log_av_r + "import osmosis data" + self.logger.log_ap
        )
        ...
        try:
            ...
            bg_proc.append(
                (self.logger.execute_err(cmd, background=parallel), "osmosis")
            )
            if parallel:
            ...
            for script in conf.osmosis_import_scripts:
                bg_proc.append(
                    (
                        self.psql_f(script, cwd=dir_country_tmp, background=parallel),
        ...
        # post import scripts
        self.logger.log(
            self.logger.log_av_r + "import osmosis post scripts" + self.logger.log_ap
        )
    ...
    def clean_database(self, conf, no_clean):
        ...
        if schema_size is not None:
            self.logger.sub().log(
                "Schema size is: {} bytes ({})".format(
                    schema_size, human_readable_size(schema_size)
                )
            )
    ...
    def init_osmosis_diff(self, conf):
        ...
        for line in fileinput.input(
            os.path.join(diff_path, "configuration.txt"), inplace=1
        ):
            if line.startswith("baseUrl"):
                ...
                if "geofabrik" in conf.download["diff"]:
                    ...
                    sys.stdout.write(
                        "maxInterval=" + str(60 * 60 * 24 // 2)
                    )  # 1/2 day at most
                else:
                    sys.stdout.write(
                        "maxInterval=" + str(7 * 60 * 60 * 24)
                    )  # 7 day at most
            else:
    ...
    def run_osmosis_diff(self, conf):
        self.logger.log(
            self.logger.log_av_r + "run osmosis replication" + self.logger.log_ap
        )
        diff_path = conf.download["diff_path"]
    ...
    def run_osmium_diff(self, conf):
        self.logger.log(
            self.logger.log_av_r + "run pyosmium replication" + self.logger.log_ap
        )
        pbf_file = conf.download["dst"]
        ...
        try:
            ...
            prev_timestamp = dateutil.parser.isoparse(
            ...
            while not is_uptodate and nb_iter < 10:
                ...
                osmobject = osmium.io.Reader(pbf_file)
                cur_timestamp = dateutil.parser.isoparse(
                    osmobject.header().get("osmosis_replication_timestamp")
                )
    ...
    def init_change(self, conf):
        ...
        self.logger.log(
            self.logger.log_av_r
            + "import osmosis change post scripts"
            + self.logger.log_ap
        )
    ...
    def run_change(self, conf):
        ...
        try:
            ...
            cmd += [
                "--write-pgsql-change",
                "database=%s" % conf.db_base,
                "user=%s" % conf.db_user,
                "password=%s" % conf.db_password,
            ]
            cmd += ["-quiet"]
            self.logger.execute_err(cmd)
            ...
            giscurs.execute(
                "UPDATE metainfo SET tstamp = %s, tstamp_action = %s",
                [osm_state.timestamp(), osm_state_old.timestamp()],
            )
    ...
    def run_resume(self, conf):
        self.logger.log(
            self.logger.log_av_r
            + "import osmosis resume post scripts"
            + self.logger.log_ap
        )
        self.set_pgsql_schema()
