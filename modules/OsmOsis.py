...
class OsmOsis:
    ...
    def NodeGet(self, NodeId):
        self._PgCurs.execute(
            "SELECT nodes.id, st_y(nodes.geom), st_x(nodes.geom), nodes.version, users.name, nodes.tags FROM nodes LEFT JOIN users ON nodes.user_id = users.id WHERE nodes.id = %d;"
            % NodeId
        )
    ...
    def WayGet(self, WayId, dump_sub_elements=False):
        self._PgCurs.execute(
            "SELECT ways.id, ways.version, users.name, ways.tags, ways.nodes FROM ways LEFT JOIN users ON ways.user_id = users.id WHERE ways.id = %d;"
            % WayId
        )
        r1 = self._PgCurs.fetchone()
        if not r1:
        ...
        return {
            ...
            "user": r1[2] or "",
            "tag": r1[3],
            "nd": r1[4] if dump_sub_elements else [],
        }

    def RelationGet(self, RelationId, dump_sub_elements=False):
        self._PgCurs.execute(
            "SELECT relations.id, relations.version, users.name, relations.tags FROM relations LEFT JOIN users ON relations.user_id = users.id WHERE relations.id = %d;"
            % RelationId
        )
        r1 = self._PgCurs.fetchone()
        if not r1:
        ...
        if dump_sub_elements:
            self._PgCurs.execute(
                "SELECT member_id, member_type, member_role FROM relation_members WHERE relation_id = %d ORDER BY sequence_id;"
                % RelationId
            )
            for r1 in self._PgCurs.fetchall():
                data["member"].append(
                    {
                        "ref": r1[0],
                        "type": {"N": "node", "W": "way", "R": "relation"}[r1[1]],
                        "role": r1[2],
                    }
                )
