...
class OsmGis:
    ...
    def NodeGet(self, NodeId):
        ...
        self._PgCurs.execute(
            "SELECT lat, lon, tags FROM %s_nodes WHERE id = %d;"
            % (self._prefix, NodeId)
        )
        ...
        for i in range(len(r1[2]) // 2):
            data["tag"][r1[2][2 * i].decode("utf8")] = r1[2][2 * i + 1].decode("utf8")

        if self._user_patch and "user" in data["tag"]:
    ...
    def WayGet(self, WayId, dump_sub_elements=False):
        ...
        self._PgCurs.execute(
            "SELECT nodes, tags FROM %s_ways WHERE id = %d;" % (self._prefix, WayId)
        )
        r1 = self._PgCurs.fetchone()
        if not r1:
            return None
        ...
        for i in range(len(r1[1]) // 2):
            data["tag"][r1[1][2 * i].decode("utf8")] = r1[1][2 * i + 1].decode("utf8")
    ...
    def RelationGet(self, RelationId, dump_sub_elements=False):
        ...
        self._PgCurs.execute(
            "SELECT members, tags FROM %s_rels WHERE id = %d;"
            % (self._prefix, RelationId)
        )
        r1 = self._PgCurs.fetchone()
        if not r1:
            return None
        ...
        for i in range(len(r1[0]) // 2):
            data["member"].append(
                {
                    "type": {"n": "node", "w": "way", "r": "relation"}[r1[0][2 * i][0]],
                    "ref": int(r1[0][2 * i][1:]),
                    "role": r1[0][2 * i + 1].decode("utf8"),
                }
            )
        data["tag"] = {}
        for i in range(len(r1[1]) // 2):
