...
def analyse_log(filename):
    ...
    for line in f:
        ...
        if new_task != cur_task:
            if cur_task and "iteration=" not in cur_task:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "country",
        nargs="+",
        help="Country to download (can be repeated, can end with *)",
    )
    parser.add_argument(
        "--task", dest="tasks", action="append", help="Task to check (can be repeated)"
    )
    parser.add_argument(
        "--force", action="store_true", help="Force re-downloading previous logs"
    )
    parser.add_argument(
        "--no-buildbot-check",
        action="store_true",
        help="Don't check if Buildbot server has more recent builds ",
    )
    parser.add_argument(
        "--num-builds", action="store", type=int, help="Number of builds to fetch"
    )
    parser.add_argument(
        "--past-build",
        action="store",
        ...
        type=int,
        default=0,
        help="Check build before %(metavar)s",
    )
    ...
    group.add_argument(
        ...
        dest="disabled",
        action="store_true",
        help="Check countries that were disabled on builbot",
    )
    ...
    for country in list_country:
        ...
        if args.no_buildbot_check and os.listdir(c_dir):
            ...
            builds = json.loads(
                requests.get(
                    buildbot_api
                    + "builders/%d/builds?complete__eq=true&order=-complete_at&limit=%d"
                    % (builders[country], args.num_builds + args.past_build)
                ).text
            )
        ...
        for i in orig_list_builds:
            ...
            try:
                for step in (3, 2):
                    u = buildbot_api + "builds/%d/steps/%d/logs/stdio/contents" % (
                        i,
                        step,
                    )
                    r = requests.get(u)
        ...
        if args.stats_country:
            ...
            if args.tasks:
                ...
                for k, _v in sorted(
                    tasks_longest.items(), key=lambda x: x[1][0], reverse=True
                ):
                    big_tasks.append(k)
                print("longest tasks:")
            ...
            for i in range(num_big_tasks):
                print(
                    "  ",
                    str_timedelta(
                        tasks_longest[big_tasks[i]][0], print_day=longer_than_day
                    ),
                    " ",
                    big_tasks[i].split(":")[-1].strip(),
                )
            print()
            ...
            for i in list_builds:
                ...
                print("  ", end=" ")
                print(
                    str_timedelta(stats[i]["total"], print_day=longer_than_day), end=" "
                )
                for t in range(num_big_tasks):
                    ...
                    if tasks_longest[taskname][1] == i:
                        ...
                        taskname in tasks_increase and tasks_increase[taskname][1] == i
                    ):
                        color = "yellow"
                        attrs = None
                    elif (
                        taskname in tasks_decrease and tasks_decrease[taskname][1] == i
                    ):
                        color = "green"
                        attrs = None
                    else:
                    ...
                    try:
                        val = str_timedelta(
                            stats[i]["tasks"][taskname],
                            print_day=tasks_longest[taskname][0].days,
                        )
                    except:
            ...
            if not args.tasks:
                ...
                big_tasks = []
                for k, _v in sorted(
                    tasks_increase.items(), key=lambda x: x[1][0], reverse=True
                ):
                    big_tasks.append(k)
                ...
                for i in range(min(5, len(big_tasks))):
                    print(
                        "  ",
                        str_timedelta(tasks_increase[big_tasks[i]][0]),
                        " ",
                        tasks_increase[big_tasks[i]][1],
                        " ",
                        big_tasks[i].split(":")[-1].strip(),
                    )
                print()
        ...
        if args.stats_global:
            ...
            for t, tt in last_stat["tasks"].items():
                a = t.split(":")[-1].strip()
                global_tasks_longest[a] = (
                    global_tasks_longest.get(a, timedelta_zero) + tt
                )
    ...
    if args.stats_global:
        ...
        if args.tasks:
            ...
            for k, _v in sorted(
                global_tasks_longest.items(), key=lambda x: x[1], reverse=True
            ):
                big_tasks.append(k)
            print("longest tasks:")
        for i in range(min(10, len(big_tasks))):
            print(
                "  ",
                str_timedelta(global_tasks_longest[big_tasks[i]]),
                " ",
                big_tasks[i],
            )
        print()
        ...
        big_countries = []
        for k, _v in sorted(
            global_countries_longest.items(), key=lambda x: x[1], reverse=True
        ):
            big_countries.append(k)
        for i in range(min(10, len(big_countries))):
            print(
                "  ",
                str_timedelta(global_countries_longest[big_countries[i]]),
                " ",
                big_countries[i],
            )
        print()
