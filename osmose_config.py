...
class template_config:
    ...
    osmosis_pre_scripts = [
        ...
        dir_scripts
        + "/osmosis/osmosis-0.48.3-36-gada1b6f5-SNAPSHOT/script/pgsnapshot_schema_0.6_linestring.sql",
    ...
    osmosis_change_init_post_scripts = [  # Scripts to run on database initialisation
        ...
        dir_scripts
        + "/osmosis/osmosis-0.48.3-36-gada1b6f5-SNAPSHOT/script/pgsnapshot_schema_0.6_action.sql",
    ]
    ...
    def __init__(
        self, country, polygon_id=None, analyser_options=None, download_repo=GEOFABRIK
    ):
        config[country] = self
        ...
        self.sql_post_scripts = (
            []
        )  # Scripts to run everytime, just before launching analysers
...
class default_simple(template_config):
    def __init__(
        ...
        country,
        polygon_id=None,
        analyser_options=None,
        download_url=None,
        download_repo=None,
    ):

        template_config.__init__(
            self, country, polygon_id, analyser_options, download_repo
        )
        self.db_extension_check += ["fuzzystrmatch", "unaccent"]
        self.download = {"url": download_url}
...
class default_country_simple(default_simple):
    def __init__(
        ...
        default_simple.__init__(
            self, country, polygon_id, analyser_options, download_repo=download_repo
        )
        ...
        if download_repo == GEOFABRIK:
            self.download["diff"] = (
                self.download_repo + part + download_country + "-updates/"
            )
            self.download["state.txt"] = self.download["diff"] + "state.txt"
        if download_repo == OSMFR:
            self.download["poly"] = self.download["poly"].replace(
                "/extracts/", "/polygons/"
            )
            self.download["diff"] = (
                self.download_repo
            ...
            self.download["state.txt"] = (
                self.download_repo + part + download_country + ".state.txt"
            )
        if download_repo == OSMCH:
            self.download["url"] = (
                self.download_repo + download_country + "-padded.osm.pbf"
            )
...
class default_country(default_country_simple):
    def __init__(
        ...
        default_country_simple.__init__(
            self,
            part,
            country,
            polygon_id,
...
def gen_country(
    ...
    **analyser_options_default
    ...
    def init(
        ...
        country_base=country_base_default,
        download_repo=download_repo_default,
        include=[],
        exclude=[],
        **analyser_options
    ):
...
france_departement = gen_country(
    "europe",
    "france",
    download_repo=OSMFR,
    language="fr",
    proj=2154,
    municipality_ref="ref:INSEE",
    phone_code="33",
    phone_len=9,
...
france_departement(
    "aquitaine/lot_et_garonne", 1284995, "FR-47", include=include_aquitaine
)
france_departement(
    "aquitaine/pyrenees_atlantiques", 7450, "FR-64", include=include_aquitaine
)
...
france_departement(
    "haute_normandie/seine_maritime",
    7426,
    "FR-76",
    include=[
        # Le Havre
...
france_departement(
    "ile_de_france/seine_saint_denis", 7389, "FR-93", include=include_ile_de_france
)
france_departement(
    "ile_de_france/val_de_marne", 7458, "FR-94", include=include_ile_de_france
)
france_departement(
    "ile_de_france/essonne", 7401, "FR-91", include=include_ile_de_france
)
france_departement(
    "ile_de_france/seine_et_marne", 7383, "FR-77", include=include_ile_de_france
)
france_departement(
    "ile_de_france/val_d_oise", 7433, "FR-95", include=include_ile_de_france
)
france_departement(
    "ile_de_france/yvelines", 7457, "FR-78", include=include_ile_de_france
)

france_departement("languedoc_roussillon/aude", 7446, "FR-11")
france_departement("languedoc_roussillon/gard", 7461, "FR-30")
france_departement(
    "languedoc_roussillon/herault",
    7429,
    "FR-34",
    include=[
        # Montpellier
...
france_departement(
    "pays_de_la_loire/loire_atlantique",
    7432,
    "FR-44",
    include=[
        # Nantes
...
france_departement(
    "pays_de_la_loire/maine_et_loire",
    7409,
    "FR-49",
    include=[
        # Angers
...
france_departement(
    "provence_alpes_cote_d_azur/hautes_alpes",
    7436,
    "FR-05",
    include=[
...
france_departement(
    "provence_alpes_cote_d_azur/alpes_maritimes",
    7385,
    "FR-06",
    include=[
...
france_departement_dom = gen_country(
    None,
    country_base="france",
    download_repo=OSMFR,
    language="fr",
...
france_departement_dom(
    ["africa", "mayotte"], 1259885, "FR-YT", dep_code=976, proj=32738, phone_code="262"
)
france_departement_dom(
    ["africa", "reunion"], 1785276, "FR-RE", dep_code=974, proj=2975, phone_code="262"
)

france_com = gen_country(
    None,
    country_base="france",
    download_repo=OSMFR,
    language="fr",
...
france_com(
    ["central-america", "saint_barthelemy"],
    537967,
    "FR-BL",
    dep_code=977,
    proj=2969,
...
france_com(
    ["north-america", "saint_pierre_et_miquelon"],
    3406826,
    "FR-PM",
    dep_code=975,
    proj=32621,
    phone_code="508",
    country="saintpierreetmiquelon",
    include=[
        "merge_cemetery_FR",
        "merge_man_made_FR",
        "merge_poi_FR",
...
france_com(
    ["oceania", "wallis_et_futuna"],
    290162,
    "FR-WF",
    proj=32701,
    phone_code="681",
    country="wallisetfutuna",
)
france_com(
    ["oceania", "polynesie"],
    3412620,
...
default_country(
    ...
    2186658,
    download_repo=OSMFR,
    analyser_options={"country": "TF", "language": "fr", "proj": 32738},
)
default_country(
    "oceania",
    "clipperton",
    2573009,
    download_repo=OSMFR,
    analyser_options={"country": "CP", "language": "fr", "proj": 32612},
)

###########################################################################

france_local_db = template_config(
    "france_local_db",
    1403916,
    {"project": "openstreetmap", "country": "FR", "language": "fr", "proj": 2154},
)
...
default_country(
    "europe", "albania", 53292, {"country": "AL", "language": "sq", "proj": 32634}
)
default_country(
    "europe", "andorra", 9407, {"country": "AD", "language": "ca", "proj": 2154}
)
default_country(
    "europe",
    "belarus",
    59065,
    {"country": "BY", "language": ["be", "ru"], "proj": 32635},
    download_repo=GEOFABRIK,
)
default_country(
    "europe",
    "bosnia-herzegovina",
    2528142,
    {"country": "BA", "language": ["bs", "hr", "sr"], "proj": 32633},
    download_repo=GEOFABRIK,
)
default_country(
    "europe",
    "bulgaria",
    186382,
    {"country": "BG", "language": "bg", "proj": 32635},
    download_repo=GEOFABRIK,
)
default_country(
    "europe",
    "croatia",
    214885,
    {"country": "HR", "language": "hr", "proj": 32633},
    download_repo=GEOFABRIK,
)
default_country(
    "europe",
    "estonia",
    79510,
    {"country": "EE", "language": "et", "proj": 32634},
    download_repo=GEOFABRIK,
)
default_country(
    "europe",
...
default_country(
    "europe", "faroe-islands", 52939, {"country": "FO", "language": "fo", "proj": 2169}
)
default_country(
    "europe",
    "greece",
    192307,
    {"country": "GR", "language": "el", "proj": 32635},
    download_repo=GEOFABRIK,
)
default_country(
    "europe",
...
default_country(
    ...
    "hungary",
    21335,
    {"country": "HU", "language": "hu", "proj": 32633},
    download_repo=GEOFABRIK,
)
default_country(
    "europe",
    "ireland",
    62273,
    {"country": "IE", "driving_side": "left", "language": ["en", "ga"], "proj": 32629},
    download_repo=OSMFR,
)
default_country(
    "europe",
...
default_country(
    ...
    "liechtenstein",
    1155955,
    {"country": "LI", "language": "de", "proj": 32632},
)
default_country(
    "europe",
    "lithuania",
...
default_country(
    ...
    "latvia",
    72594,
    {"country": "LV", "language": "lv", "proj": 32634},
    download_repo=GEOFABRIK,
)
default_country(
    "europe",
    "luxembourg",
    2171347,
    {"country": "LU", "language": "fr_LU", "proj": 2169, "boundary_detail_level": 6},
    include=[
...
default_country(
    ...
    "malta",
    365307,
    {"country": "MT", "language": "en", "driving_side": "left", "proj": 32633},
)
default_country(
    "europe", "macedonia", 53293, {"country": "MK", "language": "sq", "proj": 32634}
)
default_country(
    "europe",
    "moldova",
    58974,
    {"country": "MD", "language": "ro", "proj": 32635},
    download_repo=GEOFABRIK,
)
default_country(
    "europe",
    ...
    1124039,
    {
        ...
        "phone_code": "377",
        "phone_len": 8,
        "phone_format": r"^[+]%s([- ./]*[469])([- ./]*[0-9]){6}[0-9]$",
        "phone_international": "00",
    },
    download_repo=OSMFR,
)
...
default_country(
    "europe", "romania", 90689, {"country": "RO", "language": "ro", "proj": 31700}
)
default_country(
    "europe",
    "san_marino",
    54624,
    {"country": "SM", "language": "it", "proj": 23032},
    download_repo=OSMFR,
)
default_country(
    "europe",
    "serbia",
    1741311,
    {"country": "RS", "language": "sr", "proj": 32634},
    download_repo=GEOFABRIK,
)
default_country(
    "europe",
    "slovenia",
    218657,
    {"country": "SI", "language": ["sl", "hu", "it"], "proj": 32633},
    download_repo=GEOFABRIK,
)
default_country(
    "europe",
    "vatican_city",
    36989,
    {"country": "VA", "language": "it", "proj": 23032},
    download_repo=OSMFR,
)
default_country(
    "europe",
    "united_kingdom_akrotiri_and_dhekelia",
    3263728,
    {"country": "GB", "language": ["en", "he"], "driving_side": "left", "proj": 32636},
    download_country="cyprus",
)  # British Sovereign Base in Cyprus
default_country(
    "europe",
    "united_kingdom_gibraltar",
    1278736,
    {"country": "GI", "language": "en", "proj": 32630},
    download_repo=OSMFR,
    download_country="gibraltar",
)
default_country(
    "europe",
    ...
    download_repo=OSMFR,
    download_country="united_kingdom/northern_ireland",
)
default_country(
    "europe",
    ...
    download_repo=GEOFABRIK,
    download_country="great-britain/wales",
)
default_country(
    "europe",
    ...
    download_repo=GEOFABRIK,
    download_country="great-britain/scotland",
)

iceland = default_country(
    "europe",
    "iceland",
    299133,
    {"country": "IS", "language": "is", "proj": 32627},
    download_repo=GEOFABRIK,
)  # 299133
...
tr_part = gen_country(
    "europe", "turkey", download_repo=OSMFR, language="tr", proj=32636
)
...
se_part = gen_country(
    "europe", "sweden", download_repo=OSMFR, proj=32633, language="sv"
)
...
ch_part = gen_country(
    ...
    download_repo=OSMFR,
    proj=2056,
    municipality_ref=["swisstopo:BFS_NUMMER", "swisstopo:BEZIRKSNUM"],
    phone_code="41",
    phone_len=9,
    phone_international="00",
    phone_local_prefix="0",
...
pt_part = gen_country(
    "europe", "portugal", download_repo=OSMFR, language="pt", proj=32629
)
...
ua_oblasts = gen_country(
    "europe", "ukraine", download_repo=OSMFR, language="uk", proj=32636
)
...
no_county = gen_country(
    "europe", "norway", download_repo=OSMFR, language="no", proj=32632
)

no_county(
    "nordland", (408105, 409035), "NO-18"
)  # 409035 is an old municipality which was in previous nordland boundary
...
antartica = default_country_simple(
    "", "antarctica", 2186646, {"proj": 3031}, download_repo=GEOFABRIK
)
...
default_country(
    ...
    "greenland",
    2184073,
    {"country": "GL", "language": "kl", "proj": 3184},
)
default_country(
    "north-america",
    "united_kingdom_bermuda",
    1993208,
    {"country": "BM", "language": "en", "driving_side": "left", "proj": 32620},
    download_repo=OSMFR,
    download_country="bermuda",
)

#########################################################################

mexico_state = gen_country(
    "north-america",
    "mexico",
    download_repo=OSMFR,
    language="es",
    proj=32614,
    exclude=["osmosis_highway_name_close"],  # Complicated Street Numbering
)
...
us_state = gen_country(
    "north-america/us", country_base="usa", language="en", speed_limit_unit="mph"
)
...
us_ca_county = gen_country(
    "north-america/us-west/california",
    country_base="usa_california",
    download_repo=OSMFR,
    language="en",
    proj=26910,
)
...
us_co_region = gen_country(
    "north-america/us-west/colorado",
    country_base="usa_colorado",
    country_code="US-CA",
    download_repo=OSMFR,
    language="en",
    proj=26713,
...
us_co_region(
    "denver", (1411327, 1411333, 1411321, 1411338, 1411339, 1411346, 1411351, 442310)
)
us_co_region(
    "southeast",
    (
...
us_fl_region = gen_country(
    "north-america/us-south/florida",
    country_base="usa_florida",
    country_code="US-FL",
    download_repo=OSMFR,
    language="en",
    proj=3513,
...
us_fl_region(
    "northeast",
    (1210735, 1210713, 1210715, 1210733, 1210719, 1210749, 1210720, 1210689, 1210745),
)
us_fl_region(
    "northwest",
    (
...
us_ga_region = gen_country(
    "north-america/us-south/georgia",
    country_base="usa_georgia",
    country_code="US-GA",
    download_repo=OSMFR,
    language="en",
    proj=26917,
...
us_mi_region = gen_country(
    "north-america/us-midwest/michigan",
    country_base="usa_michigan",
    country_code="US-MI",
    download_repo=OSMFR,
    language="en",
    proj=3746,
...
us_mi_region(
    "southeast",
    (1582241, 1582242, 1801749, 1801750, 1907314, 359765, 582246, 901500, 901501),
)
...
us_mi_region(
    "southwest",
    (1801747, 1801748, 1801751, 1907304, 1907307, 1907309, 1907317, 1907340, 407519),
)
us_mi_region(
    "west",
    (
...
us_ny_region = gen_country(
    "north-america/us-northeast/new-york",
    country_base="usa_new_york",
    country_code="US-NY",
    download_repo=OSMFR,
    language="en",
    proj=3623,
...
us_ny_region(
    "capital_district",
    (1428041, 87979, 1428790, 1428121, 1428091, 1427734, 1427757, 1838018),
)
...
us_ny_region(
    "hudson_valley", (1838023, 1838026, 1427979, 1838022, 1835362, 1835363, 1838027)
)
...
us_ny_region(
    "southern_tier",
    (1838015, 1838025, 1837994, 1837996, 1837991, 1838024, 1838014, 1838017),
)
us_ny_region(
    "western_new_york", (68855, 36267, 78970, 1837997, 36074, 1837989, 1838089, 78971)
)

us_nc_region = gen_country(
    "north-america/us-south/north-carolina",
    country_base="usa_north_carolina",
    country_code="US-NC",
    download_repo=OSMFR,
    language="en",
    proj=3631,
...
us_tx_region = gen_country(
    "north-america/us-south/texas",
    country_base="usa_texas",
    country_code="US-TX",
    download_repo=OSMFR,
    language="en",
    proj=3082,
...
us_va_region = gen_country(
    "north-america/us-south/virginia",
    country_base="usa_virginia",
    country_code="US-VA",
    download_repo=OSMFR,
    language="en",
    proj=3968,
...
us_va_region(
    "fairfax", (1149984, 206609, 206637, 206642, 206870, 206874, 944948, 945043, 962190)
)
...
default_country(
    ...
    "usa_guam",
    306001,
    {"country": "GU", "language": "en", "proj": 32654},
    download_repo=OSMFR,
    download_country="guam",
)
default_country(
    "oceania",
    "usa_northern_mariana_islands",
    306004,
    {"country": "MP", "language": "en", "proj": 32654},
    download_repo=OSMFR,
    download_country="northern_mariana_islands",
)
default_country(
    "oceania",
    "usa_american_samoa",
    2177187,
    {"country": "AS", "language": "en", "proj": 32601},
    download_repo=OSMFR,
    download_country="american_samoa",
)
...
canada_province = gen_country(
    "north-america", "canada", language="en", **canada_options
)
...
canada_ontario_region = gen_country(
    "north-america",
    "canada/ontario",
    proj=32616,
    country_code="CA-ON",
...
canada_quebec_region = gen_country(
    "north-america",
    "canada/quebec",
    proj=2138,
    country_code="CA-QC",
...
default_country(
    ...
    "algeria",
    192756,
    {"country": "DZ", "language": ["ar", "fr"], "proj": 32631},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "angola",
    195267,
    {"country": "AO", "language": "pt", "proj": 32733},
    download_repo=OSMFR,
)
default_country(
    "africa",
...
default_country(
    ...
    "botswana",
    1889339,
    {"country": "BW", "language": "en", "driving_side": "left", "proj": 32734},
)
default_country(
    "africa",
    "burkina_faso",
    192783,
    {"country": "BF", "language": "fr", "proj": 32630},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "burundi",
    195269,
    {"country": "BI", "language": "fr", "proj": 32735},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "cameroon",
    192830,
    {"country": "CM", "language": "fr", "proj": 32632},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "cape_verde",
    535774,
    {"country": "CV", "language": "pt", "proj": 32626},
    download_repo=OSMFR,
)
default_country(
    "africa",
    ...
    192790,
    {"country": "CF", "language": "fr", "proj": 32634},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "chad",
    2361304,
    {"country": "TD", "language": ["ar", "fr"], "proj": 32634},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "comoros",
    535790,
    {"country": "KM", "language": ["ar", "fr"], "proj": 32738},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "congo_brazzaville",
    192794,
    {"country": "CG", "language": "fr", "proj": 32733},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "congo_kinshasa",
    192795,
    {"country": "CD", "language": "fr", "proj": 32734},
    download_repo=OSMFR,
)
default_country(
    "africa",
    ...
    192801,
    {
        ...
        "language": ["fr", "ar"],
        "proj": 32638,
        "multilingual_style": "dj",
    },
    download_repo=OSMFR,
)
default_country(
    "africa", "egypt", 1473947, {"country": "EG", "language": "ar", "proj": 32635}
)
default_country(
    "africa",
    "equatorial_guinea",
    192791,
    {"country": "GQ", "language": "es", "proj": 32732},
    download_repo=OSMFR,
)
default_country(
    "africa", "eritrea", 296961, {"country": "ER", "proj": 32637}, download_repo=OSMFR
)
default_country(
    "africa", "ethiopia", 192800, {"country": "ET", "proj": 32638}, download_repo=OSMFR
)
default_country(
    "africa",
    "gabon",
    192793,
    {"country": "GA", "language": "fr", "proj": 32732},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "gambia",
    192774,
    {"country": "GM", "language": "en", "proj": 32628},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "ghana",
    192781,
    {"country": "GH", "language": "en", "proj": 32630},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "guinea",
    192778,
    {"country": "GN", "language": "fr", "proj": 32628},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "guinea-bissau",
    192776,
    {"country": "GW", "language": "pt", "proj": 32628},
)
default_country(
    "africa",
    "ivory_coast",
    192779,
    {"country": "CI", "language": "fr", "proj": 32630},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "kenya",
    192798,
    {"country": "KE", "language": "en", "driving_side": "left", "proj": 32737},
    download_repo=OSMFR,
)
default_country(
    "africa",
    ...
    {"country": "KE", "language": "en", "driving_side": "left", "proj": 32737},
    download_repo=OSMFR,
)  # Disputed area, defacto controled by Kenya

default_country(
    "africa",
    "lesotho",
    2093234,
    {"country": "LS", "language": "en", "driving_side": "left", "proj": 32735},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "liberia",
    192780,
    {"country": "LR", "language": "en", "speed_limit_unit": "mph", "proj": 32629},
)
default_country(
    "africa", "libya", 192758, {"country": "LY", "language": "ar", "proj": 32633}
)
default_country(
    "africa",
    "madagascar",
    447325,
    {"country": "MG", "language": ["fr", "mg"], "proj": 32738},
    download_repo=GEOFABRIK,
)
default_country(
    "africa",
    "malawi",
    195290,
    {"country": "MW", "language": "en", "driving_side": "left", "proj": 32736},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "mali",
    192785,
    {"country": "ML", "language": "fr", "proj": 32630},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "mauritania",
    192763,
    {"country": "MR", "language": "ar", "proj": 32628},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "mauritius",
    535828,
    {"country": "MU", "language": ["en", "fr"], "driving_side": "left", "proj": 32740},
    download_repo=OSMFR,
)
default_country(
    "africa",
    ...
    {
        "country": "MA",
        "language": ["ar", "fr", "zgh", "ber"],
        "proj": 32629,
        "multilingual_style": "ma",
    },
)

mozambique_province = gen_country(
    "africa",
    "mozambique",
    proj=32736,
    country_code="MZ",
...
default_country(
    ...
    "namibia",
    195266,
    {"country": "NA", "language": "en", "driving_side": "left", "proj": 32733},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "niger",
    192786,
    {"country": "NE", "language": "fr", "proj": 32632},
    download_repo=OSMFR,
)

nigeria_zones = gen_country(
    "africa",
    "nigeria",
    proj=32633,
    country_code="NG",
...
nigeria_zones(
    "north_central", (3716076, 3717259, 3717971, 3718090, 3718384, 3720495, 3720611)
)
nigeria_zones("north_east", (3698564, 3720358, 3720422, 3720850, 3721167, 3722233))
nigeria_zones(
    "north_west", (3703236, 3706956, 3707368, 3707933, 3709353, 3710302, 3711481)
)
...
default_country(
    "africa",
    "norway_bouvet_island",
    2425963,
    {"country": "BV", "language": "no", "proj": 32729},
    download_repo=OSMFR,
    download_country="bouvet_island",
)
default_country(
    "africa",
    "rwanda",
    171496,
    {"country": "RW", "language": ["en", "fr"], "proj": 32735},
    download_repo=OSMFR,
)
default_country(
    "africa",
    ...
    535880,
    {"country": "ST", "language": "pt", "proj": 32632},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "senegal",
    192775,
    {"country": "SN", "language": "fr", "proj": 32628},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "seychelles",
    536765,
    {"country": "SC", "language": ["en", "fr"], "driving_side": "left", "proj": 32739},
    download_repo=OSMFR,
)
default_country(
    "africa", "sierra-leone", 192777, {"country": "SL", "language": "en", "proj": 32629}
)
default_country(
    "africa", "somalia", 192799, {"country": "SO", "language": "so", "proj": 32638}
)

south_africa_province = gen_country(
    "africa",
    "south_africa",
    proj=32735,
    country_code="ZA",
...
default_country(
    ...
    "south_sudan",
    1656678,
    {"country": "SS", "language": "en", "proj": 32635},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "sudan",
    192789,
    {"country": "SD", "language": ["ar", "en"], "proj": 32636},
    download_repo=OSMFR,
)
default_country(
    "africa",
...
default_country(
    ...
    "swaziland",
    88210,
    {"country": "SZ", "language": "en", "driving_side": "left", "proj": 32736},
    download_repo=OSMFR,
)

tanzania_zone = gen_country(
    "africa",
    "tanzania",
    proj=32736,
    country_code="TZ",
...
default_country(
    ...
    "togo",
    192782,
    {"country": "TG", "language": "fr", "proj": 32631},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "tunisia",
    192757,
    {"country": "TN", "language": ["ar", "fr"], "proj": 32632},
    download_repo=OSMFR,
)

uganda_region = gen_country(
    "africa",
    "uganda",
    proj=32636,
    country_code="UG",
...
default_country(
    "africa",
    "united_kingdom_saint_helena_ascension_tristan_da_cunha",
    1964272,
    {"country": "SH", "language": "en", "driving_side": "left", "proj": 32729},
    download_repo=OSMFR,
    download_country="saint_helena_ascension_tristan_da_cunha",
)
default_country(
    "africa",
    "western_sahara",
    2559126,
    {"country": "EH", "proj": 32629},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "zambia",
    195271,
    {"country": "ZM", "language": "en", "driving_side": "left", "proj": 32736},
    download_repo=OSMFR,
)
default_country(
    "africa",
    "zimbabwe",
    195272,
    {"country": "ZW", "language": "en", "driving_side": "left", "proj": 32736},
    download_repo=OSMFR,
)
...
for _country, c in config.items():
    if (
        c.download
        and "url" in c.download
        and "/africa/" in c.download["url"]
        and not ("mayotte" in c.download["url"] or "reunion" in c.download["url"])
    ):
...
default_country(
    "asia", "afghanistan", 303427, {"country": "AF", "proj": 32641}, download_repo=OSMFR
)
default_country(
    "asia",
    "armenia",
    364066,
    {"country": "AM", "language": "hy", "proj": 32641},
    download_repo=OSMFR,
)
default_country(
    "asia", "azerbaijan", 364110, {"country": "AZ", "language": "az", "proj": 32638}
)
default_country(
    "asia",
    "bangladesh",
    184640,
    {"country": "BD", "language": "bn", "driving_side": "left", "proj": 32646},
)
default_country(
    "asia",
    "bahrain",
    378734,
    {"country": "BH", "language": "ar", "proj": 32639},
    download_repo=OSMFR,
)
default_country(
    "asia",
    "bhutan",
    184629,
    {"country": "BT", "language": ["dz", "en"], "driving_side": "left", "proj": 32646},
    download_repo=OSMFR,
)
default_country(
    "asia",
    "brunei",
    2103120,
    {"country": "BN", "driving_side": "left", "language": "ms", "proj": 32650},
    download_repo=OSMFR,
)
default_country(
    "asia",
    "cambodia",
    49898,
    {"country": "KHM", "language": "km", "proj": 32648},
    download_repo=OSMFR,
)
default_country(
    "asia",
    "east_timor",
    305142,
    {"country": "TL", "language": "pt", "driving_side": "left", "proj": 32651},
    download_repo=OSMFR,
)
default_country(
    "asia",
    "georgia",
    28699,
    {"country": "GE", "language": "ka", "proj": 32637},
    download_repo=OSMFR,
)
default_country(
    "asia",
    "israel",
    1473946,
    {"country": "IL", "language": ["he", "ar"], "proj": 32636},
    download_repo=OSMFR,
)
default_country(
    "asia",
...
default_country(
    ...
    "iran",
    304938,
    {"country": "IR", "language": "fa", "proj": 32640},
    download_repo=GEOFABRIK,
)
default_country(
    "asia", "iraq", 304934, {"country": "IQ", "language": "ar", "proj": 32638}
)
default_country(
    "asia", "jordan", 184818, {"country": "JO", "language": "ar", "proj": 32637}
)
default_country(
    "asia",
    "kazakhstan",
    214665,
    {"country": "KZ", "proj": 32640},
    download_repo=GEOFABRIK,
)
default_country(
    "asia",
    "kuwait",
    305099,
    {"country": "KW", "language": "ar", "proj": 32639},
    download_repo=OSMFR,
)
default_country(
    "asia",
    "kyrgyzstan",
    178009,
    {"country": "KG", "language": ["ky", "ru"], "proj": 32643},
)
default_country(
    "asia",
    "laos",
    49903,
    {"country": "LA", "language": ["lo", "en"], "proj": 32648},
    download_repo=OSMFR,
)
default_country(
    "asia", "lebanon", 184843, {"country": "LB", "language": "ar", "proj": 32636}
)
default_country(
    "asia",
    "malaysia",
    2108121,
    {"country": "MY", "language": "ms", "driving_side": "left", "proj": 32649},
    download_repo=OSMFR,
)
default_country(
    "asia",
    "maldives",
    536773,
    {"country": "MV", "language": "dv", "driving_side": "left", "proj": 32643},
    download_repo=OSMFR,
)
default_country(
    "asia", "mongolia", 161033, {"country": "MN", "language": "mn", "proj": 32648}
)
default_country(
    "asia",
    "myanmar",
    50371,
    {"country": "MM", "language": "my", "proj": 32646},
    download_repo=OSMFR,
)
default_country(
    "asia",
    "north_korea",
    192734,
    {"country": "KP", "language": "ko", "proj": 32652},
    download_country="north-korea",
)
default_country(
    "asia",
    "nepal",
    184633,
    {"country": "NP", "language": "ne", "driving_side": "left", "proj": 32645},
)
default_country(
    "asia",
    "oman",
    305138,
    {"country": "OM", "language": "ar", "proj": 32640},
    download_repo=OSMFR,
)
default_country(
    "asia",
    "pakistan",
    307573,
    {"country": "PK", "language": ["en", "ur"], "driving_side": "left", "proj": 32642},
)
default_country(
    "asia",
    "palestine",
    1703814,
    {"country": "PS", "language": "ar", "proj": 32636},
    download_repo=OSMFR,
)
default_country(
    "asia",
    "qatar",
    305095,
    {"country": "QA", "language": "ar", "proj": 32639},
    download_repo=OSMFR,
)
default_country(
    "asia",
    "saudi_arabia",
    307584,
    {"country": "SA", "language": "ar", "proj": 32637},
    download_repo=OSMFR,
)
default_country(
    "asia",
    "singapore",
    536780,
    {"country": "SG", "language": "en", "driving_side": "left", "proj": 32648},
    download_repo=OSMFR,
)
default_country(
    "asia",
...
default_country(
    ...
    "south_korea",
    307756,
    {"country": "KR", "language": "ko", "proj": 32652},
    download_country="south-korea",
)
default_country(
    "asia", "syria", 184840, {"country": "SY", "language": "ar", "proj": 32637}
)
default_country(
    "asia", "tajikistan", 214626, {"country": "TJ", "language": "tg", "proj": 32642}
)
default_country(
    "asia",
    "taiwan",
    449220,
    {"country": "TW", "language": ["zh_TW", "en"], "proj": 32651},
    download_repo=GEOFABRIK,
)
default_country(
    "asia",
    "thailand",
    2067731,
    {"country": "TH", "language": "th", "proj": 32647, "driving_side": "left"},
)
default_country(
    "asia", "turkmenistan", 223026, {"country": "TM", "language": "tk", "proj": 32640}
)
default_country(
    "asia",
    "united_arab_emirates",
    307763,
    {"country": "AE", "language": "ar", "proj": 32640},
    download_repo=OSMFR,
    exclude=[
...
default_country(
    "asia",
    "united_kingdom_british_indian_ocean_territory",
    1993867,
    {"country": "IO", "language": "en", "driving_side": "left", "proj": 32742},
    download_repo=OSMFR,
    download_country="british_indian_ocean_territory",
)
default_country(
    "asia",
    "uzbekistan",
    196240,
    {"country": "UZ", "proj": 32640},
    download_repo=GEOFABRIK,
)
default_country(
    "asia",
    "vietnam",
    49915,
    {"country": "VN", "language": "vi", "proj": 32648},
    download_repo=GEOFABRIK,
)
default_country(
    "asia",
    "yemen",
    305092,
    {"country": "YE", "language": "ar", "proj": 32638},
    download_repo=GEOFABRIK,
)

#########################################################################

ph_state = gen_country(
    "asia",
    "philippines",
    download_repo=OSMFR,
    language="en",
...
default_country(
    ...
    "belize",
    287827,
    {"country": "BZ", "language": "en", "speed_limit_unit": "mph", "proj": 32616},
)
default_country(
    "central-america",
    "costa_rica",
    287667,
    {"country": "CR", "language": "es", "proj": 32617},
    download_repo=OSMFR,
)
default_country(
    "central-america",
    "el_salvador",
    1520612,
    {"country": "SV", "language": "es", "proj": 32616},
    download_repo=OSMFR,
)
default_country(
    "central-america",
    "guatemala",
    1521463,
    {"country": "GT", "language": "es", "proj": 32616},
)
default_country(
    "central-america",
    "honduras",
    287670,
    {"country": "HN", "language": "es", "proj": 32616},
    download_repo=OSMFR,
)
default_country(
    "central-america",
    "panama",
    287668,
    {"country": "PA", "language": "es", "proj": 32617},
    download_repo=OSMFR,
)
default_country(
    "central-america",
    "trinidad_and_tobago",
    555717,
    {"country": "TT", "language": "en", "driving_side": "left", "proj": 32620},
    download_repo=OSMFR,
)
...
default_country(
    ...
    "haiti",
    307829,
    {"country": "HT", "language": "fr", "proj": 32618},
    download_repo=GEOFABRIK,
    download_country="haiti-and-domrep",
)
...
default_country(
    ...
    "antigua_and_barbuda",
    536900,
    {"country": "BB", "language": "en", "driving_side": "left", "proj": 32620},
    download_repo=OSMFR,
)
default_country(
    "central-america",
    "barbados",
    547511,
    {"country": "BB", "language": "en", "driving_side": "left", "proj": 32621},
    download_repo=OSMFR,
)
default_country(
    "central-america",
...
default_country(
    ...
    "dominica",
    307823,
    {"country": "DM", "language": "en", "driving_side": "left", "proj": 32620},
    download_repo=OSMFR,
)
default_country(
    "central-america",
    "dominican_republic",
    307828,
    {"country": "DO", "language": "es", "proj": 32619},
    download_repo=GEOFABRIK,
    download_country="haiti-and-domrep",
)
default_country(
    ...
    "grenada",
    550727,
    {"country": "GD", "language": "en", "driving_side": "left", "proj": 32620},
    download_repo=OSMFR,
)
default_country(
    "central-america",
    "jamaica",
    555017,
    {"country": "JM", "language": "en", "driving_side": "left", "proj": 32620},
    download_repo=OSMFR,
)
default_country(
    "central-america",
    "nicaragua",
    287666,
    {"country": "NI", "language": "es", "proj": 32616},
    download_repo=OSMFR,
)
default_country(
    "central-america",
    "saint_lucia",
    550728,
    {"country": "LC", "language": "en", "driving_side": "left", "proj": 32620},
    download_repo=OSMFR,
)
default_country(
    "central-america",
    ...
    550725,
    {"country": "VC", "language": "en", "driving_side": "left", "proj": 32620},
    download_repo=OSMFR,
)
default_country(
    "central-america",
    ...
    536899,
    {"country": "KN", "language": "en", "driving_side": "left", "proj": 2005},
    download_repo=OSMFR,
)
default_country(
    "central-america",
    ...
    2177161,
    {"country": "AI", "language": "en", "driving_side": "left", "proj": 32620},
    download_repo=OSMFR,
    download_country="anguilla",
)
default_country(
    "central-america",
    "united_kingdom_cayman_islands",
    2185366,
    {"country": "KY", "language": "en", "driving_side": "left", "proj": 32617},
    download_repo=OSMFR,
    download_country="cayman_islands",
)
default_country(
    "central-america",
    "united_kingdom_montserrat",
    537257,
    {"country": "MS", "language": "en", "driving_side": "left", "proj": 2005},
    download_repo=OSMFR,
    download_country="montserrat",
)
default_country(
    "central-america",
    "united_kingdom_turks_and_caicos_islands",
    547479,
    {"country": "TC", "language": "en", "driving_side": "left", "proj": 32619},
    download_repo=OSMFR,
    download_country="turks_and_caicos_islands",
)
default_country(
    "central-america",
    "united_kingdom_virgin_islands",
    285454,
    {"country": "VG", "language": "en", "driving_side": "left", "proj": 32620},
    download_repo=OSMFR,
    download_country="british_virgin_islands",
)
default_country(
    "central-america",
    "usa_puerto_rico",
    4422604,
    {
        ...
        "language": ["es", "en"],
        "proj": 32619,
        "boundary_detail_level": 6,
    },
    download_repo=OSMFR,
    download_country="puerto_rico",
)
default_country(
    "central-america",
    "usa_virgin_islands",
    286898,
    {"country": "VI", "language": "en", "driving_side": "left", "proj": 4437},
    download_repo=OSMFR,
    download_country="usa_virgin_islands",
)
...
default_country(
    ...
    "cook_islands",
    2184233,
    {"country": "CK", "language": "en", "driving_side": "left", "proj": 32603},
    download_repo=OSMFR,
)
default_country(
    "oceania",
    "marshall_islands",
    571771,
    {"country": "MH", "language": "en", "proj": 32660},
    download_repo=OSMFR,
)
default_country(
    "oceania",
    "nauru",
    571804,
    {"country": "NR", "language": "en", "driving_side": "left", "proj": 32659},
    download_repo=OSMFR,
)
default_country(
    "oceania",
    "niue",
    1558556,
    {"country": "NU", "language": "en", "driving_side": "left", "proj": 32602},
    download_repo=OSMFR,
)
default_country(
    "oceania",
    "palau",
    571805,
    {"country": "PW", "language": "en", "proj": 32653},
    download_repo=OSMFR,
)
default_country(
    "oceania",
    "micronesia",
    571802,
    {"country": "FM", "language": "en", "speed_limit_unit": "mph", "proj": 32656},
    download_repo=OSMFR,
)
default_country(
    "oceania",
    "papua_new_guinea",
    307866,
    {"country": "PG", "language": "en", "driving_side": "left", "proj": 32755},
    download_repo=OSMFR,
)
default_country(
    "oceania",
...
default_country(
    ...
    "solomon_islands",
    1857436,
    {"country": "SB", "language": "en", "driving_side": "left", "proj": 32657},
    download_repo=OSMFR,
)
default_country(
    "oceania",
    "new_zealand_tokelau",
    2186600,
    {"country": "TK", "language": "en", "driving_side": "left", "proj": 32602},
    download_repo=OSMFR,
    download_country="tokelau",
)
default_country(
    "oceania",
    "tonga",
    2186665,
    {"country": "TO", "language": "en", "driving_side": "left", "proj": 32601},
    download_repo=OSMFR,
)
default_country(
    "oceania",
    "tuvalu",
    2177266,
    {"country": "TV", "language": "en", "driving_side": "left", "proj": 32660},
    download_repo=OSMFR,
)
default_country(
    "oceania",
    "united_kingdom_pitcairn",
    2185375,
    {"country": "PN", "language": "en", "driving_side": "left", "proj": 32709},
    download_repo=OSMFR,
    download_country="pitcairn",
)
default_country(
    "oceania",
    "vanuatu",
    2177246,
    {"country": "VU", "language": ["en", "fr"], "proj": 32658},
    download_repo=OSMFR,
)
...
default_country(
    ...
    "fiji",
    571747,
    {"country": "FJ", "language": "en", "driving_side": "left", "proj": 32660},
    download_repo=OSMFR,
)
default_country(
    "merge",
    "kiribati",
    571178,
    {"country": "KL", "language": "en", "driving_side": "left", "proj": 32660},
    download_repo=OSMFR,
)

#########################################################################

au_state = gen_country(
    "oceania", "australia", download_repo=OSMFR, language="en", driving_side="left"
)

au_state("australian_capital_territory", 2354197, "AU-ACT", proj=32755)

au_state(
    "new_south_wales/greater_metropolitan_sydney",
    (
        7038238,
        6217858,
        2161967,
...
au_state(
    "new_south_wales/sydney_surrounds",
    (6299568, 6195207, 6222206, 6287823),
    "AU-NSW",
    proj=32755,
)
au_state(
    "new_south_wales/mid_north_coast",
    (6165980, 6143220, 6165983, 6178731, 6165979, 6180709, 8425530),
    "AU-NSW",
    proj=32755,
)
au_state(
    "new_south_wales/murray",
    (3487773, 5897433, 6230406, 6139012, 6232188, 3487850, 6134295, 5884892),
    "AU-NSW",
    proj=32755,
)
au_state(
    "new_south_wales/the_riverina",
    (
        6257747,
        6095007,
        6321566,
...
au_state(
    "new_south_wales/greater_metropolitan_newcastle",
    (
        6191219,
        6185449,
        6191221,
...
au_state(
    "new_south_wales/illawarra",
    (6311642, 6304532, 6311648, 6304530, 6303077),
    "AU-NSW",
    proj=32755,
)
au_state(
    "new_south_wales/richmond_tweed",
    (6168644, 6170304, 6163138, 6168485, 6143221, 6170302),
    "AU-NSW",
    proj=32755,
)
au_state(
    "new_south_wales/south_east_region",
    (6239664, 6239661, 6310493, 6256597, 6137309, 6239422, 6257920, 6247446),
    "AU-NSW",
    proj=32755,
)
au_state(
    "new_south_wales/northern",
    (
        6165969,
        6163165,
        6389934,
...
default_country(
    ...
    "bolivia",
    252645,
    {"country": "BO", "language": "es", "proj": 32720},
)

chile_region = gen_country(
    "south-america",
    "chile",
    proj=32718,
    country_code="CL",
...
colombia = default_country(
    "south-america",
    "colombia",
    120027,
    {"country": "CO", "language": "es", "proj": 32618},
    exclude=[
        "osmosis_highway_name_close",  # Complicated Street Numbering
    ],
)
default_country(
    ...
    "ecuador",
    108089,
    {"country": "EC", "language": "es", "proj": 32727},
)
default_country(
    "south-america",
    "guyana",
    287083,
    {"country": "GY", "language": "en", "driving_side": "left", "proj": 32621},
    download_repo=OSMFR,
)
default_country(
    "south-america",
    "paraguay",
    287077,
    {"country": "PY", "language": "es", "proj": 32721},
    download_repo=OSMFR,
)
default_country(
    "south-america", "peru", 288247, {"country": "PE", "language": "es", "proj": 32718}
)
default_country(
    "south-america",
    "suriname",
    287082,
    {"country": "SR", "language": "nl", "driving_side": "left", "proj": 32621},
    download_repo=OSMFR,
)
default_country(
    "south-america",
    "united_kingdom_falkland",
    2185374,
    {"country": "FK", "language": "en", "driving_side": "left", "proj": 32721},
    download_repo=OSMFR,
    download_country="falkland",
)
default_country(
    "south-america",
    "united_kingdom_south_georgia_and_south_sandwich",
    1983628,
    {"country": "GS", "language": "en", "driving_side": "left", "proj": 32725},
    download_repo=OSMFR,
    download_country="south_georgia_and_south_sandwich",
)
default_country(
    "south-america",
    "uruguay",
    287072,
    {"country": "UY", "language": "es", "proj": 32721},
)
default_country(
    "south-america",
    "venezuela",
    272644,
    {"country": "VE", "language": "es", "proj": 32620},
    download_repo=OSMFR,
)

#########################################################################

ar_state = gen_country(
    "south-america",
    "argentina",
    download_repo=OSMFR,
    language="es",
...
it_region = gen_country(
    "europe",
    "italy",
    download_repo=OSMFR,
    language="it",
    proj=23032,
    municipality_ref="ref:ISTAT",
    phone_code="39",
    phone_len=[6, 11],
    phone_len_short=[3, 4],
    phone_international="00",
...
nl_province = gen_country(
    "europe", "netherlands", download_repo=OSMFR, language="nl", proj=23032
)
...
nl_province(
    "curacao", 1216719, "CW", area="central-america", path_base=None, proj=32620
)
nl_province(
    "sint_maarten", 1231790, "SX", area="central-america", path_base=None, proj=32620
)
nl_province(
    "caribbean", 1216720, "NL", area="central-america", path_base=None, proj=32620
)

#########################################################################

cz_kraj = gen_country(
    "europe", "czech_republic", download_repo=OSMFR, language="cs", proj=32633
)
...
de_state = gen_country(
    "europe",
    "germany",
    language="de",
    proj=32632,
    municipality_ref="de:regionalschluessel",
    phone_code="49",
    phone_international="00",
    phone_local_prefix="0",
...
at_state = gen_country(
    "europe", "austria", download_repo=OSMFR, language="de", proj=32633
)
...
es_prov = gen_country(
    ...
    download_repo=OSMFR,
    language="es",
    municipality_ref="ine:municipio",
    phone_code="34",
    phone_len=9,
...
es_prov(
    ...
    "ES-AL",
    proj=32629,
    hydro=["Cuencas Mediterráneas Andaluzas"],
)
es_prov(
    "andalucia/cadiz",
    349017,
    "ES-CA",
    proj=32629,
    hydro=["Guadalete y Barbate", "Cuencas Mediterráneas Andaluzas"],
)
es_prov(
    "andalucia/cordoba", 349016, "ES-CO", proj=32629, hydro=["Guadalquivir", "Guadiana"]
)
es_prov(
    "andalucia/granada",
    349026,
    "ES-GR",
    proj=32629,
    hydro=["Guadalquivir", "Cuencas Mediterráneas Andaluzas"],
)
es_prov(
    "andalucia/huelva",
    348995,
    "ES-H",
    proj=32629,
    hydro=["Tinto Odiel y Piedras", "Guadiana", "Guadalquivir"],
)
...
es_prov(
    ...
    proj=32629,
    language=["es", "ast"],
    multilingual_style="sp_ast",
    hydro=["Cantábrico Occidental"],
)
...
es_prov(
    "castilla_la_mancha/ciudad_real",
    349019,
    "ES-CR",
    proj=32630,
    hydro=["Guadiana", "Guadalquivir"],
)
es_prov(
    "castilla_la_mancha/cuenca",
    349024,
    "ES-CU",
    proj=32630,
    hydro=["Júcar", "Guadiana", "Tajo"],
)
es_prov("castilla_la_mancha/guadalajara", 349025, "ES-GU", proj=32630, hydro=["Tajo"])
es_prov(
    "castilla_la_mancha/toledo", 348982, "ES-TO", proj=32630, hydro=["Tajo", "Guadiana"]
)
...
es_prov(
    "castilla_y_leon/leon", 349010, "ES-LE", proj=32629, hydro=["Duero", "Miño-Sil"]
)
...
es_prov(
    ...
    proj=32630,
    language="ca",
    hydro=["Cuencas Internas de Cataluña"],
)
es_prov(
    "catalunya/girona",
    349023,
...
es_prov(
    ...
    proj=32630,
    language="ca",
    hydro=["Ebro", "Cuencas Internas de Cataluña"],
)

es_prov(
    "comunitat_valenciana/alicante",
    349012,
    "ES-A",
    proj=32630,
    language=["ca", "es"],
...
es_prov(
    "comunitat_valenciana/castellon",
    349020,
    "ES-CS",
    proj=32630,
    language=["ca", "es"],
...
es_prov(
    ...
    proj=32629,
    language=["gl", "es"],
    hydro=["Miño-Sil", "Cantábrico Occidental", "Galicia-Costa"],
)
es_prov(
    "galicia/ourense",
    348988,
...
es_prov(
    "canarias/santa_cruz_de_tenerife",
    349014,
    "ES-TF",
    proj=32628,
    area="africa",
...
sk_kraj = gen_country(
    "europe", "slovakia", download_repo=OSMFR, language="sk", proj=32634
)
...
russia_region(
    ["central_federal_district", "belgorod_oblast"], 83184, "RU-BEL", proj=32637
)
russia_region(
    ["central_federal_district", "bryansk_oblast"], 81997, "RU-BRY", proj=32636
)
russia_region(
    ["central_federal_district", "ivanovo_oblast"], 85617, "RU-IVA", proj=32637
)
russia_region(
    ["central_federal_district", "kaluga_oblast"], 81995, "RU-KLU", proj=32636
)
russia_region(
    ["central_federal_district", "kostroma_oblast"], 85963, "RU-KOS", proj=32637
)
russia_region(["central_federal_district", "kursk_oblast"], 72223, "RU-KRS", proj=32637)
russia_region(
    ["central_federal_district", "lipetsk_oblast"], 72169, "RU-LIP", proj=32637
)
russia_region(
    ["central_federal_district", "moscow_oblast"], 51490, "RU-MOS", proj=32637
)
...
russia_region(
    ["central_federal_district", "ryazan_oblast"], 71950, "RU-RYA", proj=32637
)
russia_region(
    ["central_federal_district", "smolensk_oblast"], 81996, "RU-SMO", proj=32636
)
russia_region(
    ["central_federal_district", "tambov_oblast"], 72180, "RU-TAM", proj=32637
)
russia_region(["central_federal_district", "tula_oblast"], 81993, "RU-TUL", proj=32637)
russia_region(
    ["central_federal_district", "tver_oblast"], 2095259, "RU-TVE", proj=32637
)
russia_region(
    ["central_federal_district", "vladimir_oblast"], 72197, "RU-VLA", proj=32637
)
russia_region(
    ["central_federal_district", "voronezh_oblast"], 72181, "RU-VOR", proj=32637
)
russia_region(
    ["central_federal_district", "yaroslavl_oblast"], 81994, "RU-YAR", proj=32637
)
russia_region(
    ["far_eastern_federal_district", "amur_oblast"], 147166, "RU-AMU", proj=32652
)
russia_region(
    ["far_eastern_federal_district", "chukotka_autonomous_okrug"],
    151231,
    "RU-CHU",
    proj=32659,
)
russia_region(
    ["far_eastern_federal_district", "jewish_autonomous_oblast"],
    147167,
    "RU-YEV",
    proj=32653,
)
russia_region(
    ["far_eastern_federal_district", "kamchatka_krai"], 151233, "RU-KAM", proj=32658
)
russia_region(
    ["far_eastern_federal_district", "khabarovsk_krai"], 151223, "RU-KHA", proj=32653
)
russia_region(
    ["far_eastern_federal_district", "magadan_oblast"], 151228, "RU-MAG", proj=32656
)
russia_region(
    ["far_eastern_federal_district", "primorsky_krai"], 151225, "RU-PRI", proj=32653
)
russia_region(
    ["far_eastern_federal_district", "sakha_republic"], 151234, "RU-SA", proj=32652
)
russia_region(
    ["far_eastern_federal_district", "sakhalin_oblast"], 394235, "RU-SAK", proj=32654
)
russia_region(
    ["north_caucasian_federal_district", "chechen_republic"],
    109877,
    "RU-CE",
    proj=32638,
)
russia_region(
    ["north_caucasian_federal_district", "dagestan_republic"],
    109876,
    "RU-DA",
    proj=32638,
)
russia_region(
    ["north_caucasian_federal_district", "ingushetia_republic"],
    253252,
    "RU-IN",
    proj=32638,
)
russia_region(
    ["north_caucasian_federal_district", "kabardino_balkar_republic"],
    109879,
    "RU-KB",
    proj=32638,
)
russia_region(
    ["north_caucasian_federal_district", "karachay_cherkess_republic"],
    109878,
    "RU-KC",
    proj=32638,
)
russia_region(
    ["north_caucasian_federal_district", "north_ossetia_alania_republic"],
    110032,
    "RU-SE",
    proj=32638,
)
russia_region(
    ["north_caucasian_federal_district", "stavropol_krai"], 108081, "RU-STA", proj=32638
)
russia_region(
    ["northwestern_federal_district", "arkhangelsk_oblast"],
    140337,
    "RU-ARK",
    proj=32638,
)
russia_region(
    ["northwestern_federal_district", "kaliningrad_oblast"],
    103906,
    "RU-KGD",
    proj=32634,
)
russia_region(
    ["northwestern_federal_district", "karelia_republic"], 393980, "RU-KR", proj=32636
)
russia_region(
    ["northwestern_federal_district", "komi_republic"], 115136, "RU-KO", proj=32640
)
russia_region(
    ["northwestern_federal_district", "leningrad_oblast"], 176095, "RU-LEN", proj=32636
)
russia_region(
    ["northwestern_federal_district", "murmansk_oblast"], 2099216, "RU-MUR", proj=32636
)
russia_region(
    ["northwestern_federal_district", "nenets_autonomous_okrug"],
    274048,
    "RU-NEN",
    proj=32639,
)
russia_region(
    ["northwestern_federal_district", "novgorod_oblast"], 89331, "RU-NGR", proj=32636
)
russia_region(
    ["northwestern_federal_district", "pskov_oblast"], 155262, "RU-PSK", proj=32636
)
russia_region(
    ["northwestern_federal_district", "saint_petersburg"], 337422, "RU-SPE", proj=32636
)
russia_region(
    ["northwestern_federal_district", "vologda_oblast"], 115106, "RU-VLG", proj=32637
)
russia_region(["siberian_federal_district", "altai_krai"], 144764, "RU-ALT", proj=32644)
russia_region(
    ["siberian_federal_district", "altai_republic"], 145194, "RU-AL", proj=32645
)
russia_region(
    ["siberian_federal_district", "buryatia_republic"], 145729, "RU-BU", proj=32647
)
russia_region(
    ["siberian_federal_district", "irkutsk_oblast"], 145454, "RU-IRK", proj=32648
)
russia_region(
    ["siberian_federal_district", "kemerovo_oblast"], 144763, "RU-KEM", proj=32645
)
russia_region(
    ["siberian_federal_district", "khakassia_republic"], 190911, "RU-KK", proj=32646
)
russia_region(
    ["siberian_federal_district", "krasnoyarsk_krai"], 190090, "RU-KYA", proj=32646
)
russia_region(
    ["siberian_federal_district", "novosibirsk_oblast"], 140294, "RU-NVS", proj=32644
)
russia_region(
    ["siberian_federal_district", "omsk_oblast"], 140292, "RU-OMS", proj=32643
)
russia_region(
    ["siberian_federal_district", "tomsk_oblast"], 140295, "RU-TOM", proj=32644
)
russia_region(
    ["siberian_federal_district", "tuva_republic"], 145195, "RU-TY", proj=32646
)
russia_region(
    ["siberian_federal_district", "zabaykalsky_krai"], 145730, "RU-ZAB", proj=32650
)
russia_region(
    ["southern_federal_district", "crimea_republic"], 3795586, "RU-CR", proj=32636
)
russia_region(
    ["southern_federal_district", "adygea_republic"], 253256, "RU-AD", proj=32637
)
russia_region(
    ["southern_federal_district", "astrakhan_oblast"], 112819, "RU-AST", proj=32638
)
russia_region(
    ["southern_federal_district", "kalmykia_republic"], 108083, "RU-KL", proj=32638
)
russia_region(
    ["southern_federal_district", "krasnodar_krai"], 108082, "RU-KDA", proj=32637
)
russia_region(
    ["southern_federal_district", "rostov_oblast"], 85606, "RU-ROS", proj=32637
)
russia_region(["southern_federal_district", "sevastopol"], 1574364, "RU", proj=32636)
russia_region(
    ["southern_federal_district", "volgograd_oblast"], 77665, "RU-VGG", proj=32638
)
russia_region(
    ["ural_federal_district", "chelyabinsk_oblast"], 77687, "RU-CHE", proj=32641
)
russia_region(
    ["ural_federal_district", "khanty_mansi_autonomous_okrug"],
    140296,
    "RU-KHM",
    proj=32642,
)
...
russia_region(
    ["ural_federal_district", "sverdlovsk_oblast"], 79379, "RU-SVE", proj=32641
)
russia_region(["ural_federal_district", "tyumen_oblast"], 140291, "RU-TYU", proj=32642)
russia_region(
    ["ural_federal_district", "yamalo_nenets_autonomous_okrug"],
    191706,
    "RU-YAN",
    proj=32643,
)
russia_region(
    ["volga_federal_district", "bashkortostan_republic"], 77677, "RU-BA", proj=32640
)
russia_region(
    ["volga_federal_district", "chuvash_republic"], 80513, "RU-CU", proj=32639
)
russia_region(["volga_federal_district", "kirov_oblast"], 115100, "RU-KIR", proj=32639)
russia_region(
    ["volga_federal_district", "mari_el_republic"], 115114, "RU-ME", proj=32639
)
russia_region(
    ["volga_federal_district", "mordovia_republic"], 72196, "RU-MO", proj=32638
)
russia_region(
    ["volga_federal_district", "nizhny_novgorod_oblast"], 72195, "RU-NIZ", proj=32638
)
russia_region(
    ["volga_federal_district", "orenburg_oblast"], 77669, "RU-ORE", proj=32640
)
...
russia_region(
    ["volga_federal_district", "tatarstan_republic"], 79374, "RU-TA", proj=32639
)
russia_region(
    ["volga_federal_district", "udmurt_republic"], 115134, "RU-UD", proj=32639
)
russia_region(
    ["volga_federal_district", "ulyanovsk_oblast"], 72192, "RU-ULY", proj=32639
)

#########################################################################

japan_region = gen_country(
    "asia",
    "japan",
    download_repo=OSMFR,
    country_code="JP",
...
china_province(
    "hong_kong", 913110, "CN-91", proj=32650, language=["zh", "en"], driving_side="left"
)
china_province(
    "macau", 1867188, "CN-92", proj=32649, language=["zh", "pt"], driving_side="left"
)
...
class TestPluginCommon(unittest.TestCase):
    def test_availableMethodes(self):
        ...
        polygon_ids = list(
            map(
                lambda cc: (
                    cc.polygon_id if type(cc.polygon_id) is tuple else (cc.polygon_id,)
                ),
                c,
            )
        )
        polygon_ids = reduce(concat, polygon_ids)
        duplicate_polygon_ids = list(
            map(
                lambda a: a[0],
                filter(lambda c: c[1] >= 2, Counter(polygon_ids).items()),
            )
        )
        assert len(duplicate_polygon_ids) == 0, "Duplicate relation IDs: {}".format(
            sorted(duplicate_polygon_ids)
        )
    ...
    def test_analysersExist(self):
        ...
        all_analysers = {
            i for country_list in analysers_per_country for i in country_list
        }
