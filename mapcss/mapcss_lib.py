    ...
    def __eq__(self, o):
        if self.none:
            ...
            try:
                ...
                pass  # Couldn't convert to numeric
...
def _tag_uncapture(capture, string):
    if string is not None:
        return uncapture_param_re.sub(
            lambda a: _uncapture_param(capture, a.group(1)), string
        )
...
str_value_num_wrapper2 = lambda function: lambda s, ss: str_value(
    function(str_value(s).to_n(), str_value(ss).to_n())
)
    ...
    try:
        if args is not None:
            return next(
                ...
                for item in args
                if item is not None
                and (not isinstance(item, str_value_) or not item.none)
            )
        ...
        return str_value(
            "".join(
                filter(
                    ...
                    and (not isinstance(arg, str_value_) or not arg.none),
                    args,
                )
            )
        )
...
#    test if string matches pattern regexp; flags is a string that may contain "i" (case insensitive), "m" (multiline)
#    and "s" ("dot all") [since 5699]
# regexp_match(regexp, string)
#    Tries to match string against pattern regexp. Returns a list of capture groups in case of success.
#    The first element (index 0) is the complete match (i.e. string). Further elements correspond to the bracketed parts
#    of the regular expression. [since 5701]
...
#    Tries to match string against pattern regexp. Returns a list of capture groups in case of success.
#    The first element (index 0) is the complete match (i.e. string). Further elements correspond to the bracketed parts of the
#    regular expression. Flags is a string that may contain "i" (case insensitive), "m" (multiline) and "s" ("dot all") [since 5701]
...
#    Get value from the JOSM advanced preferences. This way you can offer certain options to the user and make the style customizable.
#    It works with strings, numbers, colors and boolean values.
#    [This function exists since version 3856, but with some restrictions. JOSM_pref always returns a string, but in version 7237
#    and earlier, the automatic conversion of string to boolean and color was not working. You can use the following workarounds for
#    boolean values and color in version 7237 and earlier: cond(JOSM_pref("myprefkey", "true")="true", "X", "O") and
#    html2color(JOSM_pref("mycolor", "#FF345611")). These explicit conversions should be no longer necessary in version 7238 and later.
#    Automatic conversion to a number works in any version.]
...
# at(lat,lon)
#    returns true if the object centroid lies at given lat/lon coordinates, e.g. to check for nodes at "null island" node[at(0.0,0.0)]
#    [since 12514]

# is_similar(str1, str2)
#    returns true if the two strings are similar, but not identical, i.e., have a Levenshtein distance of 1 or 2.
#    Example: way[highway][name][is_similar(tag(name), "Main Street")] checks for streets with a possible typo in the name
#    (e.g. Main Streeg). [since r14371]
...
def inside(options, areas):
    ...
    return (
        country and any(map(lambda c: country.startswith(c), areas.split(","))) or False
    )
...
def outside(options, areas):
    ...
    return (
        country
        and all(map(lambda c: not country.startswith(c), areas.split(",")))
        or False
    )
...
def language(options, locales):
    ...
    return (
        language
        and not isinstance(language, list)
        and any(map(lambda c: language.startswith(c), locales.split(",")))
        or False
    )
...
def no_language(options, locales):
    ...
    return (
        language
        and not isinstance(language, list)
        and all(map(lambda c: not language.startswith(c), locales.split(",")))
        or False
    )
