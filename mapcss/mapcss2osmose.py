...
def simple_selector_pseudo_class(t, c):
    ...
    t["pseudo_class"] = list(
        filter(
            lambda p: not (
                p["not_class"]
                and p["pseudo_class"] in ("completely_downloaded", "in-downloaded-area")
            ),
            t["pseudo_class"],
        )
    )
...
def convert_area_selectors(t, c):
    ...
    areaselectors = list(
        filter(
            lambda selector: selector["simple_selectors"][0]["type_selector"] == "area",
            t["selectors"],
        )
    )
...
def convert_closed_pseudo_relation_node(t, c):
    ...
    for selector in t["selectors"]:
        ...
        if type_selector == "node":
            selector.update(
                {
                    "pseudo_class": list(
                        filter(
                            lambda p: not (
                                p["not_class"]
                                and p["pseudo_class"] in ("closed", "closed2")
                            ),
            ...
            selector["simple_selectors"][0]["pseudo_class"] = list(
                filter(
                    lambda p: not (
                        p["not_class"] and p["pseudo_class"] in ("closed", "closed2")
                    ),
                    selector["pseudo_class"],
                )
            )
        if type_selector == "relation":
            isPseudoClosed = any(
                map(
                    lambda p: p["pseudo_class"] in ("closed", "closed2")
                    and not p["not_class"],
                    selector["pseudo_class"],
                )
            )
            isPseudoNotClosed = any(
                map(
                    lambda p: p["pseudo_class"] in ("closed", "closed2")
                    and p["not_class"],
                    selector["pseudo_class"],
            ...
            selector.update(
                {
                    "pseudo_class": list(
                        filter(
                            lambda p: not p["pseudo_class"] in ("closed", "closed2"),
                            selector["pseudo_class"],
                        )
                    )
                }
            ...
            if isPseudoClosed:
                extra_predicate = deepcopy(mock_rules["type_eq_multipolygon"])
                extra_predicate.update(
                    {"selector_index": selector_index_map["closedrelation"]}
                )
...
def rule_exclude_throw_other(t, c):
    ...
    if not next(
        filter(
            lambda declaration: declaration["property"]
            and declaration["property"] == "-osmoseItemClassLevel",
            t["declarations"],
        ),
        False,
    ):
        t["declarations"] = list(
            filter(
                lambda declaration: not declaration["property"]
                or declaration["property"] != "throwOther",
                t["declarations"],
            )
        )
    return t
...
def rule_exclude_unsupported_meta(t, c):
    ...
    if t["selectors"][0]["simple_selectors"][0]["type_selector"] == "meta":
        ...
        t["declarations"] = list(
            filter(
                lambda declaration: not declaration["property"]
                or declaration["property"] in ("-osmoseTags",),
                t["declarations"],
            )
        )
    return t
...
def predicate_simple_dereference(t, c):
    ...
    t["predicate"] = {
        "type": "functionExpression",
        "name": "_tag_capture",
        "params": ["capture_tags", str(t["selector_index"]), "tags", t["predicate"]],
    }
    return t
...
def booleanExpression_capture_first_operand(t, c):
    ...
    if (
        len(t["operands"]) >= 1
        and t["operands"][0]["type"] == "functionExpression"
        and t["operands"][0]["name"] == "maintag"
    ):
        ...
        t["operands"][0] = {
            ...
            "params": [
                "capture_tags",
                str(t["selector_index"]),
                "tags",
                t["operands"][0]["params"][0],
            ],
        }
        if t["operator"] in ("!", "!=", "!~") and t["operands"][1]["type"] in (
            "quoted",
            "osmtag",
            "regexExpression",
        ):
            t["operands"][1] = {
                "type": "functionExpression",
                "name": "_value_const_capture",
                "params": [
                    "capture_tags",
                    str(t["selector_index"]),
                    t["operands"][1],
                    {"type": "quoted", "value": str(t["operands"][1]["value"])},
                ],
            }
        else:
            t["operands"][1] = {
                "type": "functionExpression",
                "name": "_value_capture",
                "params": ["capture_tags", str(t["selector_index"]), t["operands"][1]],
            }
    return t
...
def booleanExpression_operator_to_function(t, c):
    ...
    if operands_0["type"] == "regexExpression":
        operands_0 = {
            "type": "functionExpression",
            "name": "_match_regex",
            "params": ["tags", operands_0],
        }
    if t["operator"] == "=~":
        t = {
            "type": "functionExpression",
            "name": booleanExpression_operator_to_function_map[t["operator"]],
            "params": [t["operands"][1], operands_0],
        }
        ...
        t = {
            "type": "functionExpression",
            "name": booleanExpression_operator_to_function_map[t["operator"]],
            "params": [operands_0, t["operands"][1]],
        }
    return t
...
def pseudo_class_righthandtraffic(t, c):
    ...
    if t["pseudo_class"] == "righthandtraffic":
        setting_selector = deepcopy(
            mock_rules[
                (
                    "setting_drivingside_eq_left"
                    if t["not_class"]
                    else "setting_drivingside_neq_left"
                )
            ]
        )
        setting_selector = rewrite_tree_rules(
            rewrite_rules_clean, None, setting_selector, {}
        )
...
def rule_declarations_order(t, c):
    ...
    """
    t["declarations"] = sorted(
        t["declarations"],
        key=lambda d: (
            d.get("property")
            and [
                rule_declarations_order_map.get(d["property"])
                or print("W: Unknown property: " + d["property"])
                and -1,
                str(d["value"]),
            ]
        )
...
def selector_after_capture(t, c):
    ...
    t["_main_tags"] = list(
        map(
            lambda a: a["type"] in ("quoted", "osmtag") and a["value"] or None,
            c["selector_capture"],
        )
    )
...
def functionExpression_runtime(t, c):
    ...
    if t["name"] == "osm_id":
        ...
        t["params"] = (
            ["tags"]
            if t["name"] in ("tag", "tag_regex")
            else (
                ["data['lat']", "data['lon']"]
                if t["name"] == "at"
                else (
                    ["self.father.config.options"]
                    if t["name"]
                    in ("inside", "outside", "language", "no_language", "setting")
                    else []
                )
            )
        ) + t["params"]
...
def checkValidFunction(fn_name, num_params):
    ...
    if not fn_name[7:] in dir(mapcss_lib):
        raise NotImplementedError(
            "Undefined function '{0}'. Blacklist or implement to avoid errors".format(
                fn_name
            )
        )
    else:
        ...
        has_vararg = any(
            map(lambda p: p.kind == Parameter.VAR_POSITIONAL, sig.parameters.values())
        )
        num_optional_arg = len(
            set(filter(lambda p: p.default != Parameter.empty, sig.parameters.values()))
        )
        if has_vararg:
            argcount = argcount - 1  # *args can be zero-length
        if num_params < argcount - num_optional_arg or (
            not has_vararg and num_params > argcount
        ):
            raise NotImplementedError(
                "Undefined function '{0}' with {1} arguments. Blacklist or implement to avoid errors".format(
                    fn_name, num_params
                )
            )
...
def rewrite_tree_rules(before_rules, after_rules, t, c):
    if isinstance(t, str):
        ...
        t = list(
            map(lambda tt: rewrite_tree_rules(before_rules, after_rules, tt, c), t)
        )
    return t
...
def rewrite_tree(t):
    ...
    t = rewrite_tree_rules(
        rewrite_rules_change_before, rewrite_rules_change_after, t, {}
    )
    return t
...
def segregate_selectors_by_complexity(t):
        ...
        if rule.get("_meta"):
            ...
            for selector in rule["selectors"]:
                if selector["operator"]:
                    ...
                    map(
                        lambda a: not a["pseudo_class"]
                        in ("closed", "closed2", "righthandtraffic"),
    ...
    return {
        ...
        "rules_complex": rules_complex,
        "rules_simple": rules_simple,
    }
...
def segregate_selectors_type(rules):
        ...
        if rule.get("_meta"):
            ...
            for t in "node", "way", "relation":
                if out_selector[t]:
                    ...
                    out_rules[t][-1]["declarations"] = list(
                        filter(
                            lambda d: not d["property"]
                            or not (
                                d["property"].startswith("assert")
                                or d["property"].startswith("-osmoseAssert")
                            )
                            or (
                                d["value"]["type"] == "single_value"
                                and d["value"]["value"]["value"].startswith(t)
                            )
                            or (
                                d["value"]["type"] == "functionExpression"
                                and d["value"]["params"][0]["value"].startswith(t)
                            ),
    ...
    return dict(
        filter(
            lambda kv: next(filter(lambda rule: not rule.get("_meta"), kv[1]), False),
            out_rules.items(),
        )
    )
...
def filter_non_productive_rules(rules):
    return list(
        filter(
            lambda rule: rule.get("_meta")
            or next(
                filter(
                    lambda declaration: (
                        declaration["property"]
                        and declaration["property"].startswith("throw")
                    )
                    or declaration["set"],
                    rule["declarations"],
                ),
            ...
            or print("W: Skip non productive rule"),
            rules,
        )
    )
...
def filter_osmose_none_rules(rules):
    return list(
        filter(
            ...
            or not next(
                filter(
                    lambda declaration: declaration.get("property")
                    == "-osmoseItemClassLevel"
                    and declaration["value"].get("type") == "single_value"
                    and declaration["value"]["value"]["value"] == "none",
                    rule["declarations"],
                ),
                None,
            ),
...
def filter_typeselector_rules(rules):
    return list(
        filter(
            lambda rule: rule.get("_meta") or len(rule["selectors"]) > 0,
            map(
                lambda rule: rule.get("_meta")
                and rule
                or rule.update(
                    {
                        "selectors": list(
                            filter(
                                lambda selector: selector["simple_selectors"][0][
                                    "type_selector"
                                ]
                                in ("node", "way", "relation", "*")
                                or print("W: Skip unknown type selector"),
                                rule["selectors"],
                            )
                        )
                    }
...
def filter_support_rules(rules):
    ...
    return list(
        filter(
            lambda rule: rule.get("_meta")
            or not rule.get("in_supports_declaration")
            or next(
                filter(
                    lambda declaration: declaration.get("property")
                    == "-osmoseItemClassLevel",
...
def to_p(t):
    ...
    if isinstance(t, str):
        ...
        item = class_id = level = tags = group = group_class = text = text_class = (
            None  # For safety
        )
        ...
        if subclass_id in subclass_blacklist:
            ...
            main_tags = tuple(
                set(
                    map(
                        lambda s: tuple(
                            set(filter(lambda z: z is not None, s.get("_main_tags")))
                        ),
                        t["selectors"],
                    )
                )
            ...
            fix = dict(
                map(
                    lambda kv: [
                        {"fixAdd": "+", "fixChangeKey": "~", "fixRemove": "-"}[kv[0]],
                        kv[1],
                    ],
                    filter(lambda kv: len(kv[1]) > 0, fix.items()),
                )
            )
            fix = len(fix) > 0 and map(
                lambda om: "'"
                + om[0]
                + "': "
                + ("dict" if om[0] != "-" else "")
            ...
            return (
                ...
                + "        # "
                + "\n        # ".join(
                    filter(lambda a: a, map(lambda d: d["text"], t["declarations"]))
                )
                + "\n"
                + (
                ...
                + (
                    (
                        ...
                        + ", "
                        + "'subclass': "
                        + str(subclass_id or 0)
                        + ", "
                        + "'text': "
                        + (
                            text
                            if text.startswith("mapcss.tr")
                            else "{'en': " + text + "}"
                        )
        ...
        if t["operator"]:
            ...
            return (
                "    # Skip selector using undeclared class "
                + ", ".join(sorted(t["_require_set"]))
                + "\n"
            )
        else:
            return (
                ...
                + "        capture_tags = {}\n"
                + "        try: match = "
                + " and ".join(map(to_p, t["simple_selectors"]))
                + "\n"
        ...
        sp = (
            list(map(to_p, t["class_selectors"]))
            + list(map(to_p, t["predicates"]))
            + list(map(to_p, t["pseudo_class"]))
        )
        return "((" + ") and (".join(sp) + "))"
        ...
        return (
            ...
            + to_p(t["predicate"])
            + (
                (" not" if t["question_mark_negated"] else "")
                + " in ('yes', 'true', '1')"
                if t["question_mark"] or t["question_mark_negated"]
                else ""
            )
        )
        ...
        # Standard propoerties
            ...
            group_class = (
                ...
                and t["value"]["name"] == "mapcss.tr"
                else t["value"]
            )
            group_class = (
                group_class["value"]["value"]
                if group_class["type"] == "single_value"
                and group_class["value"]["type"] == "quoted"
                else to_p(group_class["value"])
            )
        elif t["property"] == "-osmoseItemClassLevel":
            item, class_id, level = t["value"]["value"]["value"].split("/")
            item, class_id, subclass_id, level = (
                int(item),
                int(class_id.split(":")[0]),
                ":" in class_id and int(class_id.split(":")[1]),
                int(level),
            )
        elif t["property"] in (
            "-osmoseDetail",
            ...
            if (
                ...
                class_info_text[whichMsg] = (
                    text  # hyperlink as string, no need for language
                )
            else:
            ...
            text_class = (
                text_class["value"]["value"]
                if text_class["type"] == "single_value"
                and text_class["value"]["type"] == "quoted"
                else to_p(text_class["value"])
            )
            if not class_id:
            ...
            if (
                t["value"]["type"] == "single_value"
                and t["value"]["value"]["type"] == "quoted"
            ):
                fix[t["property"]].append(
                    "["
                    + ",".join(
                        map(
                            lambda a: "'" + a.strip().replace("'", "\\'") + "'",
                            t["value"]["value"]["value"].split("=", 1),
                        )
                    )
                    + "]"
                )
            ...
            if (
                ...
                fix["fixAdd"].append(
                    "[" + l + "[1].strip(), mapcss.tag(tags, " + l + "[0].strip())]"
                )
            ...
            # fix[
            #    "fixRemove"
            # ] == "*keys"  # TODO delete the object completely in place of removing all tags
            # Commented because of error B015: Result of comparison is not used. This line doesn't do anything.
            # Did you intend to prepend it with assert?
        elif t["property"].startswith("assert") or t["property"].startswith(
            "-osmoseAssert"
        ):
            if t["value"]["type"] == "single_value":
                ...
                what, context = (
                    to_p(t["value"]["params"][0]),
                    to_p(t["value"]["params"][1])[1:-1],
                )
            tests.append(
                {
                    "type": t["property"],
        ...
        if t["value"] in regex_store:
            ...
            regex_var = regex_store[t["value"], t.get("flags")] = (
                "re_%08x" % stablehash(t["value"] + t.get("flags", ""))
            )
...
def build_mock_rules():
    ...
    for f in list(map(lambda fn: fn[0:-7], files)):
        ...
        r = listener.stylesheet["rules"][0]["selectors"][0]["simple_selectors"][0][
            "predicates"
        ][0]
...
def build_tests(tests):
    ...
    for test in tests:
        ...
        if test["context"]:
            context = dict(
                map(
                    lambda l: map(lambda s: s.strip(), l.split("=", 1)),
                    test["context"].split(","),
                )
            )
            context = dict(
                map(
                    lambda kv: (
                        context_map[kv[0]] if kv[0] in context_map else kv[0],
                        kv[1],
                    ),
                    context.items(),
                )
        ...
        o, kvs = okvs[0], list(
            map(
                lambda a: a[0] in "\"'" and a[0] == a[-1] and a[1:-1] or a,
                map(lambda a: a.replace('\\"', '"').replace("\\'", "'"), okvs[1:]),
            )
        )
        ...
        test_code += (
            ...
            + (
                "check_err"
                if test["type"].startswith("assertMatch")
                or test["type"].startswith("-osmoseAssertMatch")
                else "check_not_err"
            )
            + "("
            + "n."
...
def compile(inputfile, class_name, mapcss_url=None, only_for=[], not_for=[], prefix=""):
    ...
    if len(selectors_by_complexity["rules_complex"]) > 0:
        print(
            "W: Drop %d complex rules" % len(selectors_by_complexity["rules_complex"])
        )
    tree = rewrite_tree(
        selectors_by_complexity["rules_meta"] + selectors_by_complexity["rules_simple"]
    )
    ...
    rules = dict(
        map(
            lambda t: [t, to_p({"type": "stylesheet", "rules": selectors_type[t]})],
            sorted(
                selectors_type.keys(),
                key=lambda a: {"node": 0, "way": 1, "relation": 2}[a],
            ),
        )
    )
...
"""
    ...
    def init(self, logger):
        ...
        self."""
                + r[1]
                + " = re.compile(r'"
                + r[0].replace("(?U)", "").replace("'", "\\'")
                + "'"
                + (", " + {"i": "re.I", "m": "re.M", "s": "re.I"}[r[2]] if r[2] else "")
                + ")",
                map(
                    lambda a: [a[0][0], a[1], a[0][1]],
                    sorted(regex_store.items(), key=lambda s: s[1]),
                ),
            )
        )
        + """
...
"""
    ...
    def """
                + t
                + """(self, data, tags"""
                + {"node": "", "way": ", nds", "relation": ", members"}[t]
                + """):
        capture_tags = {}
        keys = tags.keys()
        err = []
        """
                + (
                    (
                        " = ".join(list(map(lambda s: "set_" + s, sorted(set_store))))
                        + " = False"
                    )
                    if len(set_store) > 0
                    else ""
...
""",
                sorted(
                    rules.keys(), key=lambda a: {"node": 0, "way": 1, "relation": 2}[a]
                ),
            )
        )
        + """
...
def mapcss2osmose(mapcss, output_path=None):
    class_name = original_class_name = ".".join(
        os.path.basename(mapcss).replace(".validator.", ".").split(".")[:-1]
    )
    global item_default, class_map, subclass_blacklist, class_index, meta_tags
    if class_name in item_map:
        ...
        meta_tags = (
            ('["' + ('", "').join(i.get("tags")) + '"]') if i.get("tags") else None
        )
    else:
        item_default = 0
        class_map = {}
