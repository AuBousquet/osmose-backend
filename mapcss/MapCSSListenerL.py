...
class MapCSSListenerL(MapCSSListener):
    ...
    def exitRule_(self, ctx: MapCSSParser.Rule_Context):
        self.rules.append(
            {
                "type": "rule",
                "selectors": self.selectors or self.attribute_selectors,
                "declarations": self.declarations,
                "in_supports_declaration": self.supportCondition is not None,
            }
        )
        ...
        self.selectors.append(
            {
                ...
                "operator": (
                    ctx.simple_selector_operator()
                    and ctx.simple_selector_operator().getText()
                )
                or (
                    ctx.parent_child_selector_operator()
                    and ctx.parent_child_selector_operator().getText()
                ),
    ...
    def enterPredicate(self, ctx: MapCSSParser.PredicateContext):
        self.stack.append({"predicate_simple": None, "booleanExpressions": []})
    ...
    def exitPredicate(self, ctx: MapCSSParser.PredicateContext):
        ...
        self.predicates.append(
            predicate["predicate_simple"] or predicate["booleanExpressions"][0]
        )
    ...
    def enterPredicate_simple(self, ctx: MapCSSParser.Predicate_simpleContext):
        self.stack.append({"quoted": None, "osmtag": None, "regexExpression": []})
    ...
    def enterDeclaration(self, ctx: MapCSSParser.DeclarationContext):
        self.stack.append({"single_value": None, "functionExpression": None})
        ...
        self.declarations.append(
            {
                "type": "declaration",
                "text": (
                    re.sub(r"^set", "set ", ctx.getText(), flags=re.IGNORECASE)
                    if ctx.cssident()
                    else ctx.getText()
                ),
                "set": ctx.cssident() and ctx.cssident().getText(),
                "property": ctx.declaration_property()
                and ctx.declaration_property().getText(),
    ...
    def enterSingle_value(self, ctx: MapCSSParser.Single_valueContext):
        self.stack.append({"quoted": None, "osmtag": None})
        ...
        self.stack[-1]["booleanExpressions"].append(
            {
                ...
                "operator": (
                    None
                    if v["functionExpression"]
                    else (ctx.op and ctx.op.text)
                    or (
                        ctx.booleanOperator()
                        or ctx.valueOperator()
                        or ctx.regexOperator()
                    ).getText()
                ),
                "operands": v["booleanExpressions"]
                or (  # Juste get operands array
                    v["functionExpression"] and [v["functionExpression"]]
                )
                or [],
                "selector_index": self.selector_index,
            }
        ...
        self.stack[-1]["valueExpressions"].append(
            {
                ...
                "operands": (len(v["valueExpressions"]) > 0 and v["valueExpressions"])
                or [v["primaryExpression"] or v["functionExpression"]],
            }
        )
    ...
    def exitZoom_selector(self, ctx: MapCSSParser.Zoom_selectorContext):
        self.zoom_selector = {"type": "zoom_selector", "value": ctx.getText()}
    ...
    def exitQuoted(self, ctx: MapCSSParser.QuotedContext):
        self.stack[-1]["quoted"] = {"type": "quoted", "value": ctx.getText()}
    ...
    def exitOsmtag(self, ctx: MapCSSParser.OsmtagContext):
        self.stack[-1]["osmtag"] = {"type": "osmtag", "value": ctx.getText()}
    ...
    def exitPrimaryExpression(self, ctx: MapCSSParser.PrimaryExpressionContext):
        ...
        self.stack[-1]["primaryExpression"] = {
            ...
            "value": (ctx.v and ctx.v.text)
            or v["osmtag"]
            or v["quoted"]
            or v["regexExpression"][0],
        }
